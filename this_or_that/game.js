/**
 * games/this_or_that/game.js
 * Game logic for "This or That? (Tinder for Things & Concepts)"
 * Supports CEFR Levels (A0-A1, A2, B1, B2) & Rich Tinder Decks:
 * - Physical Appearance
 * - Professions
 * - Properties (Homes, Flats, Lofts, Chalets)
 * - Character Traits & Personality
 * - Hobbies & Passions
 */
(function() {
    const GAME_ID = 'thisorthat';
    const GAME_TITLE = 'This or That? 🔥';
    const GAME_META = 'Tinder Profiles & Speaking Drills (A0–B2 CEFR)';

    // CEFR Level Organized Decks
    const CEFR_DECKS = {
  "A0_A1": {
    "appearance": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Physical Appearance 👁️",
        "title": "Anna",
        "age": "22",
        "location": "📍 1 mile away • City Center",
        "verified": true,
        "avatar": "👩",
        "gradient": "linear-gradient(135deg, #f43f5e 0%, #fb7185 100%)",
        "visualDescription": "Young woman with short black hair and brown eyes. She wears a bright red sweater and white sneakers.",
        "interests": [
          "#Tall",
          "#BlackHair",
          "#RedSweater"
        ],
        "anthem": "🎵 'Shake It Off' by Taylor Swift",
        "film": "🎬 'Paddington 2' (Paul King)",
        "pages": [
          {
            "tag": "🖼️ Physical Look",
            "bio": "Hi! I am Anna. I am 22 years old. I am tall. I have short black hair and brown eyes. I wear a bright red sweater, blue jeans, and white shoes.",
            "greenFlags": [
              "✓ Big friendly smile",
              "✓ Tall and slim",
              "✓ Easy to see"
            ],
            "redFlags": [
              "✗ Always late for shoes",
              "✗ Wears only red",
              "✗ Speaks very fast"
            ]
          },
          {
            "tag": "⚡ Daily Routine",
            "bio": "I wake up early every morning. I drink milk and eat fresh apples. I walk in the park with my dog. I like happy music!",
            "greenFlags": [
              "✓ Loves walking outdoors",
              "✓ Kind to pets",
              "✓ Early riser"
            ],
            "redFlags": [
              "✗ Dog barks loudly",
              "✗ Sings early in the morning",
              "✗ Leaves shoes everywhere"
            ]
          },
          {
            "tag": "💭 Favorites",
            "bio": "I love sunny summer days and fruit ice cream. I do not like rainy days or cold tea. I want to meet friendly people!",
            "greenFlags": [
              "✓ Very positive mood",
              "✓ Enjoys simple food",
              "✓ Easy to talk to"
            ],
            "redFlags": [
              "✗ Hates cold weather",
              "✗ Avoids rainy walks",
              "✗ Picky about drinks"
            ]
          }
        ],
        "opener": "Hello! I am Anna. Do you like red sweaters or blue jeans?",
        "icebreakers": [
          "I like red sweaters!",
          "I prefer blue jeans!",
          "You have a nice smile!"
        ],
        "prompt": "Describe Anna. What color is her hair? What clothes does she wear?"
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Physical Appearance 👁️",
        "title": "Marco",
        "age": "25",
        "location": "📍 3 miles away • Near Park",
        "verified": true,
        "avatar": "🧔",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #38bdf8 100%)",
        "visualDescription": "Tall young man with curly brown hair, green eyes, and black glasses. He wears a green t-shirt and blue denim jacket.",
        "interests": [
          "#CurlyHair",
          "#Beard",
          "#Glasses"
        ],
        "anthem": "🎵 'Happy' by Pharrell Williams",
        "film": "🎬 'Finding Nemo' (Pixar)",
        "pages": [
          {
            "tag": "🖼️ Physical Look",
            "bio": "Hello! I am Marco, 25 years old. I am short and active. I have brown curly hair and green eyes. I wear a navy jacket and dark blue jeans.",
            "greenFlags": [
              "✓ Warm green eyes",
              "✓ Nice dark jacket",
              "✓ Energetic look"
            ],
            "redFlags": [
              "✗ Messy curly hair",
              "✗ Always wears sneakers",
              "✗ Forgets his jacket"
            ]
          },
          {
            "tag": "⚡ Daily Life",
            "bio": "I work in a small shop. I ride my yellow bicycle to work. I play soccer with my friends on Saturdays. I am very fast!",
            "greenFlags": [
              "✓ Rides bike every day",
              "✓ Plays team sports",
              "✓ Hard worker"
            ],
            "redFlags": [
              "✗ Only talks about soccer",
              "✗ Rides bike very fast",
              "✗ Tired on Saturday nights"
            ]
          }
        ],
        "opener": "Hi! Do you wear glasses or have curly hair?",
        "icebreakers": [
          "I wear glasses too!",
          "I love green eyes!",
          "Cool denim jacket!"
        ],
        "prompt": "What color are Marco's eyes and hair? Does he wear glasses?"
      }
    ],
    "professions": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Professions 💼",
        "title": "Chef John",
        "age": "30",
        "location": "📍 0.5 miles away • Sunrise Bakery",
        "verified": true,
        "avatar": "👨‍🍳",
        "gradient": "linear-gradient(135deg, #d97706 0%, #f59e0b 100%)",
        "visualDescription": "Man in a white hat and apron. He makes fresh bread and cakes in a bakery.",
        "interests": [
          "#FreshBread",
          "#Bakery",
          "#Cakes"
        ],
        "anthem": "🎵 'Sweet Caroline' by Neil Diamond",
        "film": "🎬 'Chef' (Jon Favreau)",
        "pages": [
          {
            "tag": "🖼️ Kitchen Look",
            "bio": "I am Chef John. I wear a clean white chef coat and a tall white hat. I work in a busy kitchen. I have short brown hair and a friendly face.",
            "greenFlags": [
              "✓ Clean white uniform",
              "✓ Friendly smile",
              "✓ Experienced cook"
            ],
            "redFlags": [
              "✗ Smells like garlic",
              "✗ Messy apron after work",
              "✗ Wears chef hat indoors"
            ]
          },
          {
            "tag": "⚡ Daily Work",
            "bio": "I cook hot soup and fresh bread every morning. I love good food! I make sweet cakes for birthday parties.",
            "greenFlags": [
              "✓ Bakes fresh bread",
              "✓ Makes delicious soup",
              "✓ Loves sweet desserts"
            ],
            "redFlags": [
              "✗ Tastes food while cooking",
              "✗ Kitchen gets very hot",
              "✗ Works late hours"
            ]
          },
          {
            "tag": "💭 Food Values",
            "bio": "Good food makes people happy. I use fresh vegetables and clean water. I like eating meals with good friends!",
            "greenFlags": [
              "✓ Uses fresh ingredients",
              "✓ Shares meals with friends",
              "✓ Passionate about cooking"
            ],
            "redFlags": [
              "✗ Criticizes fast food",
              "✗ Eats dinner very late",
              "✗ Chooses menu for everyone"
            ]
          }
        ],
        "opener": "Good morning! Do you like fresh bread or sweet cake?",
        "icebreakers": [
          "Fresh bread please!",
          "Chocolate cake for me!",
          "I love croissants!"
        ],
        "prompt": "What does Chef John make every morning? Is his job early or late?"
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Professions 💼",
        "title": "Dr. Sarah",
        "age": "31",
        "location": "📍 2 miles away • City Hospital",
        "verified": true,
        "avatar": "👩‍⚕️",
        "gradient": "linear-gradient(135deg, #059669 0%, #34d399 100%)",
        "visualDescription": "Doctor wearing a white coat and blue stethoscope in a bright clinic.",
        "interests": [
          "#Doctor",
          "#WhiteCoat",
          "#Health"
        ],
        "anthem": "🎵 'Fix You' by Coldplay",
        "film": "🎬 'Patch Adams' (Tom Shadyac)",
        "pages": [
          {
            "tag": "🖼️ Doctor Profile",
            "bio": "I am Dr. Sarah, 29 years old. I wear a white doctor coat and carry a stethoscope. I have long brown hair tied in a pony tail.",
            "greenFlags": [
              "✓ Calm and gentle voice",
              "✓ Clean professional coat",
              "✓ Good listener"
            ],
            "redFlags": [
              "✗ Phone rings constantly",
              "✗ Wears clinical shoes",
              "✗ Always checking the clock"
            ]
          },
          {
            "tag": "⚡ Hospital Life",
            "bio": "I help sick people get healthy. I check heartbeats and give helpful advice. I work at a big city hospital every day.",
            "greenFlags": [
              "✓ Helps people every day",
              "✓ Patient and caring",
              "✓ Works in city center"
            ],
            "redFlags": [
              "✗ Night shifts on weekends",
              "✗ Quick rushed lunches",
              "✗ Very busy schedule"
            ]
          }
        ],
        "opener": "Hello! How are you today? Do you feel good?",
        "icebreakers": [
          "I feel great!",
          "I have a headache",
          "Thank you doctor!"
        ],
        "prompt": "Where does Dr. Sarah work? What clothes does she wear?"
      }
    ],
    "properties": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Properties 🏠",
        "title": "Leo",
        "age": "24",
        "location": "📍 0.8 miles away • Sunny Avenue",
        "verified": true,
        "avatar": "🏙️",
        "gradient": "linear-gradient(135deg, #8b5cf6 0%, #c084fc 100%)",
        "visualDescription": "Owner of a cozy studio flat with a big sunny window, yellow sofa, and small kitchen.",
        "interests": [
          "#StudioFlat",
          "#SunnyWindow",
          "#YellowSofa"
        ],
        "anthem": "🎵 'Home' by Edward Sharpe",
        "film": "🎬 '500 Days of Summer' (Marc Webb)",
        "pages": [
          {
            "tag": "🖼️ Flat Overview",
            "bio": "Hi, I am Leo! I live in a cozy studio flat. It has one bright room, a red sofa, a small wooden table, and a big window.",
            "greenFlags": [
              "✓ Cozy red sofa",
              "✓ Big sunny window",
              "✓ Clean wooden floor"
            ],
            "redFlags": [
              "✗ Very small space",
              "✗ No private balcony",
              "✗ Tiny kitchen sink"
            ]
          },
          {
            "tag": "⚡ Living Routine",
            "bio": "I read books on my red sofa. I cook simple pasta in my small kitchen. My flat is quiet and near a metro station.",
            "greenFlags": [
              "✓ Near metro station",
              "✓ Quiet living area",
              "✓ Perfect for reading"
            ],
            "redFlags": [
              "✗ No space for big parties",
              "✗ Cooking smell stays inside",
              "✗ Only two chairs"
            ]
          },
          {
            "tag": "💭 Home Ideas",
            "bio": "A good home is warm and simple. I like green plants and soft lights. I want a peaceful place to rest after work.",
            "greenFlags": [
              "✓ Nice house plants",
              "✓ Warm soft lighting",
              "✓ Relaxing atmosphere"
            ],
            "redFlags": [
              "✗ Dislikes loud visitors",
              "✗ Strict about shoes inside",
              "✗ Early bedtime routine"
            ]
          }
        ],
        "opener": "Welcome! Do you like small cozy flats or big houses?",
        "icebreakers": [
          "I love cozy studio flats!",
          "I need a big garden!",
          "Great price!"
        ],
        "prompt": "Name 3 things inside Leo's studio flat using simple words."
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Properties 🏠",
        "title": "Lily",
        "age": "26",
        "location": "📍 2 miles away • Green Village",
        "verified": true,
        "avatar": "🏡",
        "gradient": "linear-gradient(135deg, #16a34a 0%, #4ade80 100%)",
        "visualDescription": "Small wooden cottage with a sunny front garden and blooming red roses.",
        "interests": [
          "#SmallCottage",
          "#GreenGarden",
          "#Flowers"
        ],
        "anthem": "🎵 'Wildflowers' by Tom Petty",
        "film": "🎬 'The Secret Garden' (Marc Munden)",
        "pages": [
          {
            "tag": "🖼️ Cottage Overview",
            "bio": "Hi, I am Lily! I live in a sweet small house with a big garden. It has three rooms, white walls, and bright red flowers outside.",
            "greenFlags": [
              "✓ Pretty garden",
              "✓ Fresh flowers",
              "✓ Quiet neighborhood"
            ],
            "redFlags": [
              "✗ Far from city center",
              "✗ Requires daily garden watering",
              "✗ Small kitchen"
            ]
          },
          {
            "tag": "⚡ Garden Life",
            "bio": "I grow tomatoes and yellow sunflowers in my garden. I drink tea outside every afternoon when it is sunny.",
            "greenFlags": [
              "✓ Fresh vegetables",
              "✓ Sunny outdoor tea spot",
              "✓ Peaceful birdsong"
            ],
            "redFlags": [
              "✗ Insects in summer",
              "✗ Mowing grass on Saturdays",
              "✗ Cold in winter"
            ]
          }
        ],
        "opener": "Hello! Do you like small houses with gardens or city flats?",
        "icebreakers": [
          "I love houses with gardens!",
          "City flats are easier!",
          "Pretty roses!"
        ],
        "prompt": "What does Lily grow in her garden? Describe her cottage."
      }
    ],
    "character": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Character Traits 🧠",
        "title": "Mia",
        "age": "20",
        "location": "📍 1 mile away • Neighborhood",
        "verified": true,
        "avatar": "😊",
        "gradient": "linear-gradient(135deg, #f59e0b 0%, #fcd34d 100%)",
        "visualDescription": "Smiling young person holding a warm cup of cocoa.",
        "interests": [
          "#Friendly",
          "#KindHeart",
          "#GoodFriend"
        ],
        "anthem": "🎵 'Count on Me' by Bruno Mars",
        "film": "🎬 'Inside Out' (Pixar)",
        "pages": [
          {
            "tag": "🖼️ Mia's Profile",
            "bio": "Hi, I am Mia! I am a kind and friendly person. I have short blonde hair, blue eyes, and a warm smile for everyone.",
            "greenFlags": [
              "✓ Always smiling",
              "✓ Kind personality",
              "✓ Easy to talk to"
            ],
            "redFlags": [
              "✗ Talks a lot",
              "✗ Laughs very loudly",
              "✗ Forgets names easily"
            ]
          },
          {
            "tag": "⚡ How I Help",
            "bio": "I love helping my friends. I help my neighbors carry heavy groceries. I share my umbrellas when it rains.",
            "greenFlags": [
              "✓ Helps neighbors",
              "✓ Shares umbrellas",
              "✓ Generous heart"
            ],
            "redFlags": [
              "✗ Too trusting of strangers",
              "✗ Says yes to everything",
              "✗ Gets tired helping everyone"
            ]
          }
        ],
        "opener": "Hello my friend! How are you today?",
        "icebreakers": [
          "You are so nice!",
          "Let us drink tea together!",
          "Have a great day!"
        ],
        "prompt": "What does Mia do as a kind person? Describe 2 simple actions."
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Character Traits 🧠",
        "title": "David",
        "age": "25",
        "location": "📍 0.5 miles away • Maple Street",
        "verified": true,
        "avatar": "😇",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #38bdf8 100%)",
        "visualDescription": "Quiet young man smiling kindly while holding open a door.",
        "interests": [
          "#Polite",
          "#QuietAndKind",
          "#HelpfulNeighbor"
        ],
        "anthem": "🎵 'You've Got a Friend' by Carole King",
        "film": "🎬 'Paddington' (Paul King)",
        "pages": [
          {
            "tag": "🖼️ David's Profile",
            "bio": "Hello! I am David. I am a calm and polite neighbor. I have short brown hair and wear clean blue shirts.",
            "greenFlags": [
              "✓ Polite and gentle",
              "✓ Quiet neighbor",
              "✓ Always holds the door"
            ],
            "redFlags": [
              "✗ Very shy at first",
              "✗ Speaks quietly",
              "✗ Rare party visitor"
            ]
          },
          {
            "tag": "⚡ Neighbor Life",
            "bio": "I collect packages for my neighbors when they are at work. I water plants when friends travel.",
            "greenFlags": [
              "✓ Helps neighbors",
              "✓ Trustworthy keyholder",
              "✓ Very reliable"
            ],
            "redFlags": [
              "✗ Never stays late",
              "✗ Prefers quiet evenings",
              "✗ Shy in large groups"
            ]
          }
        ],
        "opener": "Hi! Do you prefer quiet neighbors or energetic friends?",
        "icebreakers": [
          "Quiet neighbors are great!",
          "Energetic friends!",
          "Thank you David!"
        ],
        "prompt": "How does David help his neighbors on Maple Street?"
      }
    ],
    "hobbies": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Hobbies 🎨",
        "title": "Oliver",
        "age": "23",
        "location": "📍 0.3 miles away • River Park",
        "verified": true,
        "avatar": "🚴",
        "gradient": "linear-gradient(135deg, #0ea5e9 0%, #38bdf8 100%)",
        "visualDescription": "Red bicycle parked near a green tree in a park.",
        "interests": [
          "#Bicycle",
          "#Park",
          "#Exercise"
        ],
        "anthem": "🎵 'Bicycle Race' by Queen",
        "film": "🎬 'E.T. the Extra-Terrestrial' (Steven Spielberg)",
        "pages": [
          {
            "tag": "🖼️ Hobby Look",
            "bio": "I am Oliver, 26. My favorite hobby is cycling in the park! I wear a blue bicycle helmet and bright sports shoes.",
            "greenFlags": [
              "✓ Healthy active hobby",
              "✓ Safe helmet user",
              "✓ Loves fresh air"
            ],
            "redFlags": [
              "✗ Bike dirty from mud",
              "✗ Always wears sports shoes",
              "✗ Leaves helmet on table"
            ]
          },
          {
            "tag": "⚡ Park Rides",
            "bio": "I ride my bike for one hour every evening. I see green trees and birds in the park. It makes me feel happy and strong.",
            "greenFlags": [
              "✓ Daily exercise habit",
              "✓ Enjoys green parks",
              "✓ Great physical energy"
            ],
            "redFlags": [
              "✗ Rides in light rain",
              "✗ Talks about bikes constantly",
              "✗ Skips movie nights for cycling"
            ]
          },
          {
            "tag": "💭 Fitness Dreams",
            "bio": "I want to ride my bike near the ocean one day. I like fresh air, sunshine, and active weekends with good friends!",
            "greenFlags": [
              "✓ Loves ocean trips",
              "✓ Active weekend plans",
              "✓ Inspiring outdoor energy"
            ],
            "redFlags": [
              "✗ Dislikes sitting inside",
              "✗ Wakes up at 6 AM on Sundays",
              "✗ Only plans bicycle trips"
            ]
          }
        ],
        "opener": "Hello! Can you ride a bicycle? Do you ride in the park?",
        "icebreakers": [
          "Yes, I ride every week!",
          "No, but I want to learn!",
          "I prefer walking!"
        ],
        "prompt": "Where does Oliver like to ride his bicycle?"
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Hobbies 🎨",
        "title": "Emma",
        "age": "21",
        "location": "📍 1 mile away • Rose Garden",
        "verified": true,
        "avatar": "🌻",
        "gradient": "linear-gradient(135deg, #f59e0b 0%, #fcd34d 100%)",
        "visualDescription": "Young woman in a sunhat holding a wooden watering can near potted flowers.",
        "interests": [
          "#Gardening",
          "#FreshFlowers",
          "#NatureLover"
        ],
        "anthem": "🎵 'Sunflower' by Post Malone",
        "film": "🎬 'A Bug's Life' (Pixar)",
        "pages": [
          {
            "tag": "🖼️ Gardening Look",
            "bio": "I am Emma! My favorite hobby is growing colorful plants and flowers on my balcony. I wear a big sunhat and yellow gloves.",
            "greenFlags": [
              "✓ Beautiful balcony flowers",
              "✓ Calm outdoor hobby",
              "✓ Loves nature"
            ],
            "redFlags": [
              "✗ Dirt under nails",
              "✗ Water drops on balcony below",
              "✗ Talks to her plants"
            ]
          },
          {
            "tag": "⚡ Flower Care",
            "bio": "I water my red tulips and green ferns every morning. Green plants make my home feel fresh and happy!",
            "greenFlags": [
              "✓ Daily watering routine",
              "✓ Bright fresh home",
              "✓ Loves green leaves"
            ],
            "redFlags": [
              "✗ Balcony full of plant pots",
              "✗ Wakes up early to water plants",
              "✗ Dislikes plastic plants"
            ]
          }
        ],
        "opener": "Hello! Do you have plants or flowers at home?",
        "icebreakers": [
          "I have houseplants!",
          "I love tulips!",
          "Yellow sunhat is cute!"
        ],
        "prompt": "What plants does Emma water every morning?"
      }
    ],
    "nationalities": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Nationalities & Cultures 🌍",
        "title": "Kenji",
        "age": "24",
        "location": "📍 Tokyo, Japan • 🇯🇵 Japanese",
        "verified": true,
        "avatar": "🇯🇵",
        "gradient": "linear-gradient(135deg, #dc2626 0%, #f87171 100%)",
        "visualDescription": "Young Japanese man with neat short dark hair, wearing a traditional yukata.",
        "interests": [
          "#Japanese",
          "#Tokyo",
          "#Ramen"
        ],
        "anthem": "🎵 'Sukiyaki' by Kyu Sakamoto",
        "film": "🎬 'Spirited Away' (Hayao Miyazaki)",
        "pages": [
          {
            "tag": "🖼️ Tokyo Profile",
            "bio": "Konnichiwa! I am Kenji from Tokyo, Japan. I have short straight dark hair and brown eyes. I wear dark blue clothes and clean white sneakers.",
            "greenFlags": [
              "✓ Very polite and neat",
              "✓ Stylish blue clothes",
              "✓ Respectful greeting"
            ],
            "redFlags": [
              "✗ Bows repeatedly",
              "✗ Very quiet at first",
              "✗ Wears slippers everywhere"
            ]
          },
          {
            "tag": "⚡ Tokyo Life",
            "bio": "I live near Shibuya in Tokyo. I ride fast trains to work. I eat hot ramen noodles and green tea ice cream.",
            "greenFlags": [
              "✓ Knows Tokyo spots",
              "✓ Loves ramen soup",
              "✓ Punctual train rider"
            ],
            "redFlags": [
              "✗ Eats noodles very fast",
              "✗ Drinks green tea all day",
              "✗ Walks very quickly"
            ]
          }
        ],
        "opener": "Konnichiwa! What is your nationality? Where are you from?",
        "icebreakers": [
          "I am from Spain!",
          "I love Japanese food!",
          "Tokyo is a big city!"
        ],
        "prompt": "What is Kenji's nationality and home country? What food does he eat?"
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Nationalities & Cultures 🌍",
        "title": "Sofia",
        "age": "23",
        "location": "📍 Rome, Italy • 🇮🇹 Italian",
        "verified": true,
        "avatar": "🇮🇹",
        "gradient": "linear-gradient(135deg, #16a34a 0%, #facc15 100%)",
        "visualDescription": "Italian woman with brown wavy hair in Rome.",
        "interests": [
          "#Italian",
          "#Rome",
          "#Pasta"
        ],
        "anthem": "🎵 'Volare' by Domenico Modugno",
        "film": "🎬 'La Dolce Vita' (Federico Fellini)",
        "pages": [
          {
            "tag": "🖼️ Rome Profile",
            "bio": "Ciao! I am Sofia from Rome, Italy. I have long dark wavy hair and brown eyes. I love colorful summer dresses and sunglasses.",
            "greenFlags": [
              "✓ Stylish Italian dress",
              "✓ Warm friendly personality",
              "✓ Expressive eyes"
            ],
            "redFlags": [
              "✗ Speaks with hands constantly",
              "✗ Always wearing dark sunglasses",
              "✗ Late for lunch"
            ]
          },
          {
            "tag": "⚡ Italian Daily Life",
            "bio": "In Rome, I drink espresso coffee at small cafes. I walk near historical fountains and talk with my family every evening.",
            "greenFlags": [
              "✓ Loves Italian coffee",
              "✓ Family person",
              "✓ Knows historic Rome"
            ],
            "redFlags": [
              "✗ Drinks 4 espressos daily",
              "✗ Talks loud on phone",
              "✗ Refuses instant coffee"
            ]
          },
          {
            "tag": "💭 Food & Traditions",
            "bio": "My grandmother teaches me how to cook fresh pasta. I love hot pizza with basil and cheese. Eating together is important!",
            "greenFlags": [
              "✓ Bakes fresh pizza",
              "✓ Family recipe traditions",
              "✓ Welcoming host"
            ],
            "redFlags": [
              "✗ Strict about pasta recipes",
              "✗ No pineapple on pizza",
              "✗ Long 3-hour dinners"
            ]
          }
        ],
        "opener": "Ciao! Do you like Italian pizza or sweet gelato?",
        "icebreakers": [
          "I love pizza and gelato!",
          "I want to visit Rome!",
          "Ciao Sofia!"
        ],
        "prompt": "Where is Sofia from? What is her nationality and native food?"
      }
    ],
    "food": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Food & Cuisine 🍜",
        "title": "Marco",
        "age": "23",
        "location": "📍 1 mile away • Little Italy",
        "verified": true,
        "avatar": "🍕",
        "gradient": "linear-gradient(135deg, #ef4444 0%, #f97316 100%)",
        "visualDescription": "Friendly Italian chef wearing a crisp white apron and holding a freshly baked Margherita pizza.",
        "interests": [
          "#FreshPizza",
          "#ItalianPasta",
          "#HomemadeBread"
        ],
        "anthem": "🎵 'Volare' by Domenico Modugno",
        "film": "🎬 'Ratatouille' (Brad Bird)",
        "pages": [
          {
            "tag": "🍕 Pizza & Pasta",
            "bio": "Hi! I am Marco. I love Italian food. I make fresh pizza and hot pasta every day. Tomato sauce and cheese are my favorite ingredients!",
            "greenFlags": [
              "✓ Makes fresh pizza",
              "✓ Loves homemade sauce",
              "✓ Friendly kitchen helper"
            ],
            "redFlags": [
              "✗ Puts cheese on everything",
              "✗ Eats pizza for breakfast",
              "✗ Dislikes pineapple on pizza"
            ]
          },
          {
            "tag": "🥖 Baking Routine",
            "bio": "Every morning I bake fresh bread. The kitchen smells wonderful. I drink hot espresso and eat bread with butter.",
            "greenFlags": [
              "✓ Fresh warm bread",
              "✓ Early morning baker",
              "✓ Loves good espresso"
            ],
            "redFlags": [
              "✗ Flour everywhere in the room",
              "✗ Bakes at 5 AM",
              "✗ Burns crust sometimes"
            ]
          },
          {
            "tag": "🍷 Dinner Time",
            "bio": "In the evening, I eat dinner with friends. We share big bowls of spaghetti. Cooking for people makes me very happy!",
            "greenFlags": [
              "✓ Enjoys big dinners",
              "✓ Shares good food",
              "✓ Generous cooking portions"
            ],
            "redFlags": [
              "✗ Talks loudly while eating",
              "✗ Takes long dinner breaks",
              "✗ Insists on Italian recipes"
            ]
          }
        ],
        "opener": "Hello! Do you like pizza or pasta for dinner?",
        "icebreakers": [
          "I love fresh pizza!",
          "Spaghetti is my favorite!",
          "I like homemade bread!"
        ],
        "prompt": "Describe Marco's favorite foods and cooking routine using simple words."
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Food & Cuisine 🍜",
        "title": "Kenji",
        "age": "25",
        "location": "📍 2 miles away • Asian Quarter",
        "verified": true,
        "avatar": "🍜",
        "gradient": "linear-gradient(135deg, #eab308 0%, #84cc16 100%)",
        "visualDescription": "Young man at a noodle stall holding a bowl of steaming ramen with green onions and eggs.",
        "interests": [
          "#RamenNoodles",
          "#GreenTea",
          "#StreetFood"
        ],
        "anthem": "🎵 'Sukiyaki' by Kyu Sakamoto",
        "film": "🎬 'Tampopo' (Juzo Itami)",
        "pages": [
          {
            "tag": "🍜 Noodle Bowls",
            "bio": "Hello! I am Kenji. I eat hot ramen noodles every week. I like warm soup, green onions, and fresh eggs in my bowl.",
            "greenFlags": [
              "✓ Delicious hot broth",
              "✓ Loves fresh noodles",
              "✓ Simple healthy eating"
            ],
            "redFlags": [
              "✗ Eats noodles very fast",
              "✗ Makes loud slurping sounds",
              "✗ Only drinks green tea"
            ]
          },
          {
            "tag": "🍵 Tea Time",
            "bio": "After lunch, I drink hot green tea. It is fresh and healthy. I do not add sugar or milk to my tea.",
            "greenFlags": [
              "✓ Healthy drink habits",
              "✓ Enjoys quiet tea moments",
              "✓ Simple natural tastes"
            ],
            "redFlags": [
              "✗ Dislikes sweet drinks",
              "✗ Drinks tea 6 times a day",
              "✗ Very strict about water temperature"
            ]
          },
          {
            "tag": "🍣 Sushi Snacks",
            "bio": "On weekends, I make simple rice rolls with cucumber and salmon. Fresh fish and rice make a great lunch snack!",
            "greenFlags": [
              "✓ Makes fresh sushi",
              "✓ Clean cooking style",
              "✓ Light healthy meals"
            ],
            "redFlags": [
              "✗ Uses too much spicy wasabi",
              "✗ Only buys fresh fish",
              "✗ Dislikes cooked meat"
            ]
          }
        ],
        "opener": "Hi! Do you prefer hot ramen soup or fresh sushi rolls?",
        "icebreakers": [
          "Hot ramen soup!",
          "Fresh sushi rolls!",
          "Green tea with no sugar!"
        ],
        "prompt": "Talk about Kenji's favorite Japanese dishes and tea habits."
      }
    ],
    "travel": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Travel & Destinations ✈️",
        "title": "Lucas",
        "age": "24",
        "location": "📍 2 miles away • Airport District",
        "verified": true,
        "avatar": "🏖️",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #38bdf8 100%)",
        "visualDescription": "Relaxed traveler wearing a sun hat and sunglasses on a bright sandy beach with turquoise blue ocean water.",
        "interests": [
          "#SunnyBeach",
          "#SummerVacation",
          "#SeaAndSun"
        ],
        "anthem": "🎵 'Island in the Sun' by Weezer",
        "film": "🎬 'Moana' (Clements & Musker)",
        "pages": [
          {
            "tag": "🏖️ Beach Holiday",
            "bio": "Hi! I am Lucas. I love warm sunny beaches! I swim in the blue sea and walk on white sand. Summer is my favorite time to travel.",
            "greenFlags": [
              "✓ Loves sunny weather",
              "✓ Great swimmer in the sea",
              "✓ Relaxed holiday mood"
            ],
            "redFlags": [
              "✗ Dislikes cold winter trips",
              "✗ Gets sunburnt very easily",
              "✗ Sleeps on the beach all day"
            ]
          },
          {
            "tag": "🧳 Packing Light",
            "bio": "I take one small bag when I travel. I pack t-shirts, shorts, and sunglasses. Traveling light makes moving easy and fast!",
            "greenFlags": [
              "✓ Travels light with one bag",
              "✓ Easy going traveler",
              "✓ Ready in 5 minutes"
            ],
            "redFlags": [
              "✗ Forgets his toothbrush",
              "✗ Wears sandals everywhere",
              "✗ Loses his sunglasses often"
            ]
          },
          {
            "tag": "🍦 Seaside Snacks",
            "bio": "On holiday, I eat sweet ice cream and drink cold coconut water. Sitting near the ocean waves makes me feel happy and calm.",
            "greenFlags": [
              "✓ Simple enjoyable habits",
              "✓ Loves ocean views",
              "✓ Friendly beach companion"
            ],
            "redFlags": [
              "✗ Eats ice cream every hour",
              "✗ Drops sand on towels",
              "✗ Refuses indoor museum visits"
            ]
          }
        ],
        "opener": "Hello! Do you like sunny beach holidays or cold mountain trips?",
        "icebreakers": [
          "Sunny beach holidays!",
          "Cold mountain trips!",
          "Swimming in the sea!"
        ],
        "prompt": "Describe Lucas's beach vacation habits using simple A0-A1 vocabulary."
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Travel & Destinations ✈️",
        "title": "Sophia",
        "age": "22",
        "location": "📍 1 mile away • Old Town",
        "verified": true,
        "avatar": "🏰",
        "gradient": "linear-gradient(135deg, #ec4899 0%, #f43f5e 100%)",
        "visualDescription": "Cheerful traveler holding a camera and map in front of an ancient castle on a cobblestone street.",
        "interests": [
          "#OldCastles",
          "#CityWalks",
          "#TakingPhotos"
        ],
        "anthem": "🎵 'Postcards' by James Blunt",
        "film": "🎬 'Roman Holiday' (William Wyler)",
        "pages": [
          {
            "tag": "🏰 Historic Cities",
            "bio": "Hello! I am Sophia. I visit old European cities. I like looking at tall castles, old bridges, and beautiful street lamps.",
            "greenFlags": [
              "✓ Loves history and cities",
              "✓ Takes beautiful photos",
              "✓ Walks everywhere on foot"
            ],
            "redFlags": [
              "✗ Stops to take 100 photos",
              "✗ Gets lost in small streets",
              "✗ Wears uncomfortable shoes"
            ]
          },
          {
            "tag": "📷 Photo Memories",
            "bio": "I carry a small camera everywhere. I take pictures of old buildings, colorful flowers, and happy people in the town square.",
            "greenFlags": [
              "✓ Creates nice photo albums",
              "✓ Notices pretty details",
              "✓ Enjoys city sightseeing"
            ],
            "redFlags": [
              "✗ Camera battery dies quickly",
              "✗ Asks strangers to take photos",
              "✗ Walks slowly while taking pictures"
            ]
          },
          {
            "tag": "🗺️ Map Explorer",
            "bio": "I read paper maps and walk through narrow streets. I like finding small local bakeries and buying fresh fruit juice.",
            "greenFlags": [
              "✓ Adventurous city explorer",
              "✓ Finds hidden bakeries",
              "✓ Positive travel attitude"
            ],
            "redFlags": [
              "✗ Refuses to use phone GPS",
              "✗ Reads maps upside down",
              "✗ Walks 20 kilometers a day"
            ]
          }
        ],
        "opener": "Hi! Do you like taking photos of castles or reading travel maps?",
        "icebreakers": [
          "Taking city photos!",
          "Exploring with a map!",
          "Visiting old castles!"
        ],
        "prompt": "Talk about Sophia's city tours and photography habits in basic English."
      }
    ],
    "entertainment": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Books, Films & Music 🎬",
        "title": "Maya",
        "age": "21",
        "location": "📍 1 mile away • Cinema District",
        "verified": true,
        "avatar": "🍿",
        "gradient": "linear-gradient(135deg, #dc2626 0%, #f87171 100%)",
        "visualDescription": "Happy movie fan holding a large bucket of popcorn inside a brightly lit movie theater.",
        "interests": [
          "#AnimatedFilms",
          "#PopcornSnacks",
          "#CinemaNights"
        ],
        "anthem": "🎵 'Can't Stop the Feeling!' by Justin Timberlake",
        "film": "🎬 'Toy Story' (John Lasseter)",
        "pages": [
          {
            "tag": "🍿 Movie Nights",
            "bio": "Hi! I am Maya. I love watching movies at the cinema! Animated films and funny comedies make me laugh. Popcorn is my favorite movie snack.",
            "greenFlags": [
              "✓ Loves movie theaters",
              "✓ Shares her big popcorn",
              "✓ Always happy and laughing"
            ],
            "redFlags": [
              "✗ Talks during the movie",
              "✗ Eats popcorn loudly",
              "✗ Prefers short cartoons"
            ]
          },
          {
            "tag": "🎶 Happy Music",
            "bio": "I listen to pop music on my headphones every day. I sing sweet songs while I clean my room or walk to school.",
            "greenFlags": [
              "✓ Positive musical taste",
              "✓ Sings with joy",
              "✓ Enjoys cheerful tunes"
            ],
            "redFlags": [
              "✗ Sings out of tune",
              "✗ Music playing too loud",
              "✗ Plays the same song 10 times"
            ]
          },
          {
            "tag": "📚 Picture Books",
            "bio": "In the evening, I read colorful comic books and graphic novels. The big pictures help me learn new stories easily.",
            "greenFlags": [
              "✓ Loves visual stories",
              "✓ Fun book collection",
              "✓ Relaxing evening habit"
            ],
            "redFlags": [
              "✗ Only reads comic books",
              "✗ Leaves books on the floor",
              "✗ Finishes books in 10 minutes"
            ]
          }
        ],
        "opener": "Hello! Do you like watching animated movies or reading comic books?",
        "icebreakers": [
          "Animated movies at the cinema!",
          "Reading comic books!",
          "Listening to cheerful pop songs!"
        ],
        "prompt": "Describe Maya's entertainment habits, favorite movie snacks, and musical taste."
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Books, Films & Music 🎬",
        "title": "Leo",
        "age": "23",
        "location": "📍 2 miles away • Music Hall",
        "verified": true,
        "avatar": "🎸",
        "gradient": "linear-gradient(135deg, #2563eb 0%, #60a5fa 100%)",
        "visualDescription": "Young musician smiling while playing an acoustic guitar on a wooden bench.",
        "interests": [
          "#AcousticGuitar",
          "#RockMusic",
          "#LiveConcerts"
        ],
        "anthem": "🎵 'Three Little Birds' by Bob Marley",
        "film": "🎬 'School of Rock' (Richard Linklater)",
        "pages": [
          {
            "tag": "🎸 Acoustic Guitar",
            "bio": "Hello! I am Leo. I play the acoustic guitar. I know many simple songs. Playing music for my friends makes me very happy!",
            "greenFlags": [
              "✓ Plays nice guitar music",
              "✓ Friendly song leader",
              "✓ Enjoys sharing tunes"
            ],
            "redFlags": [
              "✗ Practises guitar late at night",
              "✗ Plays only 3 chords",
              "✗ Sings loudly in public"
            ]
          },
          {
            "tag": "📻 Radio Hits",
            "bio": "I turn on the radio every morning. I like listening to old rock music and catchy pop radio hits while drinking tea.",
            "greenFlags": [
              "✓ Enjoys classic rock music",
              "✓ Upbeat radio listener",
              "✓ Good music energy"
            ],
            "redFlags": [
              "✗ Dislikes modern hip-hop",
              "✗ Changes radio channels often",
              "✗ Hums radio songs loudly"
            ]
          },
          {
            "tag": "🎬 Action Movies",
            "bio": "On Sunday afternoons, I watch fun action movies on TV. Fast cars, superheroes, and big explosions are exciting!",
            "greenFlags": [
              "✓ Easy entertainment choice",
              "✓ Fun movie partner",
              "✓ Enjoys superhero films"
            ],
            "redFlags": [
              "✗ Gets scared by dark scenes",
              "✗ Spoilers the movie end",
              "✗ Watches TV for 4 hours"
            ]
          }
        ],
        "opener": "Hi! Do you play a musical instrument or listen to the radio?",
        "icebreakers": [
          "I play guitar!",
          "I listen to radio hits!",
          "I love action movies!"
        ],
        "prompt": "Talk about Leo's guitar playing, musical preferences, and Sunday movie choices."
      }
    ],
    "daily_habits": [
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Daily Routines & Habits 🌅",
        "title": "Emma",
        "age": "22",
        "location": "📍 1 mile away • Garden District",
        "verified": true,
        "avatar": "🌅",
        "gradient": "linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%)",
        "visualDescription": "Bright early riser smiling with a cup of morning tea beside an open sunlit window.",
        "interests": [
          "#EarlyRiser",
          "#MorningTea",
          "#DailyWalks"
        ],
        "anthem": "🎵 'Here Comes the Sun' by The Beatles",
        "film": "🎬 'Amélie' (Jean-Pierre Jeunet)",
        "pages": [
          {
            "tag": "🌅 Early Mornings",
            "bio": "Hi! I am Emma. I wake up early at 6 AM every day. I drink hot green tea and open my windows to let in fresh morning air.",
            "greenFlags": [
              "✓ Early morning energy",
              "✓ Clean fresh room",
              "✓ Positive start to the day"
            ],
            "redFlags": [
              "✗ Goes to sleep at 9 PM",
              "✗ Makes noise early in the morning",
              "✗ Dislikes sleeping in"
            ]
          },
          {
            "tag": "🚶‍♀️ Daily Park Walk",
            "bio": "After breakfast, I walk in the green park for 30 minutes. I listen to birds sing and say hello to neighbor dogs.",
            "greenFlags": [
              "✓ Daily walking habit",
              "✓ Friendly to neighbors",
              "✓ Enjoys green nature"
            ],
            "redFlags": [
              "✗ Walks even in heavy rain",
              "✗ Stops to pet every dog",
              "✗ Dislikes riding buses"
            ]
          },
          {
            "tag": "📖 Bedtime Reading",
            "bio": "Before I go to sleep, I read a simple book for 15 minutes. I turn off my phone and sleep very peacefully.",
            "greenFlags": [
              "✓ Healthy sleep habits",
              "✓ No phone before bed",
              "✓ Calm quiet evening"
            ],
            "redFlags": [
              "✗ Falls asleep while reading",
              "✗ Turns off lights early",
              "✗ Dislikes late night parties"
            ]
          }
        ],
        "opener": "Hello! Are you an early morning bird or a late night owl?",
        "icebreakers": [
          "Early morning bird!",
          "Late night owl!",
          "Daily park walks!"
        ],
        "prompt": "Describe Emma's morning routine, walking habits, and early sleep schedule."
      },
      {
        "type": "profile",
        "level": "A0-A1",
        "category": "Daily Routines & Habits 🌅",
        "title": "Noah",
        "age": "24",
        "location": "📍 2 miles away • Downtown Loft",
        "verified": true,
        "avatar": "🌙",
        "gradient": "linear-gradient(135deg, #312e81 0%, #6366f1 100%)",
        "visualDescription": "Night owl working on a laptop with soft desk lamp lighting and music headphones.",
        "interests": [
          "#NightOwl",
          "#LateNightSnacks",
          "#EveningGym"
        ],
        "anthem": "🎵 'Blinding Lights' by The Weeknd",
        "film": "🎬 'Night at the Museum' (Shawn Levy)",
        "pages": [
          {
            "tag": "🌙 Night Owl",
            "bio": "Hello! I am Noah. I like the night! I stay awake until 1 AM reading or playing games. Quiet nights are peaceful for me.",
            "greenFlags": [
              "✓ Great night company",
              "✓ Calm quiet energy",
              "✓ Creative late hours"
            ],
            "redFlags": [
              "✗ Sleeps until 10 AM",
              "✗ Misses early breakfasts",
              "✗ Hard to wake up"
            ]
          },
          {
            "tag": "🏋️‍♂️ Evening Gym",
            "bio": "I go to the gym at 8 PM after dinner. Lifting weights and running on the treadmill keeps me strong and healthy.",
            "greenFlags": [
              "✓ Regular fitness habit",
              "✓ Gym active routines",
              "✓ High evening strength"
            ],
            "redFlags": [
              "✗ Gym clothes in laundry basket",
              "✗ Drinks energy shakes late",
              "✗ Eats dinner at midnight"
            ]
          },
          {
            "tag": "🍎 Daily Planning",
            "bio": "Every evening, I write my simple to-do list for tomorrow in a notebook. Organizing my day helps me stay calm.",
            "greenFlags": [
              "✓ Organized daily planner",
              "✓ Keeps things orderly",
              "✓ Calm organized mind"
            ],
            "redFlags": [
              "✗ Over-plans simple tasks",
              "✗ Upset if plans change",
              "✗ Keeps too many sticky notes"
            ]
          }
        ],
        "opener": "Hi! Do you prefer going to the gym in the morning or in the evening?",
        "icebreakers": [
          "Evening gym workout!",
          "Morning exercise!",
          "Late night peaceful gaming!"
        ],
        "prompt": "Talk about Noah's night owl habits, evening gym workout, and daily notebook planning."
      }
    ]
  },
  "A2": {
    "appearance": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Physical Appearance 👁️",
        "title": "Elena",
        "age": "24",
        "location": "📍 2 miles away • Marina Bay",
        "verified": true,
        "avatar": "👩‍🦰",
        "gradient": "linear-gradient(135deg, #ea580c 0%, #fb923c 100%)",
        "visualDescription": "Athletic woman with shoulder-length wavy auburn hair, hazel eyes, light freckles, wearing a navy blue striped shirt.",
        "interests": [
          "#WavyHair",
          "#HazelEyes",
          "#CasualLook"
        ],
        "anthem": "🎵 'Levitating' by Dua Lipa",
        "film": "🎬 'La La Land' (Damien Chazelle)",
        "pages": [
          {
            "tag": "🖼️ Physical Look",
            "bio": "I am Elena, 24 years old. I am medium height and active because I exercise every day. I have wavy auburn hair and hazel eyes with light freckles. I like wearing simple striped shirts and jeans.",
            "greenFlags": [
              "✓ Friendly hazel eyes",
              "✓ Cute natural freckles",
              "✓ Likes staying active"
            ],
            "redFlags": [
              "✗ Gets sunburned easily",
              "✗ Hair gets messy in the wind",
              "✗ Wears sandals in cold weather"
            ]
          },
          {
            "tag": "⚡ Outdoor Routine",
            "bio": "Every morning I go jogging along the river path. I drink fresh orange juice and eat oats. Staying moving helps me feel happy and productive all afternoon.",
            "greenFlags": [
              "✓ Regular morning runner",
              "✓ Healthy breakfast habits",
              "✓ High afternoon energy"
            ],
            "redFlags": [
              "✗ Wakes up at 5:30 AM",
              "✗ Forces friends to jog",
              "✗ Talks about fitness stats"
            ]
          },
          {
            "tag": "💭 Style & Feelings",
            "bio": "I prefer natural looks over heavy makeup. Comfortable clothes make me feel confident. I love bright sunny days when I can spend hours outside!",
            "greenFlags": [
              "✓ Natural confidence",
              "✓ Loves bright weather",
              "✓ Practical clothing choices"
            ],
            "redFlags": [
              "✗ Avoids formal indoor events",
              "✗ Dislikes high heels",
              "✗ Complains when stuck inside"
            ]
          }
        ],
        "opener": "Hi! What is your favorite season for casual clothes?",
        "icebreakers": [
          "Summer clothes!",
          "Warm autumn coats!",
          "Casual jeans and t-shirts!"
        ],
        "prompt": "Describe Elena's hair, eyes, and clothing style using simple A2 sentences."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Physical Appearance 👁️",
        "title": "Liam",
        "age": "27",
        "location": "📍 1.5 miles away • Financial District",
        "verified": true,
        "avatar": "👨‍💼",
        "gradient": "linear-gradient(135deg, #1e293b 0%, #475569 100%)",
        "visualDescription": "Tall 6'2\" man with neat dark blonde side-part hair and sharp jawline, wearing a tailored navy blazer.",
        "interests": [
          "#TallAndSlim",
          "#SmartBlazer",
          "#NeatLook"
        ],
        "anthem": "🎵 'Smooth Operator' by Sade",
        "film": "🎬 'The Devil Wears Prada' (David Frankel)",
        "pages": [
          {
            "tag": "🖼️ Tailored Style",
            "bio": "I am Liam, 27 years old. I am tall with short blonde hair and blue eyes. I usually wear well-fitted jackets, dark trousers, and clean leather shoes.",
            "greenFlags": [
              "✓ Polished and neat look",
              "✓ Good posture",
              "✓ Tidy hairstyle"
            ],
            "redFlags": [
              "✗ Picky about shirt wrinkles",
              "✗ Spends money on dry cleaning",
              "✗ Dislikes cozy hoodies"
            ]
          },
          {
            "tag": "⚡ Office & Evenings",
            "bio": "I work in a city bank during the week. In the evening, I enjoy visiting quiet coffee shops with a good newspaper or meeting colleagues for dinner.",
            "greenFlags": [
              "✓ Responsible career",
              "✓ Enjoys quiet cafes",
              "✓ Courteous dinner guest"
            ],
            "redFlags": [
              "✗ Talks about finances often",
              "✗ Reads news at dinner table",
              "✗ Checks work emails late"
            ]
          }
        ],
        "opener": "Hello! Do you prefer smart clothes for work or cozy hoodies?",
        "icebreakers": [
          "Cozy hoodies all day!",
          "I like smart clothes!",
          "A mix of both!"
        ],
        "prompt": "How tall is Liam? What does he wear for his job?"
      }
    ],
    "professions": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Professions 💼",
        "title": "Nina",
        "age": "26",
        "location": "📍 1 mile away • Creative Loft",
        "verified": true,
        "avatar": "👩‍🎨",
        "gradient": "linear-gradient(135deg, #ec4899 0%, #f472b6 100%)",
        "visualDescription": "Creative designer sitting with a drawing tablet, stylus pen, and colorful art posters.",
        "interests": [
          "#GraphicDesign",
          "#Drawing",
          "#Posters"
        ],
        "anthem": "🎵 'Colors' by Halsey",
        "film": "🎬 'Amélie' (Jean-Pierre Jeunet)",
        "pages": [
          {
            "tag": "🖼️ Designer Look",
            "bio": "I am Nina, a graphic designer. I make colorful logos, posters, and websites on my computer. I have dark hair, round glasses, and a creative style.",
            "greenFlags": [
              "✓ Creative and fun work",
              "✓ Can work from home or cafes",
              "✓ Sees her art in the city"
            ],
            "redFlags": [
              "✗ Sits near screens for 8 hours",
              "✗ Clients change their minds",
              "✗ Tired eyes after work"
            ]
          },
          {
            "tag": "⚡ Creative Process",
            "bio": "I start my workday with a notebook and sketchpens. I try different color schemes and fonts until the design looks balanced and clear.",
            "greenFlags": [
              "✓ Thoughtful design planning",
              "✓ Attention to detail",
              "✓ Great eye for colors"
            ],
            "redFlags": [
              "✗ Paper sketches everywhere",
              "✗ Changes font 20 times",
              "✗ Obsesses over color shades"
            ]
          },
          {
            "tag": "💭 Artistic Views",
            "bio": "Design should be simple, colorful, and helpful for people. I love art galleries, clean typography, and modern city architecture.",
            "greenFlags": [
              "✓ Enjoys museum visits",
              "✓ Appreciates clean design",
              "✓ Inspiring creative ideas"
            ],
            "redFlags": [
              "✗ Judges ugly billboards",
              "✗ Criticizes website fonts",
              "✗ Too picky about poster art"
            ]
          }
        ],
        "opener": "Hey! What color combination do you like for a website?",
        "icebreakers": [
          "Blue and white!",
          "Black and yellow!",
          "Green and brown!"
        ],
        "prompt": "What tools does Nina use for her work? Why does she like her job?"
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Professions 💼",
        "title": "Sam",
        "age": "25",
        "location": "📍 0.4 miles away • Corner Café",
        "verified": true,
        "avatar": "☕",
        "gradient": "linear-gradient(135deg, #78350f 0%, #b45309 100%)",
        "visualDescription": "Smiling barista pouring artful milk foam into a hot cappuccino at a wooden coffee bar.",
        "interests": [
          "#Barista",
          "#CoffeeArt",
          "#Espresso"
        ],
        "anthem": "🎵 'Coffee & TV' by Blur",
        "film": "🎬 'Amélie' (Jean-Pierre Jeunet)",
        "pages": [
          {
            "tag": "🖼️ Barista Profile",
            "bio": "I am Sam, a coffee barista at a popular neighborhood café. I prepare espresso drinks, roast coffee beans, and draw heart shapes in milk foam.",
            "greenFlags": [
              "✓ Makes perfect cappuccinos",
              "✓ Welcoming coffee spot",
              "✓ Great morning chats"
            ],
            "redFlags": [
              "✗ Smells like coffee beans constantly",
              "✗ Wakes up at 5:00 AM",
              "✗ Critical of instant coffee"
            ]
          },
          {
            "tag": "⚡ Morning Service",
            "bio": "My shift starts early. I greet regular customers by name and make their favorite drinks quickly before they travel to work.",
            "greenFlags": [
              "✓ Remembers drink orders",
              "✓ Friendly community service",
              "✓ Quick and efficient"
            ],
            "redFlags": [
              "✗ Loud espresso machine",
              "✗ Long morning queues",
              "✗ Works on weekend mornings"
            ]
          }
        ],
        "opener": "Hi! Do you drink espresso coffee or herbal tea in the morning?",
        "icebreakers": [
          "Cappuccino with oat milk!",
          "Herbal tea for me!",
          "Coffee art looks cool!"
        ],
        "prompt": "What does Sam do as a barista in the morning?"
      }
    ],
    "properties": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Properties 🏠",
        "title": "Lucas",
        "age": "28",
        "location": "📍 1.2 miles away • Arts District",
        "verified": true,
        "avatar": "🏢",
        "gradient": "linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)",
        "visualDescription": "Owner of an industrial loft with high concrete ceilings, large glass windows, and kitchen island.",
        "interests": [
          "#HighCeilings",
          "#LoftApartment",
          "#BigWindows"
        ],
        "anthem": "🎵 'Midnight City' by M83",
        "film": "🎬 'Inception' (Christopher Nolan)",
        "pages": [
          {
            "tag": "🖼️ Loft Spaces",
            "bio": "I am Lucas! My flat is a bright one-bedroom loft apartment. It has high ceilings, big industrial windows, and modern white furniture.",
            "greenFlags": [
              "✓ Nice city views",
              "✓ Lots of sunlight",
              "✓ Modern kitchen"
            ],
            "redFlags": [
              "✗ Higher heating bills in winter",
              "✗ Street noise at night",
              "✗ No private garage"
            ]
          },
          {
            "tag": "⚡ Living Environment",
            "bio": "I enjoy cooking dinner in the open kitchen while listening to jazz music. Sunlight fills the main room all morning, making it great for plants.",
            "greenFlags": [
              "✓ Great for indoor plants",
              "✓ Open kitchen layout",
              "✓ Relaxing jazz atmosphere"
            ],
            "redFlags": [
              "✗ Echoes when people talk",
              "✗ Kitchen smells fill whole loft",
              "✗ Needs frequent dusting"
            ]
          }
        ],
        "opener": "Hi! Do you like big loft apartments with city views?",
        "icebreakers": [
          "I love loft apartments!",
          "Too noisy for me!",
          "Big windows are great!"
        ],
        "prompt": "Describe 3 things inside Lucas's apartment."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Properties 🏠",
        "title": "Grace",
        "age": "29",
        "location": "📍 3 miles away • Oak Suburbs",
        "verified": true,
        "avatar": "🏡",
        "gradient": "linear-gradient(135deg, #15803d 0%, #22c55e 100%)",
        "visualDescription": "Two-story suburban family house with a green lawn, wooden porch, and garage.",
        "interests": [
          "#SuburbanHouse",
          "#GreenLawn",
          "#PrivateGarage"
        ],
        "anthem": "🎵 'Our House' by Madness",
        "film": "🎬 'Father of the Bride' (Charles Shyer)",
        "pages": [
          {
            "tag": "🖼️ House Overview",
            "bio": "Hi! I am Grace. I live in a comfortable two-story house with three bedrooms, a large green lawn, and a private driveway.",
            "greenFlags": [
              "✓ Spacious three bedrooms",
              "✓ Private green garden",
              "✓ Quiet suburban street"
            ],
            "redFlags": [
              "✗ Long commute to city center",
              "✗ Grass mowing every week",
              "✗ Requires a car for shopping"
            ]
          },
          {
            "tag": "⚡ Living Environment",
            "bio": "I enjoy hosting weekend barbecues on the back porch. The neighborhood is safe and peaceful with wide streets and tall trees.",
            "greenFlags": [
              "✓ Great back porch for BBQs",
              "✓ Safe and quiet neighborhood",
              "✓ Plenty of parking"
            ],
            "redFlags": [
              "✗ No metro station nearby",
              "✗ Quiet evenings with few shops",
              "✗ Suburban driving required"
            ]
          }
        ],
        "opener": "Hello! Would you prefer living in a quiet suburban house or a city flat?",
        "icebreakers": [
          "Suburban house with a lawn!",
          "City flat near the metro!",
          "Barbecues on the porch!"
        ],
        "prompt": "List 3 advantages of living in Grace's suburban house."
      }
    ],
    "character": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Character Traits 🧠",
        "title": "Sophia",
        "age": "26",
        "location": "📍 2 miles away • West End",
        "verified": true,
        "avatar": "📅",
        "gradient": "linear-gradient(135deg, #2563eb 0%, #60a5fa 100%)",
        "visualDescription": "Focused young woman checking off tasks in a paper planner at her clean desk.",
        "interests": [
          "#Punctual",
          "#DailyPlanner",
          "#Reliable"
        ],
        "anthem": "🎵 'Unstoppable' by Sia",
        "film": "🎬 'The Social Network' (David Fincher)",
        "pages": [
          {
            "tag": "🖼️ Organized Profile",
            "bio": "I am Sophia. I am organized and always arrive on time because I plan my days with a digital calendar. I like clean spaces and clear rules.",
            "greenFlags": [
              "✓ Never late for meetings",
              "✓ Helps friends when needed",
              "✓ Always organized"
            ],
            "redFlags": [
              "✗ Gets nervous if plans change",
              "✗ Very strict schedule",
              "✗ Dislikes messy rooms"
            ]
          },
          {
            "tag": "⚡ Daily Habits",
            "bio": "Every Sunday I make a plan for my week. I organize my tasks, prepare healthy meals, and list all my appointments in color-coded files.",
            "greenFlags": [
              "✓ Prepares meals in advance",
              "✓ Highly reliable person",
              "✓ Systematic problem solver"
            ],
            "redFlags": [
              "✗ Color-codes everything",
              "✗ Panics if missing a pen",
              "✗ Uncomfortable with surprises"
            ]
          },
          {
            "tag": "💭 Mindset & Life",
            "bio": "Order brings peace to life. When everything is organized, I feel calm and ready to help my family and friends with their goals.",
            "greenFlags": [
              "✓ Calm under structure",
              "✓ Supportive friend",
              "✓ Dependable family member"
            ],
            "redFlags": [
              "✗ Re-arranges other people's desks",
              "✗ Expects total order from others",
              "✗ Cannot relax in messy places"
            ]
          }
        ],
        "opener": "Hi! Do you like to plan your week ahead or make quick decisions?",
        "icebreakers": [
          "I plan everything!",
          "I make quick decisions!",
          "A little of both!"
        ],
        "prompt": "Why is Sophia a reliable friend? Mention two habits she has."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Character Traits 🧠",
        "title": "Alex",
        "age": "24",
        "location": "📍 1 mile away • Central Square",
        "verified": true,
        "avatar": "🎉",
        "gradient": "linear-gradient(135deg, #ec4899 0%, #f43f5e 100%)",
        "visualDescription": "Energetic friend laughing while inviting people to a spontaneous weekend road trip.",
        "interests": [
          "#Spontaneous",
          "#FunLover",
          "#Energetic"
        ],
        "anthem": "🎵 'Best Day of My Life' by American Authors",
        "film": "🎬 'Ferris Bueller's Day Off' (John Hughes)",
        "pages": [
          {
            "tag": "🖼️ Spontaneous Energy",
            "bio": "I am Alex! I am cheerful, energetic, and spontaneous. I love planning surprise weekend trips, trying new street food, and making people laugh.",
            "greenFlags": [
              "✓ High cheerful energy",
              "✓ Fun adventure organizer",
              "✓ Great sense of humor"
            ],
            "redFlags": [
              "✗ Changes plans last minute",
              "✗ Forgetful with dates",
              "✗ Rarely stays still"
            ]
          },
          {
            "tag": "⚡ Weekend Adventures",
            "bio": "I often call my friends on Friday evening to suggest driving to the beach or visiting a food festival. Life is full of exciting moments!",
            "greenFlags": [
              "✓ Inspires fun experiences",
              "✓ Always positive attitude",
              "✓ Brings friends together"
            ],
            "redFlags": [
              "✗ Unpredictable weekend schedule",
              "✗ Calls late at night",
              "✗ Dislikes strict routines"
            ]
          }
        ],
        "opener": "Hey! Do you like unexpected weekend trips or planned schedules?",
        "icebreakers": [
          "Spontaneous weekend trips!",
          "Planned schedules for me!",
          "Funny road trips!"
        ],
        "prompt": "Why is Alex a fun friend to hang out with? Give two examples."
      }
    ],
    "hobbies": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Hobbies 🎨",
        "title": "Hannah",
        "age": "25",
        "location": "📍 Kitchen Bench",
        "verified": true,
        "avatar": "🍞",
        "gradient": "linear-gradient(135deg, #b45309 0%, #f59e0b 100%)",
        "visualDescription": "Golden loaf of sourdough bread on a wooden cutting board with a knife.",
        "interests": [
          "#BakingBread",
          "#WarmOven",
          "#FreshFood"
        ],
        "anthem": "🎵 'Banana Pancakes' by Jack Johnson",
        "film": "🎬 'Julie & Julia' (Nora Ephron)",
        "pages": [
          {
            "tag": "🖼️ Baking Passion",
            "bio": "I am Hannah and I love baking bread at home. I mix simple flour, water, and yeast, then bake it in a hot oven until the crust is golden.",
            "greenFlags": [
              "✓ Fresh warm bread",
              "✓ Relaxing kitchen hobby",
              "✓ Simple ingredients"
            ],
            "redFlags": [
              "✗ Needs hours of waiting time",
              "✗ Flour on kitchen counter",
              "✗ Oven gets very hot"
            ]
          },
          {
            "tag": "⚡ Kitchen Routine",
            "bio": "Baking requires patience. I knead the dough by hand for ten minutes and let it rise twice. My kitchen smells wonderful when bread is baking.",
            "greenFlags": [
              "✓ Wonderful bread smell",
              "✓ Patient baking routine",
              "✓ Delicious homemade crust"
            ],
            "redFlags": [
              "✗ Wakes up early to knead dough",
              "✗ Flour dust on dark clothes",
              "✗ Eats too much warm bread"
            ]
          }
        ],
        "opener": "Fresh bread from the oven! Do you prefer butter or olive oil?",
        "icebreakers": [
          "Butter please!",
          "Olive oil for me!",
          "Garlic butter!"
        ],
        "prompt": "What simple ingredients does Hannah use to bake bread?"
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Hobbies 🎨",
        "title": "Tom",
        "age": "26",
        "location": "📍 1.5 miles away • Arts Quarter",
        "verified": true,
        "avatar": "📷",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #6366f1 100%)",
        "visualDescription": "Young photographer looking through a digital camera lens on a city bridge.",
        "interests": [
          "#Photography",
          "#CityShots",
          "#DigitalCamera"
        ],
        "anthem": "🎵 'Photograph' by Ed Sheeran",
        "film": "🎬 'The Secret Life of Walter Mitty' (Ben Stiller)",
        "pages": [
          {
            "tag": "🖼️ Camera Hobby",
            "bio": "I am Tom and my hobby is urban photography. I walk through city streets with my camera to capture interesting buildings, reflections, and sunset lights.",
            "greenFlags": [
              "✓ Great creative eye",
              "✓ Shares beautiful city pictures",
              "✓ Patient observer"
            ],
            "redFlags": [
              "✗ Stops every 10 meters for photos",
              "✗ Heavy camera bag carrying",
              "✗ Obsesses over camera lenses"
            ]
          },
          {
            "tag": "⚡ Street Photography",
            "bio": "During golden hour in the evening, I take photos of historic bridges and busy street corners. Photography helps me appreciate daily city details.",
            "greenFlags": [
              "✓ Captures golden hour light",
              "✓ Notices architectural details",
              "✓ Inspiring artistic hobby"
            ],
            "redFlags": [
              "✗ Camera battery dies during walks",
              "✗ Asks friends to pose repeatedly",
              "✗ Thousands of unedited photos"
            ]
          }
        ],
        "opener": "Hi! Do you take photos with a digital camera or your smartphone?",
        "icebreakers": [
          "Digital camera photos!",
          "Smartphone is easier!",
          "Golden hour pictures!"
        ],
        "prompt": "What does Tom capture with his camera during evening walks?"
      }
    ],
    "nationalities": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Nationalities & Cultures 🌍",
        "title": "Mateo",
        "age": "26",
        "location": "📍 Oaxaca, Mexico • 🇲🇽 Spanish",
        "verified": true,
        "avatar": "🇲🇽",
        "gradient": "linear-gradient(135deg, #059669 0%, #dc2626 100%)",
        "visualDescription": "Mexican man in an embroidered shirt holding corn tacos in a town square.",
        "interests": [
          "#MexicanFood",
          "#Oaxaca",
          "#Tacos"
        ],
        "anthem": "🎵 'Cielito Lindo' by Mariachi Vargas",
        "film": "🎬 'Coco' (Pixar)",
        "pages": [
          {
            "tag": "🖼️ Oaxaca Roots",
            "bio": "Hola! I am Mateo from Oaxaca, Mexico. I am proud of my country and traditions. We celebrate festivals with colorful flowers, music, and fresh corn tacos.",
            "greenFlags": [
              "✓ Makes homemade guacamole",
              "✓ Friendly and welcoming",
              "✓ Teaches salsa steps"
            ],
            "redFlags": [
              "✗ Says spicy food is mild",
              "✗ Arrives late to events",
              "✗ Plays loud music"
            ]
          },
          {
            "tag": "⚡ Daily Culture",
            "bio": "I work in a local craft market. During breaks, I chat with neighbors and drink traditional chocolate drinks made with cinnamon and hot milk.",
            "greenFlags": [
              "✓ Loves local craft markets",
              "✓ Shares traditional drinks",
              "✓ Friendly community ties"
            ],
            "redFlags": [
              "✗ Spends hours chatting",
              "✗ Takes very long coffee breaks",
              "✗ Over-spices lunch dishes"
            ]
          },
          {
            "tag": "💭 Family & Joy",
            "bio": "Family gatherings are the center of life in Oaxaca. We play guitars, share home-cooked meals, and celebrate life with lots of laughter and dancing.",
            "greenFlags": [
              "✓ Great family host",
              "✓ Plays acoustic guitar",
              "✓ Joyful social energy"
            ],
            "redFlags": [
              "✗ Parties go past midnight",
              "✗ Invites 20 cousins over",
              "✗ Plays guitar during nap time"
            ]
          }
        ],
        "opener": "Hola! Do you like spicy food or traditional festivals?",
        "icebreakers": [
          "I love spicy food!",
          "Festivals look fun!",
          "Teach me salsa!"
        ],
        "prompt": "Where is Mateo from? What food does he make at home?"
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Nationalities & Cultures 🌍",
        "title": "Freja",
        "age": "25",
        "location": "📍 Copenhagen, Denmark • 🇩🇰 Danish",
        "verified": true,
        "avatar": "🇩🇰",
        "gradient": "linear-gradient(135deg, #2563eb 0%, #38bdf8 100%)",
        "visualDescription": "Danish woman riding a bicycle along Copenhagen harbor in a sweater.",
        "interests": [
          "#DanishStyle",
          "#Copenhagen",
          "#Bicycles"
        ],
        "anthem": "🎵 '7 Years' by Lukas Graham",
        "film": "🎬 'Another Round' (Thomas Vinterberg)",
        "pages": [
          {
            "tag": "🖼️ Danish Profile",
            "bio": "Hej! My name is Freja. I am Danish and I live in Copenhagen, Denmark. In Denmark, we like cozy indoor spaces with candles, soft blankets, and warm drinks.",
            "greenFlags": [
              "✓ Cozy candlelit dinners",
              "✓ Rides bike every day",
              "✓ Friendly and direct"
            ],
            "redFlags": [
              "✗ Dislikes taking short car rides",
              "✗ Lights many candles at home",
              "✗ Very strict about rye bread"
            ]
          },
          {
            "tag": "⚡ Copenhagen Life",
            "bio": "I ride my bicycle to my university every morning, even when rain or cold wind arrives. We wear warm waterproof coats and ride everywhere together.",
            "greenFlags": [
              "✓ Strong weather resilience",
              "✓ Eco-friendly lifestyle",
              "✓ Active commuting habit"
            ],
            "redFlags": [
              "✗ Refuses taxi rides",
              "✗ Clothes always smell like rain",
              "✗ Insists on cycling in snow"
            ]
          }
        ],
        "opener": "Hej! Do you like riding bicycles in the city or taking café breaks?",
        "icebreakers": [
          "I love riding bikes!",
          "Café breaks for me!",
          "Copenhagen sounds nice!"
        ],
        "prompt": "What country is Freja from? How does she travel around her city?"
      }
    ],
    "food": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Food & Cuisine 🍜",
        "title": "Chloe",
        "age": "26",
        "location": "📍 1.5 miles away • French Quarter",
        "verified": true,
        "avatar": "🥐",
        "gradient": "linear-gradient(135deg, #f43f5e 0%, #fb923c 100%)",
        "visualDescription": "Charming pastry enthusiast holding a plate of golden croissants and sweet macaroons outside a bakery.",
        "interests": [
          "#FrenchPastries",
          "#Croissants",
          "#CaféCulture"
        ],
        "anthem": "🎵 'La Vie en Rose' by Édith Piaf",
        "film": "🎬 'Chocolat' (Lasse Hallström)",
        "pages": [
          {
            "tag": "🥐 Pastry Lover",
            "bio": "Hi, I'm Chloe! I work near a French bakery. I buy buttery croissants and fruit tarts every weekend. Baking sweet desserts is my passion.",
            "greenFlags": [
              "✓ Bakes delicious pastries",
              "✓ Know best local bakeries",
              "✓ Loves sweet fruit tarts"
            ],
            "redFlags": [
              "✗ Eats pastry for every meal",
              "✗ Spends too much money on cakes",
              "✗ Refuses sugar-free desserts"
            ]
          },
          {
            "tag": "☕ Café Morning",
            "bio": "In the morning, I sit at small outdoor cafés with a cappuccino and a warm croissant. I enjoy watching people walk by while eating.",
            "greenFlags": [
              "✓ Relaxing morning routine",
              "✓ Appreciates café culture",
              "✓ Great café recommendations"
            ],
            "redFlags": [
              "✗ Sits in cafés for hours",
              "✗ Gets upset if coffee is cold",
              "✗ Dislikes fast food breakfasts"
            ]
          },
          {
            "tag": "🍫 Sweet Treats",
            "bio": "I love dark chocolate and raspberry macarons. Cooking desserts for my friends brings everyone together on Sunday afternoons.",
            "greenFlags": [
              "✓ Generous dessert baker",
              "✓ Creative chocolate recipes",
              "✓ Host fun Sunday tea parties"
            ],
            "redFlags": [
              "✗ Always offers extra sugar",
              "✗ Gets flour on the furniture",
              "✗ Only eats dark chocolate"
            ]
          }
        ],
        "opener": "Bonjour! Do you prefer sweet pastries or savory breakfasts in the morning?",
        "icebreakers": [
          "Sweet pastries and coffee!",
          "Savory eggs and bacon!",
          "Warm French croissants!"
        ],
        "prompt": "Describe Chloe's bakery visits and morning café preferences in simple A2 sentences."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Food & Cuisine 🍜",
        "title": "Carlos",
        "age": "27",
        "location": "📍 3 miles away • Mission District",
        "verified": true,
        "avatar": "🌮",
        "gradient": "linear-gradient(135deg, #16a34a 0%, #eab308 100%)",
        "visualDescription": "Enthusiastic home cook squeezing fresh lime over spicy street tacos at an outdoor grill.",
        "interests": [
          "#StreetTacos",
          "#SpicySalsa",
          "#GrillMaster"
        ],
        "anthem": "🎵 'Oye Como Va' by Santana",
        "film": "🎬 'Chef' (Jon Favreau)",
        "pages": [
          {
            "tag": "🌮 Taco Saturday",
            "bio": "I'm Carlos, 27. Every Saturday I host street taco nights. I grill marinated beef and chicken with fresh cilantro, onions, and lime juice.",
            "greenFlags": [
              "✓ Fantastic taco recipes",
              "✓ Fresh homemade salsa",
              "✓ Welcoming barbecue host"
            ],
            "redFlags": [
              "✗ Salsa is extremely spicy",
              "✗ Uses endless paper towels",
              "✗ Plays loud music while grilling"
            ]
          },
          {
            "tag": "🥑 Fresh Guacamole",
            "bio": "I make fresh guacamole with ripe avocados, tomatoes, and garlic. Eating chips and dip with friends while watching football is ideal.",
            "greenFlags": [
              "✓ Homemade fresh dip",
              "✓ Uses fresh vegetables",
              "✓ Great weekend social vibe"
            ],
            "redFlags": [
              "✗ Eats all the guacamole first",
              "✗ Complains about hard avocados",
              "✗ Always double-dips chips"
            ]
          },
          {
            "tag": "🌶️ Spicy Flavors",
            "bio": "I believe good food needs rich spices and bold flavors. I enjoy discovering food trucks in different neighborhoods across the city.",
            "greenFlags": [
              "✓ Adventurous street food seeker",
              "✓ Bold culinary tastes",
              "✓ Supports local food trucks"
            ],
            "redFlags": [
              "✗ Adds hot sauce to everything",
              "✗ Ignores mild non-spicy options",
              "✗ Cooks with strong garlic smell"
            ]
          }
        ],
        "opener": "Hey there! How spicy do you like your salsa and tacos?",
        "icebreakers": [
          "Super spicy hot sauce!",
          "Mild with fresh avocado!",
          "Lots of fresh lime juice!"
        ],
        "prompt": "Write about Carlos's taco nights, fresh ingredients, and love for street food."
      }
    ],
    "travel": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Travel & Destinations ✈️",
        "title": "Mateo",
        "age": "26",
        "location": "📍 3 miles away • Mountain Base",
        "verified": true,
        "avatar": "🏔️",
        "gradient": "linear-gradient(135deg, #059669 0%, #10b981 100%)",
        "visualDescription": "Active hiker wearing a red jacket and backpack standing on a high mountain summit overlooking green valleys.",
        "interests": [
          "#MountainHiking",
          "#CampingLife",
          "#NatureTrails"
        ],
        "anthem": "🎵 'I'm Gonna Be (500 Miles)' by The Proclaimers",
        "film": "🎬 'Into the Wild' (Sean Penn)",
        "pages": [
          {
            "tag": "🏔️ Mountain Hiking",
            "bio": "Hi! I'm Mateo, 26. I love trekking in the mountains on weekends. Fresh air, green pine forests, and rocky trails make me feel active and healthy.",
            "greenFlags": [
              "✓ Great physical stamina",
              "✓ Knows safe trail routes",
              "✓ Respects nature environment"
            ],
            "redFlags": [
              "✗ Wakes up at 4 AM to hike",
              "✗ Complains about city traffic",
              "✗ Forces friends up steep hills"
            ]
          },
          {
            "tag": "⛺ Camping Under Stars",
            "bio": "I set up tents near mountain lakes and sleep under the stars. Cooking campfire meals and telling stories at night is my ideal weekend.",
            "greenFlags": [
              "✓ Skilled at tent camping",
              "✓ Makes great campfires",
              "✓ Loves starry night skies"
            ],
            "redFlags": [
              "✗ Dislikes staying in hotels",
              "✗ Mosquitoes follow him around",
              "✗ Forgets extra warm blankets"
            ]
          },
          {
            "tag": "🎒 Backpacking Gear",
            "bio": "I pack warm jackets, water bottles, and reliable hiking boots. Exploring wilderness trails gives me energy for the work week ahead.",
            "greenFlags": [
              "✓ Well-prepared for hiking",
              "✓ Quality outdoor gear",
              "✓ Environmentally conscious"
            ],
            "redFlags": [
              "✗ Talks too much about boots",
              "✗ Heavy backpack carrying",
              "✗ No cell phone signal on trips"
            ]
          }
        ],
        "opener": "Hey! Do you prefer hiking up mountains or relaxing in hotel resorts?",
        "icebreakers": [
          "Hiking up mountains!",
          "Relaxing in hotel resorts!",
          "Camping under the stars!"
        ],
        "prompt": "Describe Mateo's hiking adventures, camping trips, and love for nature."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Travel & Destinations ✈️",
        "title": "Elena",
        "age": "25",
        "location": "📍 1.8 miles away • Train Station",
        "verified": true,
        "avatar": "🚂",
        "gradient": "linear-gradient(135deg, #8b5cf6 0%, #c084fc 100%)",
        "visualDescription": "Enthusiastic train traveler sitting by a window seat watching scenic green hills roll past.",
        "interests": [
          "#TrainJourneys",
          "#ScenicRoutes",
          "#BudgetTravel"
        ],
        "anthem": "🎵 'Fast Car' by Tracy Chapman",
        "film": "🎬 'Before Sunrise' (Richard Linklater)",
        "pages": [
          {
            "tag": "🚂 Train Travel",
            "bio": "Hello! I'm Elena. I prefer taking trains instead of flying. Watching green landscapes pass by the window while listening to music is so peaceful.",
            "greenFlags": [
              "✓ Eco-friendly train traveler",
              "✓ Enjoys scenic journeys",
              "✓ Relaxed patient travel style"
            ],
            "redFlags": [
              "✗ Train delays upset her",
              "✗ Packs too many books",
              "✗ Takes very long routes"
            ]
          },
          {
            "tag": "🗺️ Hostels & Culture",
            "bio": "I stay in cozy budget hostels and meet travelers from around the world. Sharing travel tips and cooking together in shared kitchens is fun.",
            "greenFlags": [
              "✓ Friendly international mindset",
              "✓ Budget-smart travel skills",
              "✓ Makes friends easily"
            ],
            "redFlags": [
              "✗ Shared dorm noise",
              "✗ Loses hostel locker keys",
              "✗ Always talks about budget prices"
            ]
          },
          {
            "tag": "🏰 Local Discovery",
            "bio": "When I arrive in a new town, I rent a bicycle and explore local markets, quiet parks, and neighborhood art galleries.",
            "greenFlags": [
              "✓ Loves cycling exploration",
              "✓ Supports local shops",
              "✓ Curiously explores culture"
            ],
            "redFlags": [
              "✗ Bicycles on busy streets",
              "✗ Gets lost in unfamiliar towns",
              "✗ Avoids tourist taxis"
            ]
          }
        ],
        "opener": "Hi! Do you enjoy long scenic train trips or fast airplane flights?",
        "icebreakers": [
          "Scenic train journeys!",
          "Fast airplane flights!",
          "Meeting people in hostels!"
        ],
        "prompt": "Write about Elena's preference for train travel and budget hostel experiences in A2 level."
      }
    ],
    "entertainment": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Books, Films & Music 🎬",
        "title": "Oliver",
        "age": "25",
        "location": "📍 1.5 miles away • Arts District",
        "verified": true,
        "avatar": "🎧",
        "gradient": "linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)",
        "visualDescription": "Thoughtful young man wearing large over-ear headphones and holding a vinyl record album in a record shop.",
        "interests": [
          "#VinylRecords",
          "#IndieMusic",
          "#FantasyNovels"
        ],
        "anthem": "🎵 'Dreams' by Fleetwood Mac",
        "film": "🎬 'Lord of the Rings' (Peter Jackson)",
        "pages": [
          {
            "tag": "🎧 Vinyl Collector",
            "bio": "Hi! I'm Oliver, 25. I collect classic vinyl records from vintage music stores. Warm analog sound on a turntable is much better than digital streaming.",
            "greenFlags": [
              "✓ Impressive record collection",
              "✓ Deep music appreciation",
              "✓ Cool vintage aesthetic"
            ],
            "redFlags": [
              "✗ Vinyl records are expensive",
              "✗ Cleans records obsessively",
              "✗ Refuses Bluetooth speakers"
            ]
          },
          {
            "tag": "📚 Fantasy Novels",
            "bio": "I spend cozy rainy evenings reading epic fantasy book series. Immersing myself in magical worlds with dragons and wizards helps me relax.",
            "greenFlags": [
              "✓ Avid imaginative reader",
              "✓ Great book recommendations",
              "✓ Creative mind"
            ],
            "redFlags": [
              "✗ Reads 800-page heavy books",
              "✗ Compares real life to fantasy",
              "✗ Ignores texts while reading"
            ]
          },
          {
            "tag": "🎥 Sci-Fi Cinema",
            "bio": "I love watching science fiction films with space travel and time loops. Discussing movie theories with friends after the film is the best part.",
            "greenFlags": [
              "✓ Enjoys deep movie debates",
              "✓ Great sci-fi film taste",
              "✓ Thoughtful discussions"
            ],
            "redFlags": [
              "✗ Analyzes plot holes forever",
              "✗ Pauses movies to explain lore",
              "✗ Dislikes romantic comedies"
            ]
          }
        ],
        "opener": "Hey! Do you prefer listening to vinyl records or reading fantasy novels?",
        "icebreakers": [
          "Vintage vinyl records!",
          "Epic fantasy novels!",
          "Sci-fi movie nights!"
        ],
        "prompt": "Write about Oliver's vinyl record collection, reading habits, and sci-fi film discussions."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Books, Films & Music 🎬",
        "title": "Zara",
        "age": "24",
        "location": "📍 2 miles away • Theater Row",
        "verified": true,
        "avatar": "🎭",
        "gradient": "linear-gradient(135deg, #e11d48 0%, #fda4af 100%)",
        "visualDescription": "Energetic drama fan holding ticket stubs outside a bustling downtown theater.",
        "interests": [
          "#StageMusicals",
          "#MysteryNovels",
          "#PopConcerts"
        ],
        "anthem": "🎵 'Don't Stop Believin'' by Journey",
        "film": "🎬 'La La Land' (Damien Chazelle)",
        "pages": [
          {
            "tag": "🎭 Stage Musicals",
            "bio": "Hello! I'm Zara. I love live theater and Broadway musicals! Singing along to dramatic soundtrack albums brings so much joy and energy.",
            "greenFlags": [
              "✓ Enthusiastic theater lover",
              "✓ High positive energy",
              "✓ Know all musical lyrics"
            ],
            "redFlags": [
              "✗ Sings musical songs constantly",
              "✗ Expensive theater tickets",
              "✗ Dramatic personality"
            ]
          },
          {
            "tag": "🔍 Mystery Novels",
            "bio": "I read suspenseful detective mystery books before sleeping. Trying to guess the secret murderer before the final chapter is exciting!",
            "greenFlags": [
              "✓ Sharp observant mind",
              "✓ Loves clever puzzle stories",
              "✓ Fast reading speed"
            ],
            "redFlags": [
              "✗ Accuses everyone of secrets",
              "✗ Reads spoilers in advance",
              "✗ Stays up till 2 AM reading"
            ]
          },
          {
            "tag": "🎤 Live Concerts",
            "bio": "Going to stadium pop concerts with colorful light shows and thousands of cheering fans is my favorite weekend activity.",
            "greenFlags": [
              "✓ Fun energetic concert buddy",
              "✓ Loves memorable live shows",
              "✓ Great dance moves"
            ],
            "redFlags": [
              "✗ Loses her voice after concerts",
              "✗ Takes 50 concert videos",
              "✗ Buys expensive band t-shirts"
            ]
          }
        ],
        "opener": "Hi! Do you like live stage musicals or solving detective mystery books?",
        "icebreakers": [
          "Live stage musicals!",
          "Detective mystery books!",
          "Stadium pop concerts!"
        ],
        "prompt": "Describe Zara's passion for theater musicals, mystery novels, and live pop concerts."
      }
    ],
    "daily_habits": [
      {
        "type": "profile",
        "level": "A2",
        "category": "Daily Routines & Habits 🌅",
        "title": "Chloe",
        "age": "25",
        "location": "📍 1.5 miles away • Wellness District",
        "verified": true,
        "avatar": "🧘‍♀️",
        "gradient": "linear-gradient(135deg, #0d9488 0%, #2dd4bf 100%)",
        "visualDescription": "Serene yoga practitioner sitting on a mat doing morning stretching in a bright room with houseplants.",
        "interests": [
          "#YogaRoutine",
          "#Mindfulness",
          "#HealthySmoothies"
        ],
        "anthem": "🎵 'Unwritten' by Natasha Bedingfield",
        "film": "🎬 'Eat Pray Love' (Ryan Murphy)",
        "pages": [
          {
            "tag": "🧘‍♀️ Morning Yoga",
            "bio": "Hi, I'm Chloe! Every morning at 7 AM I do 20 minutes of yoga and gentle stretching on my mat. Starting the day with deep breathing keeps me calm and focused.",
            "greenFlags": [
              "✓ Healthy mindful routine",
              "✓ Flexible and physically fit",
              "✓ Positive calm presence"
            ],
            "redFlags": [
              "✗ Insists on quiet during yoga",
              "✗ Yoga mat takes up room space",
              "✗ Talks about chakra energy"
            ]
          },
          {
            "tag": "🥤 Fruit Smoothies",
            "bio": "For breakfast, I blend fresh spinach, banana, almond milk, and chia seeds. Eating healthy whole foods gives me lasting energy throughout my workday.",
            "greenFlags": [
              "✓ Nutritious food choices",
              "✓ Great smoothie recipes",
              "✓ High natural energy"
            ],
            "redFlags": [
              "✗ Loud blender at 7:30 AM",
              "✗ Dislikes fast food options",
              "✗ Spends a lot on organic chia"
            ]
          },
          {
            "tag": "🌿 Journaling Habit",
            "bio": "Before sleeping, I write three things I am grateful for in my gratitude journal. Focusing on positive moments helps me sleep peacefully.",
            "greenFlags": [
              "✓ Gratitude mindset",
              "✓ Reflective personal habits",
              "✓ Emotionally balanced"
            ],
            "redFlags": [
              "✗ Leaves journals everywhere",
              "✗ Analyzes daily feelings deeply",
              "✗ Strict 10 PM bedtime"
            ]
          }
        ],
        "opener": "Bonjour! Do you practice daily morning stretching or do you prefer a hot cup of coffee first?",
        "icebreakers": [
          "Morning yoga and stretching!",
          "Strong hot coffee first!",
          "Daily gratitude journaling!"
        ],
        "prompt": "Describe Chloe's morning yoga routine, healthy smoothie habits, and nightly journaling."
      },
      {
        "type": "profile",
        "level": "A2",
        "category": "Daily Routines & Habits 🌅",
        "title": "Ethan",
        "age": "26",
        "location": "📍 2.5 miles away • Tech Hub",
        "verified": true,
        "avatar": "⏱️",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #38bdf8 100%)",
        "visualDescription": "Organized young professional checking a smartwatch while preparing a meal-prep container.",
        "interests": [
          "#TimeManagement",
          "#MealPrep",
          "#DigitalDetox"
        ],
        "anthem": "🎵 'Higher Love' by Kygo & Whitney Houston",
        "film": "🎬 'The Pursuit of Happyness' (Gabriele Muccino)",
        "pages": [
          {
            "tag": "⏱️ Structured Days",
            "bio": "Hello! I'm Ethan, 26. I love structure and time management! I schedule my daily tasks using calendar apps and set reminders to stay organized.",
            "greenFlags": [
              "✓ Punctual and reliable",
              "✓ Never forgets appointments",
              "✓ Highly efficient habits"
            ],
            "redFlags": [
              "✗ Panics if schedule changes",
              "✗ Checks calendar 20 times a day",
              "✗ Rigid time blocks"
            ]
          },
          {
            "tag": "🍱 Sunday Meal Prep",
            "bio": "On Sunday afternoons, I cook healthy meals for the entire work week. Storing portioned containers of chicken, rice, and vegetables saves time and money.",
            "greenFlags": [
              "✓ Smart budget planner",
              "✓ Always has healthy lunch",
              "✓ Efficient kitchen skills"
            ],
            "redFlags": [
              "✗ Eats same lunch 5 days in a row",
              "✗ Refrigerator full of plastic tubs",
              "✗ Refuses spontaneous dining"
            ]
          },
          {
            "tag": "📵 Digital Detox",
            "bio": "I turn off all phone notifications after 8 PM every evening. Spending two hours screen-free reading or walking helps me rest my mind.",
            "greenFlags": [
              "✓ Healthy tech boundaries",
              "✓ Present in conversations",
              "✓ Better quality sleep"
            ],
            "redFlags": [
              "✗ Unreachable after 8 PM",
              "✗ Puts phone in a lockbox",
              "✗ Critiques people on social media"
            ]
          }
        ],
        "opener": "Hi! Do you like organizing your week with meal prep or eating spontaneously?",
        "icebreakers": [
          "Organized Sunday meal prep!",
          "Spontaneous daily eating!",
          "Digital detox after 8 PM!"
        ],
        "prompt": "Write about Ethan's time management, weekly meal prep routine, and digital detox habits."
      }
    ]
  },
  "B1": {
    "appearance": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Physical Appearance 👁️",
        "title": "Maya",
        "age": "26",
        "location": "📍 3 miles away • Cultural District",
        "verified": true,
        "avatar": "👩‍🦱",
        "gradient": "linear-gradient(135deg, #7c2d12 0%, #d97706 100%)",
        "visualDescription": "Artistic woman with voluminous dark brown curly hair and expressive dark eyes, wearing an embroidered bohemian kimono.",
        "interests": [
          "#BohoStyle",
          "#CurlyHair",
          "#HandmadeJewelry"
        ],
        "anthem": "🎵 'Dog Days Are Over' by Florence + The Machine",
        "film": "🎬 'Frida' (Julie Taymor)",
        "pages": [
          {
            "tag": "🖼️ Artistic Style",
            "bio": "I'm Maya, a 26-year-old artist. I have dark brown curly hair, dark eyes, and a warm smile. I like wearing bohemian clothes, long skirts, and handmade bracelets because they match my creative personality.",
            "greenFlags": [
              "✓ Friendly and warm expression",
              "✓ Unique personal clothing style",
              "✓ Creative and approachable"
            ],
            "redFlags": [
              "✗ Takes a long time to style her hair",
              "✗ Wears too many bracelets",
              "✗ Spends hours in vintage shops"
            ]
          },
          {
            "tag": "⚡ Studio Routine",
            "bio": "I spend my afternoons in a sunlit art studio, mixing watercolor paints and listening to indie music. My clothes often have small paint spots, which I consider badges of honor.",
            "greenFlags": [
              "✓ Dedicated to artistic craft",
              "✓ Enjoys peaceful creative spaces",
              "✓ Expressive personality"
            ],
            "redFlags": [
              "✗ Paint smudges on sofa cushions",
              "✗ Loses track of time while painting",
              "✗ Leaves paintbrushes in coffee mugs"
            ]
          },
          {
            "tag": "💭 Creative Outlook",
            "bio": "I believe art should express authentic emotions rather than perfect technical rules. I enjoy exploring flea markets for vintage clothes and old picture frames that tell stories.",
            "greenFlags": [
              "✓ Values authentic self-expression",
              "✓ Appreciates vintage aesthetics",
              "✓ Imaginative storyteller"
            ],
            "redFlags": [
              "✗ Accumulates too many vintage knick-knacks",
              "✗ Rejects strict schedules",
              "✗ Over-interprets simple art pieces"
            ]
          }
        ],
        "opener": "Hey! Do your clothes show your personality or do you wear whatever is comfortable?",
        "icebreakers": [
          "Comfortable clothes first!",
          "I like creative styles!",
          "A bit of both!"
        ],
        "prompt": "Describe Maya's appearance and explain how her clothes show her style."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Physical Appearance 👁️",
        "title": "Julian",
        "age": "29",
        "location": "📍 2.5 miles away • Timber Ridge",
        "verified": true,
        "avatar": "🧔",
        "gradient": "linear-gradient(135deg, #15803d 0%, #166534 100%)",
        "visualDescription": "Broad-shouldered man with a trimmed dark beard and short undercut hair, wearing a heavy flannel plaid shirt.",
        "interests": [
          "#TallBuild",
          "#ShortBeard",
          "#FlannelShirt"
        ],
        "anthem": "🎵 'Take Me Home, Country Roads' by John Denver",
        "film": "🎬 'Into the Wild' (Sean Penn)",
        "pages": [
          {
            "tag": "🖼️ Outdoor Look",
            "bio": "I'm Julian, a 29-year-old who loves spending time outdoors. I'm tall with a short dark beard and hazel eyes. I usually wear comfortable flannel shirts and sturdy boots because I like hiking on weekends.",
            "greenFlags": [
              "✓ Neat and tidy beard",
              "✓ Tall and confident posture",
              "✓ Practical clothes for outdoors"
            ],
            "redFlags": [
              "✗ Beard oil on shirt collars",
              "✗ Dislikes wearing formal ties",
              "✗ Owns too many flannel shirts"
            ]
          },
          {
            "tag": "⚡ Mountain Hikes",
            "bio": "On Saturday mornings, I pack my backpack with trail mix and a compass to explore national park trails. I enjoy fresh pine air and camping under starry night skies.",
            "greenFlags": [
              "✓ Experienced outdoor navigator",
              "✓ Healthy active lifestyle",
              "✓ Enjoys natural scenery"
            ],
            "redFlags": [
              "✗ Wakes up at 4:30 AM for sunrise hikes",
              "✗ No phone signal on mountain trails",
              "✗ Muddy boots near the front door"
            ]
          }
        ],
        "opener": "Hi! Do you like a casual outdoor style or a neat office look?",
        "icebreakers": [
          "Outdoor casual for me!",
          "Neat office look!",
          "Whatever is easy!"
        ],
        "prompt": "Describe Julian's look and explain why his clothes suit his outdoor hobbies."
      }
    ],
    "professions": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Professions 💼",
        "title": "Marcus",
        "age": "28",
        "location": "📍 1.5 miles away • Tech Hub",
        "verified": true,
        "avatar": "👨‍💻",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #0369a1 100%)",
        "visualDescription": "Software engineer working at a dual-monitor standing desk, wearing noise-canceling headphones.",
        "interests": [
          "#WebDeveloper",
          "#Coding",
          "#TechLife"
        ],
        "anthem": "🎵 'Digital Love' by Daft Punk",
        "film": "🎬 'The Matrix' (The Wachowskis)",
        "pages": [
          {
            "tag": "🖼️ Tech Life",
            "bio": "I'm Marcus, a software developer. I build websites and phone apps that help people organize their work. My job includes writing code, fixing computer bugs, and working in a team with other programmers.",
            "greenFlags": [
              "✓ Flexible work-from-home hours",
              "✓ Good at solving difficult problems",
              "✓ Good career opportunities"
            ],
            "redFlags": [
              "✗ Sitting at a desk for long hours",
              "✗ Looking at screens all day",
              "✗ Uses technical computer words"
            ]
          },
          {
            "tag": "⚡ Daily Workflow",
            "bio": "My workday revolves around daily team standup meetings, writing clean code, and testing software features. When a difficult bug appears, solving it feels like winning a puzzle game.",
            "greenFlags": [
              "✓ Logical problem solver",
              "✓ Good team communicator",
              "✓ Persistent under pressure"
            ],
            "redFlags": [
              "✗ Drinks 4 cups of coffee per day",
              "✗ Gets engrossed in coding problems",
              "✗ Explains code at parties"
            ]
          },
          {
            "tag": "💭 Future Tech",
            "bio": "Technology should make human lives simpler rather than more complicated. I prefer open-source software projects and apps that respect user privacy.",
            "greenFlags": [
              "✓ Advocates for user privacy",
              "✓ Supports open-source tech",
              "✓ Thoughtful about digital balance"
            ],
            "redFlags": [
              "✗ Debates operating systems",
              "✗ Refuses popular privacy-invading apps",
              "✗ Overanalyzes app interface designs"
            ]
          }
        ],
        "opener": "Hello! Would you like to work in technology and software development?",
        "icebreakers": [
          "Tech sounds interesting!",
          "Too much screen time!",
          "I already work in tech!"
        ],
        "prompt": "What does Marcus do every day in his job as a software developer?"
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Professions 💼",
        "title": "Clara",
        "age": "29",
        "location": "📍 2 miles away • Press Building",
        "verified": true,
        "avatar": "👩‍💻",
        "gradient": "linear-gradient(135deg, #047857 0%, #10b981 100%)",
        "visualDescription": "Journalist holding a recording microphone, interviewing an engineer near wind turbines.",
        "interests": [
          "#Reporter",
          "#GreenEnergy",
          "#Storytelling"
        ],
        "anthem": "🎵 'Blowin' in the Wind' by Bob Dylan",
        "film": "🎬 'Spotlight' (Tom McCarthy)",
        "pages": [
          {
            "tag": "🖼️ Reporter Profile",
            "bio": "I'm Clara, a news reporter who writes articles about the environment. I travel to interview scientists and report on solar power and clean energy projects. I love my job because I share important news with the public.",
            "greenFlags": [
              "✓ Helps people understand environmental issues",
              "✓ Travels to interesting places for stories",
              "✓ Learns new things every day"
            ],
            "redFlags": [
              "✗ Tight deadlines for news articles",
              "✗ Long and unpredictable work hours",
              "✗ Stressful newsroom environment"
            ]
          },
          {
            "tag": "⚡ Fieldwork Routine",
            "bio": "One day I'm interviewing marine biologists on a coastal boat, and the next day I'm writing articles in a noisy newsroom. I always carry two audio recorders and a waterproof notebook.",
            "greenFlags": [
              "✓ Adaptable to field environments",
              "✓ Well-prepared interviewer",
              "✓ Energetic investigation skills"
            ],
            "redFlags": [
              "✗ Phone constantly buzzing with news alerts",
              "✗ Irregular travel schedules",
              "✗ Quick rushed meals on assignment"
            ]
          }
        ],
        "opener": "Hi! What environmental news or clean energy topic interests you most?",
        "icebreakers": [
          "Solar and wind energy!",
          "Protecting wildlife!",
          "Recycling and reducing waste!"
        ],
        "prompt": "Explain what Clara does as a news reporter and why her job is important."
      }
    ],
    "properties": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Properties 🏠",
        "title": "Arthur",
        "age": "31",
        "location": "📍 1 mile away • Riverfront District",
        "verified": true,
        "avatar": "🏭",
        "gradient": "linear-gradient(135deg, #475569 0%, #334155 100%)",
        "visualDescription": "Historic paper mill converted into a luxury loft with exposed red brick, heavy timber beams, and river views.",
        "interests": [
          "#BrickWalls",
          "#RiverView",
          "#SpaciousLoft"
        ],
        "anthem": "🎵 'Riverside' by Agnes Obel",
        "film": "🎬 'The Grand Budapest Hotel' (Wes Anderson)",
        "pages": [
          {
            "tag": "🖼️ Industrial Loft",
            "bio": "I'm Arthur. I live in a two-bedroom apartment inside an old restored paper factory. It has exposed red brick walls, large wooden beams, and big windows looking over the river.",
            "greenFlags": [
              "✓ Beautiful historical character",
              "✓ Great river view from the window",
              "✓ Quiet double-glazed windows"
            ],
            "redFlags": [
              "✗ High monthly building fees",
              "✗ High ceilings are hard to heat",
              "✗ Strict rules about making changes"
            ]
          },
          {
            "tag": "⚡ Living Environment",
            "bio": "The open space is decorated with industrial metal lamps and vintage wooden furniture. In the evening, river currents reflect soft city lights across the ceiling.",
            "greenFlags": [
              "✓ Warm ambient lighting",
              "✓ Stylish industrial decor",
              "✓ Peaceful river reflection"
            ],
            "redFlags": [
              "✗ Metal furniture can feel cold",
              "✗ Echoes if music is turned up loud",
              "✗ Requires frequent window dusting"
            ]
          },
          {
            "tag": "💭 Architectural Character",
            "bio": "I value buildings that preserve their history instead of tearing down old walls. Living in a space with character makes daily life feel grounded and inspiring.",
            "greenFlags": [
              "✓ Respects heritage architecture",
              "✓ Inspiring living space",
              "✓ Passionate about restoration"
            ],
            "redFlags": [
              "✗ Dislikes modern cookie-cutter flats",
              "✗ Complains about poor building restoration",
              "✗ Overcharges for loft aesthetic"
            ]
          }
        ],
        "opener": "Welcome! Do you like living in old renovated buildings or modern new flats?",
        "icebreakers": [
          "Old renovated buildings!",
          "Modern new flats!",
          "A quiet house with a garden!"
        ],
        "prompt": "Describe Arthur's apartment and list two good things and two bad things about it."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Properties 🏠",
        "title": "Chloe",
        "age": "30",
        "location": "📍 1.5 miles away • Heritage Hill",
        "verified": true,
        "avatar": "🏛️",
        "gradient": "linear-gradient(135deg, #7c3aed 0%, #c084fc 100%)",
        "visualDescription": "Renovated three-story Victorian townhouse with high ceilings, bay windows, and private garden.",
        "interests": [
          "#VictorianTownhouse",
          "#BayWindows",
          "#PrivateGarden"
        ],
        "anthem": "🎵 'Our House' by Crosby, Stills, Nash & Young",
        "film": "🎬 'Mary Poppins Returns' (Rob Marshall)",
        "pages": [
          {
            "tag": "🖼️ Townhouse Character",
            "bio": "I'm Chloe. I restored a three-story Victorian townhouse featuring original stained-glass doors, spacious bay windows, and a walled courtyard garden.",
            "greenFlags": [
              "✓ Elegant architectural heritage",
              "✓ Private courtyard garden",
              "✓ High ceilings and sunlight"
            ],
            "redFlags": [
              "✗ Ongoing maintenance work",
              "✗ Steep stairs between floors",
              "✗ Drafty window frames in winter"
            ]
          },
          {
            "tag": "⚡ Living Space",
            "bio": "The ground floor contains a warm library with built-in bookshelves and a wood-burning fireplace where I read on chilly autumn evenings.",
            "greenFlags": [
              "✓ Cozy wood-burning fireplace",
              "✓ Built-in book collection",
              "✓ Distinct functional floors"
            ],
            "redFlags": [
              "✗ Firewood storage required",
              "✗ Chimney cleaning costs",
              "✗ Dusting endless bookshelves"
            ]
          }
        ],
        "opener": "Welcome! Would you enjoy living in a multi-story historic townhouse?",
        "icebreakers": [
          "Historic townhouses have charm!",
          "Too many stairs for me!",
          "I love bay windows!"
        ],
        "prompt": "Describe the features of Chloe's Victorian townhouse and list one drawback."
      }
    ],
    "character": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Character Traits 🧠",
        "title": "Daniel",
        "age": "28",
        "location": "📍 1.8 miles away • Maple Heights",
        "verified": true,
        "avatar": "🤝",
        "gradient": "linear-gradient(135deg, #0d9488 0%, #14b8a6 100%)",
        "visualDescription": "Warm-hearted man listening intently during a conversation at a quiet coffee shop.",
        "interests": [
          "#GoodListener",
          "#KindFriend",
          "#Helpful"
        ],
        "anthem": "🎵 'Lean on Me' by Bill Withers",
        "film": "🎬 'Good Will Hunting' (Gus Van Sant)",
        "pages": [
          {
            "tag": "🖼️ Supportive Friend",
            "bio": "I'm Daniel, 28. I am a helpful person who listens carefully when friends have problems. I try to understand people's feelings and give thoughtful advice when they ask for help.",
            "greenFlags": [
              "✓ Very supportive and trustworthy",
              "✓ Understands how friends feel",
              "✓ Helps settle arguments peacefully"
            ],
            "redFlags": [
              "✗ Takes on other people's worries",
              "✗ Avoids conflicts and arguments",
              "✗ Needs quiet time alone after long chats"
            ]
          },
          {
            "tag": "⚡ Daily Connections",
            "bio": "I prefer deep one-on-one conversations over loud group parties. I regularly call old friends to check on them and remember small details about their lives.",
            "greenFlags": [
              "✓ Excellent active listener",
              "✓ Remembers birthdays and details",
              "✓ Deep meaningful relationships"
            ],
            "redFlags": [
              "✗ Gets exhausted by large social gatherings",
              "✗ Hesitant to express personal anger",
              "✗ Overthinks text messages"
            ]
          }
        ],
        "opener": "Hello! When you make decisions, do you follow logical thinking or your feelings?",
        "icebreakers": [
          "I use logical thinking!",
          "I trust my feelings!",
          "A mix of both!"
        ],
        "prompt": "Why is Daniel a good friend? Describe his personality traits."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Character Traits 🧠",
        "title": "Olivia",
        "age": "27",
        "location": "📍 1 mile away • Community Center",
        "verified": true,
        "avatar": "🕊️",
        "gradient": "linear-gradient(135deg, #059669 0%, #34d399 100%)",
        "visualDescription": "Empathetic community mediator facilitating a peaceful discussion in a sunlit meeting room.",
        "interests": [
          "#Empathetic",
          "#ConflictResolution",
          "#GoodCommunicator"
        ],
        "anthem": "🎵 'Bridge Over Troubled Water' by Simon & Garfunkel",
        "film": "🎬 'Wonder' (Stephen Chbosky)",
        "pages": [
          {
            "tag": "🖼️ Empathetic Profile",
            "bio": "I'm Olivia, 27. I am an empathetic and emotionally intelligent person who helps friends and colleagues resolve misunderstandings through active listening and constructive dialogue.",
            "greenFlags": [
              "✓ Calming empathetic presence",
              "✓ Fair and objective mediator",
              "✓ Excellent listening skills"
            ],
            "redFlags": [
              "✗ Takes on emotional stress of others",
              "✗ Avoids quick aggressive confrontations",
              "✗ Over-analyzes tone of voice"
            ]
          },
          {
            "tag": "⚡ Community Work",
            "bio": "I facilitate group workshops on effective communication and conflict resolution. Creating safe spaces where people feel heard is deeply rewarding.",
            "greenFlags": [
              "✓ Skilled facilitator",
              "✓ Fosters group harmony",
              "✓ Patient and compassionate"
            ],
            "redFlags": [
              "✗ Exhausted after long group sessions",
              "✗ Dislikes harsh arguments",
              "✗ Spends hours mediating disputes"
            ]
          }
        ],
        "opener": "Hello! How do you usually handle misunderstandings with friends or colleagues?",
        "icebreakers": [
          "Calm open dialogue!",
          "Give it time to cool down!",
          "Ask a neutral friend for help!"
        ],
        "prompt": "Why is Olivia effective at resolving conflicts among friends?"
      }
    ],
    "hobbies": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Hobbies 🎨",
        "title": "Ethan",
        "age": "27",
        "location": "📍 Peak Climbing Gym",
        "verified": true,
        "avatar": "🧗‍♂️",
        "gradient": "linear-gradient(135deg, #15803d 0%, #22c55e 100%)",
        "visualDescription": "Climber wearing chalk bag, gripping indoor bouldering holds on an overhang wall.",
        "interests": [
          "#RockClimbing",
          "#Bouldering",
          "#Fitness"
        ],
        "anthem": "🎵 'Eye of the Tiger' by Survivor",
        "film": "🎬 'Free Solo' (Jimmy Chin & Elizabeth Chai Vasarhelyi)",
        "pages": [
          {
            "tag": "🖼️ Bouldering Focus",
            "bio": "I'm Ethan. Bouldering is my main hobby. I climb short indoor walls without ropes, using chalk on my hands to grip the holds. It gives me a great workout and helps me practice focus and problem solving.",
            "greenFlags": [
              "✓ Builds good physical strength",
              "✓ Friendly climbing community",
              "✓ Helps improve mental focus"
            ],
            "redFlags": [
              "✗ White chalk dust on clothes",
              "✗ Sore fingers and hands",
              "✗ Spends hours practicing climbing moves"
            ]
          },
          {
            "tag": "⚡ Gym & Community",
            "bio": "I spend three evenings a week at the local climbing gym analyzing route angles with gym buddies. Working out a difficult climbing sequence feels like solving a physical riddle.",
            "greenFlags": [
              "✓ Great problem-solving mindset",
              "✓ Supportive climbing partners",
              "✓ High determination and stamina"
            ],
            "redFlags": [
              "✗ Calloused palms and fingers",
              "✗ Talks endlessly about climb grades",
              "✗ Buys expensive climbing shoes"
            ]
          },
          {
            "tag": "💭 Outdoor Ambitions",
            "bio": "My goal for next summer is outdoor bouldering in granite canyons. Being outdoors surrounded by rock faces and nature tests your physical limits in the best way.",
            "greenFlags": [
              "✓ Ambitious outdoor goals",
              "✓ Respect for nature and safety",
              "✓ Athletic drive"
            ],
            "redFlags": [
              "✗ Weekend trips revolve only around rocks",
              "✗ Risks minor finger sprains",
              "✗ Obsesses over weather forecasts"
            ]
          }
        ],
        "opener": "Hey! Have you ever tried indoor rock climbing or bouldering?",
        "icebreakers": [
          "Yes, I love climbing!",
          "No, I'm afraid of heights!",
          "I want to try it!"
        ],
        "prompt": "Explain why Ethan likes bouldering and what equipment he uses."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Hobbies 🎨",
        "title": "Nora",
        "age": "26",
        "location": "📍 Stargazing Ridge",
        "verified": true,
        "avatar": "🔭",
        "gradient": "linear-gradient(135deg, #1e1b4b 0%, #4338ca 100%)",
        "visualDescription": "Amateur astronomer adjusting a telescope under a dark night sky dotted with constellations.",
        "interests": [
          "#Astronomy",
          "#Stargazing",
          "#Telescope"
        ],
        "anthem": "🎵 'A Sky Full of Stars' by Coldplay",
        "film": "🎬 'Interstellar' (Christopher Nolan)",
        "pages": [
          {
            "tag": "🖼️ Night Astronomy",
            "bio": "I'm Nora. Amateur astronomy is my passion! On clear nights, I drive to dark sky reserves with my reflector telescope to observe planetary rings, nebulae, and star clusters.",
            "greenFlags": [
              "✓ Deep knowledge of constellations",
              "✓ Fascinating night sky guide",
              "✓ Patient observation habits"
            ],
            "redFlags": [
              "✗ Stays up until 3 AM",
              "✗ Dislikes urban light pollution",
              "✗ Expensive telescope lenses"
            ]
          },
          {
            "tag": "⚡ Sky Observations",
            "bio": "Tracking celestial movements teaches you patience and perspective. Seeing Saturn's rings clearly through your own lens is an awe-inspiring experience.",
            "greenFlags": [
              "✓ Inspiring scientific curiosity",
              "✓ Great night photography skills",
              "✓ Mindful cosmic perspective"
            ],
            "redFlags": [
              "✗ Cold late-night trips",
              "✗ Sleepy on workday mornings",
              "✗ Frustrated by cloudy weather"
            ]
          }
        ],
        "opener": "Hey! Have you ever looked at planets or nebulae through a telescope?",
        "icebreakers": [
          "Yes, stargazing is amazing!",
          "I want to see Saturn's rings!",
          "Night sky photography!"
        ],
        "prompt": "Explain what equipment Nora uses and what she observes in the night sky."
      }
    ],
    "nationalities": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Nationalities & Cultures 🌍",
        "title": "Amara",
        "age": "28",
        "location": "📍 Lagos, Nigeria • 🇳🇬 Igbo & English",
        "verified": true,
        "avatar": "🇳🇬",
        "gradient": "linear-gradient(135deg, #15803d 0%, #22c55e 100%)",
        "visualDescription": "Nigerian woman wearing a patterned Ankara headwrap and traditional attire in Lagos.",
        "interests": [
          "#NigerianFood",
          "#Afrobeats",
          "#LagosLife"
        ],
        "anthem": "🎵 'Last Last' by Burna Boy",
        "film": "🎬 'The Wedding Party' (Kemi Adetiba)",
        "pages": [
          {
            "tag": "🖼️ Lagos Roots",
            "bio": "Kedu! I'm Amara from Lagos, Nigeria. Nigeria has many cultural groups, lively Afrobeats music, and great food. We love getting together with family to eat spicy Jollof rice and listen to music.",
            "greenFlags": [
              "✓ Hosts fun dinners with great music",
              "✓ Cooks delicious spicy Jollof rice",
              "✓ Welcoming and cheerful"
            ],
            "redFlags": [
              "✗ Long debates about who cooks the best rice",
              "✗ Loud phone conversations",
              "✗ Expects traditional politeness"
            ]
          },
          {
            "tag": "⚡ Lagos Energy",
            "bio": "Lagos is an energetic city that never sleeps. I love visiting colorful fabric markets, dancing to Afrobeats at weekend celebrations, and cooking rich stews for friends.",
            "greenFlags": [
              "✓ Vibrant social host",
              "✓ Great music playlists",
              "✓ Generous cooking portions"
            ],
            "redFlags": [
              "✗ Music always turned to maximum volume",
              "✗ Debates Jollof rice recipes for hours",
              "✗ Generous portions cause food comas"
            ]
          }
        ],
        "opener": "Kedu! Do you like trying spicy foods or listening to music from other countries?",
        "icebreakers": [
          "I love spicy food!",
          "I listen to music from everywhere!",
          "Tell me more about Lagos!"
        ],
        "prompt": "Describe Amara's country and food traditions using simple B1 sentences."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Nationalities & Cultures 🌍",
        "title": "Aarav",
        "age": "27",
        "location": "📍 Jaipur, India • 🇮🇳 Hindi & English",
        "verified": true,
        "avatar": "🇮🇳",
        "gradient": "linear-gradient(135deg, #d97706 0%, #f59e0b 100%)",
        "visualDescription": "Indian man in a blue kurta near Jaipur monuments holding cardamom chai.",
        "interests": [
          "#IndianCulture",
          "#Jaipur",
          "#MasalaChai"
        ],
        "anthem": "🎵 'Jai Ho' by A.R. Rahman",
        "film": "🎬 '3 Idiots' (Rajkumar Hirani)",
        "pages": [
          {
            "tag": "🖼️ Jaipur Heritage",
            "bio": "Namaste! I'm Aarav from Jaipur, India. India is famous for colorful festivals, ancient palaces, and delicious spices. In Indian culture, welcoming guests is very important. I drink warm cardamom tea every morning.",
            "greenFlags": [
              "✓ Very welcoming to guests",
              "✓ Knows how to cook with rich spices",
              "✓ Enjoys sharing local traditions"
            ],
            "redFlags": [
              "✗ Dislikes instant tea bags",
              "✗ Talks for hours about cricket",
              "✗ Wiggles head while talking"
            ]
          },
          {
            "tag": "⚡ Daily Rhythms",
            "bio": "I work in textile design, preserving traditional block-printing patterns passed down through generations. Every morning starts with fresh spiced chai brewed with real ginger and cardamom.",
            "greenFlags": [
              "✓ Preserves cultural craftsmanship",
              "✓ Master of authentic spiced chai",
              "✓ Artistic attention to detail"
            ],
            "redFlags": [
              "✗ Rejects microwave tea heating",
              "✗ Gets passionate about cricket match umpire decisions",
              "✗ Fills home with patterned fabrics"
            ]
          },
          {
            "tag": "💭 Hospitality Values",
            "bio": "'Atithi Devo Bhava' means the guest is like God in our culture. Hosting friends for home-cooked thali dinners with aromatic spices brings me immense satisfaction.",
            "greenFlags": [
              "✓ Exceptional guest hospitality",
              "✓ Authentic Indian home cooking",
              "✓ Warm cultural pride"
            ],
            "redFlags": [
              "✗ Insists guests eat third helpings",
              "✗ Disapproves of bland unspiced food",
              "✗ Long elaborate dinner preparation"
            ]
          }
        ],
        "opener": "Namaste! Have you ever tasted Indian food or visited historical monuments?",
        "icebreakers": [
          "I love Indian spices!",
          "I want to visit Jaipur!",
          "Masala chai is great!"
        ],
        "prompt": "Describe Aarav's home city and cultural traditions."
      }
    ],
    "food": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Food & Cuisine 🍜",
        "title": "Amina",
        "age": "28",
        "location": "📍 2.5 miles away • Spice Market",
        "verified": true,
        "avatar": "🍲",
        "gradient": "linear-gradient(135deg, #d97706 0%, #b45309 100%)",
        "visualDescription": "Warm home cook serving fragrant tagine stew seasoned with saffron and almonds from a clay vessel.",
        "interests": [
          "#MoroccanTagine",
          "#AromaticSpices",
          "#SlowCooking"
        ],
        "anthem": "🎵 'Desert Rose' by Sting & Cheb Mami",
        "film": "🎬 'The Hundred-Foot Journey' (Lasse Hallström)",
        "pages": [
          {
            "tag": "🍲 Slow-Cooked Stews",
            "bio": "I'm Amina, 28. I specialize in slow-cooked Moroccan tagines with chickpeas, tender lamb, apricot, and cinnamon. Slow cooking brings out deep, comforting flavors that warm the soul.",
            "greenFlags": [
              "✓ Rich flavorful dishes",
              "✓ Traditional spice blending",
              "✓ Patient culinary artisan"
            ],
            "redFlags": [
              "✗ Cooking takes 4 hours",
              "✗ Uses obscure spice mixes",
              "✗ Dislikes quick microwaved meals"
            ]
          },
          {
            "tag": "🌿 Spice Explorer",
            "bio": "I frequently visit local spice markets to source authentic saffron, cumin, and cardamom. Combining unusual savory and sweet elements creates memorable dining experiences.",
            "greenFlags": [
              "✓ Deep knowledge of spices",
              "✓ Buys high-quality ingredients",
              "✓ Creative sweet-savory combinations"
            ],
            "redFlags": [
              "✗ Kitchen cabinet overflowing with jars",
              "✗ Overwhelms guests with spice history",
              "✗ Strict about ingredient origins"
            ]
          },
          {
            "tag": "🫖 Mint Tea Tradition",
            "bio": "After dinner, I serve sweet fresh mint tea poured high into decorative glasses. Food is meant to be a slow, shared celebration rather than a rushed daily chore.",
            "greenFlags": [
              "✓ Hospitable tea ceremony",
              "✓ Values meaningful meal conversations",
              "✓ Warm welcoming host"
            ],
            "redFlags": [
              "✗ Tea has a lot of sugar",
              "✗ Dinner lasts past midnight",
              "✗ Dislikes eating on the go"
            ]
          }
        ],
        "opener": "Hello! Do you prefer quick 15-minute meals or slow-cooked dishes with complex spices?",
        "icebreakers": [
          "Slow-cooked flavorful meals!",
          "Quick and easy dinners!",
          "I love fragrant mint tea!"
        ],
        "prompt": "Discuss Amina's approach to slow cooking and how spices elevate meal experiences."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Food & Cuisine 🍜",
        "title": "Liam",
        "age": "29",
        "location": "📍 1 mile away • Greenbelt District",
        "verified": true,
        "avatar": "🥗",
        "gradient": "linear-gradient(135deg, #15803d 0%, #4ade80 100%)",
        "visualDescription": "Health-conscious farm-to-table enthusiast preparing a colorful organic Buddha bowl with quinoa and avocado.",
        "interests": [
          "#FarmToTable",
          "#PlantBased",
          "#OrganicProduce"
        ],
        "anthem": "🎵 'Sunflower' by Post Malone & Swae Lee",
        "film": "🎬 'Forks Over Knives' (Lee Fulkerson)",
        "pages": [
          {
            "tag": "🥗 Plant-Based Bowls",
            "bio": "I'm Liam, 29. I advocate for sustainable farm-to-table eating. My kitchen is filled with colorful bowls of quinoa, roasted sweet potatoes, kale, and homemade tahini dressing.",
            "greenFlags": [
              "✓ Healthy balanced nutrition",
              "✓ Eco-conscious food choices",
              "✓ Delicious homemade dressings"
            ],
            "redFlags": [
              "✗ Inspects produce for organic labels",
              "✗ Complains about processed food",
              "✗ Talks endlessly about kale"
            ]
          },
          {
            "tag": "🌱 Community Garden",
            "bio": "I grow my own heirloom tomatoes, basil, and microgreens in a community garden plot. Eating produce harvested just an hour prior completely changes your perspective on food.",
            "greenFlags": [
              "✓ Grows fresh vegetables",
              "✓ Active community member",
              "✓ Passionate about freshness"
            ],
            "redFlags": [
              "✗ Dirt under fingernails from gardening",
              "✗ Gives guests unwashed tomatoes",
              "✗ Dislikes grocery supermarket chains"
            ]
          },
          {
            "tag": "🍹 Green Smoothies",
            "bio": "My mornings start with a nutrient-dense spinach, ginger, and spirulina smoothie. Staying healthy gives me boundless energy for outdoor hiking weekend trips.",
            "greenFlags": [
              "✓ High energy lifestyle",
              "✓ Clean eating habits",
              "✓ Creative smoothie recipes"
            ],
            "redFlags": [
              "✗ Smoothies taste very earthy",
              "✗ Loud blender noise at 6 AM",
              "✗ Rejects birthday cakes"
            ]
          }
        ],
        "opener": "Hi! Are you interested in plant-based organic food or traditional comfort meals?",
        "icebreakers": [
          "Plant-based organic bowls!",
          "Hearty traditional comfort food!",
          "Fresh home-grown vegetables!"
        ],
        "prompt": "Explain Liam's farm-to-table philosophy and the benefits of organic plant-based diets."
      }
    ],
    "travel": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Travel & Destinations ✈️",
        "title": "Diego",
        "age": "28",
        "location": "📍 2 miles away • Nomad Hub",
        "verified": true,
        "avatar": "🎒",
        "gradient": "linear-gradient(135deg, #ea580c 0%, #f97316 100%)",
        "visualDescription": "Adventurous solo traveler with a large backpack standing at a bustling crossroad in Southeast Asia.",
        "interests": [
          "#SoloBackpacking",
          "#CulturalImmersion",
          "#DigitalNomad"
        ],
        "anthem": "🎵 'Society' by Eddie Vedder",
        "film": "🎬 'The Secret Life of Walter Mitty' (Ben Stiller)",
        "pages": [
          {
            "tag": "🎒 Solo Backpacking",
            "bio": "I'm Diego, 28. For three years I've worked remotely while backpacking through South America and Asia. Solo travel forces you out of your comfort zone and builds deep confidence.",
            "greenFlags": [
              "✓ Highly independent traveler",
              "✓ Adaptable to new environments",
              "✓ Rich global perspective"
            ],
            "redFlags": [
              "✗ Rarely stays in one place long",
              "✗ Moves city every two weeks",
              "✗ Lives out of a suitcase"
            ]
          },
          {
            "tag": "🌏 Cultural Immersion",
            "bio": "Rather than visiting crowded tourist traps, I stay with local families, learn basic regional phrases, and participate in traditional cooking workshops.",
            "greenFlags": [
              "✓ Respectful of local customs",
              "✓ Learns regional languages",
              "✓ Seeks authentic experiences"
            ],
            "redFlags": [
              "✗ Avoids popular famous landmarks",
              "✗ Critiques commercial tourism",
              "✗ Eats questionable street food"
            ]
          },
          {
            "tag": "💻 Remote Work Balance",
            "bio": "Balancing laptop work from beach cafés with weekend jungle treks requires self-discipline, reliable Wi-Fi, and a flexible mind.",
            "greenFlags": [
              "✓ Disciplined remote worker",
              "✓ Tech-savvy nomad",
              "✓ Balances work and adventure"
            ],
            "redFlags": [
              "✗ Searches constantly for Wi-Fi",
              "✗ Works during vacation days",
              "✗ Time zone confusion for calls"
            ]
          }
        ],
        "opener": "Hey! Could you imagine working remotely while backpacking around the world?",
        "icebreakers": [
          "That sounds like a dream!",
          "I prefer settled stability!",
          "I love learning local languages!"
        ],
        "prompt": "Analyze Diego's digital nomad lifestyle and the rewards of solo backpacking."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Travel & Destinations ✈️",
        "title": "Freja",
        "age": "27",
        "location": "📍 1.2 miles away • Nordic District",
        "verified": true,
        "avatar": "🌌",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #1e3a8a 100%)",
        "visualDescription": "Nordic traveler in a cozy wool sweater watching glowing green Northern Lights in a snowy Arctic landscape.",
        "interests": [
          "#NorthernLights",
          "#ArcticExpeditions",
          "#CozyCabin"
        ],
        "anthem": "🎵 'Ho Hey' by The Lumineers",
        "film": "🎬 'Everest' (Baltasar Kormákur)",
        "pages": [
          {
            "tag": "🌌 Arctic Lights",
            "bio": "I'm Freja, 27. I have a passion for cold-climate destinations—chasing the Northern Lights in Lapland, dog sledding across fjords, and staying in wooden wilderness cabins.",
            "greenFlags": [
              "✓ Unique travel destination taste",
              "✓ Comfortable in cold weather",
              "✓ Appreciates quiet nature"
            ],
            "redFlags": [
              "✗ Dislikes hot tropical beaches",
              "✗ Heavy winter clothing luggage",
              "✗ Freezing night photography sessions"
            ]
          },
          {
            "tag": "🔥 Sauna & Hygge",
            "bio": "Nothing compares to plunging into a cold lake after a traditional wood-fired sauna, followed by hot berry juice beside a roaring fireplace.",
            "greenFlags": [
              "✓ Embraces Scandinavian wellness",
              "✓ Creates cozy hygge atmosphere",
              "✓ Healthy outdoor mindset"
            ],
            "redFlags": [
              "✗ Insists on ice swimming",
              "✗ High cost of Arctic travel",
              "✗ Dislikes crowded warm resorts"
            ]
          },
          {
            "tag": "📸 Night Astronomy",
            "bio": "Photographing auroras and constellations beneath crisp Arctic night skies requires patience, tripod setups, and sub-zero thermal gear.",
            "greenFlags": [
              "✓ Skilled night photographer",
              "✓ Patient nature observer",
              "✓ Well-equipped outdoor adventurer"
            ],
            "redFlags": [
              "✗ Stays outside in -20°C weather",
              "✗ Loses feeling in fingers",
              "✗ Obsessed with weather forecasts"
            ]
          }
        ],
        "opener": "Hello! Would you rather chase the Northern Lights in winter or sunbathe on a tropical island?",
        "icebreakers": [
          "Northern Lights expedition!",
          "Tropical island sunbathing!",
          "Cozy cabin by the fire!"
        ],
        "prompt": "Discuss Freja's Arctic expedition interests and Scandinavian sauna traditions."
      }
    ],
    "entertainment": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Books, Films & Music 🎬",
        "title": "Felix",
        "age": "27",
        "location": "📍 1.2 miles away • Indie Cinema",
        "verified": true,
        "avatar": "🎞️",
        "gradient": "linear-gradient(135deg, #1e293b 0%, #475569 100%)",
        "visualDescription": "Film student holding a classic 35mm film camera in front of an independent arthouse cinema marquee.",
        "interests": [
          "#ArthouseCinema",
          "#FilmAnalysis",
          "#ClassicLiterature"
        ],
        "anthem": "🎵 'Heroes' by David Bowie",
        "film": "🎬 'Cinema Paradiso' (Giuseppe Tornatore)",
        "pages": [
          {
            "tag": "🎞️ Arthouse Cinema",
            "bio": "I'm Felix, 27. I'm deeply passionate about independent cinema and classic European director retrospectives. Subtitled foreign films offer profound glimpses into diverse human conditions.",
            "greenFlags": [
              "✓ Cultured cinematic knowledge",
              "✓ Appreciates artistic storytelling",
              "✓ Thoughtful film discussions"
            ],
            "redFlags": [
              "✗ Refuses mainstream blockbusters",
              "✗ Over-analyzes simple scenes",
              "✗ Prefers 3-hour black-and-white films"
            ]
          },
          {
            "tag": "📖 Classic Literature",
            "bio": "My bookshelf is filled with 19th-century literature and philosophical essays. Analyzing character motivations and thematic symbolism enriches my worldview.",
            "greenFlags": [
              "✓ Well-read intellectual mind",
              "✓ Articulate vocabulary",
              "✓ Deep analytical capacity"
            ],
            "redFlags": [
              "✗ Uses overly complex words",
              "✗ Judges people by their reading list",
              "✗ Quiet and introverted at parties"
            ]
          },
          {
            "tag": "🎷 Jazz & Soundtrack",
            "bio": "I listen to instrumental jazz and orchestral film scores while writing script analyses. Music should evoke subtle mood atmospheres rather than loud repetitive hooks.",
            "greenFlags": [
              "✓ Sophisticated musical taste",
              "✓ Great background music playlists",
              "✓ Creative writing passion"
            ],
            "redFlags": [
              "✗ Finds pop radio songs annoying",
              "✗ Plays somber jazz endlessly",
              "✗ Strict noise requirements"
            ]
          }
        ],
        "opener": "Hello! Do you prefer independent foreign cinema or mainstream Hollywood movies?",
        "icebreakers": [
          "Independent arthouse films!",
          "Exciting Hollywood blockbusters!",
          "Classic literature books!"
        ],
        "prompt": "Discuss Felix's appreciation for arthouse cinema, classic literature, and film score music."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Books, Films & Music 🎬",
        "title": "Nadia",
        "age": "26",
        "location": "📍 2.2 miles away • Podcast Studio",
        "verified": true,
        "avatar": "🎙️",
        "gradient": "linear-gradient(135deg, #0d9488 0%, #14b8a6 100%)",
        "visualDescription": "Podcast host wearing studio headphones speaking into a professional microphone.",
        "interests": [
          "#TrueCrimePodcasts",
          "#Audiobooks",
          "#IndieFolk"
        ],
        "anthem": "🎵 'Skinny Love' by Bon Iver",
        "film": "🎬 'Knives Out' (Rian Johnson)",
        "pages": [
          {
            "tag": "🎙️ True Crime Podcasts",
            "bio": "I'm Nadia, 26. I produce an investigative crime podcast. Unraveling real-life mysteries, psychological motives, and historical cold cases completely fascinates me.",
            "greenFlags": [
              "✓ Great storytelling voice",
              "✓ Sharp investigative curiosity",
              "✓ Engaging conversationalist"
            ],
            "redFlags": [
              "✗ Listens to crime stories while falling asleep",
              "✗ Suspects plot twists in real life",
              "✗ Talks about morbid cases"
            ]
          },
          {
            "tag": "🎧 Audiobook Enthusiast",
            "bio": "I consume audiobooks during daily commutes and long walks. Listening to professional voice actors narrate gripping biographies turns mundane travel into immersive journeys.",
            "greenFlags": [
              "✓ Always learning new facts",
              "✓ Efficient multitasking reader",
              "✓ Recommends great audiobooks"
            ],
            "redFlags": [
              "✗ Wears earbuds constantly",
              "✗ Misses people calling her name",
              "✗ Talks back to audiobook narrators"
            ]
          },
          {
            "tag": "🪕 Indie Folk Music",
            "bio": "Acoustic indie folk with poetic lyrics and harmonious vocals is my comfort genre. Attending intimate acoustic house shows is my favorite weekend outing.",
            "greenFlags": [
              "✓ Appreciates meaningful lyrics",
              "✓ Supports independent artists",
              "✓ Cozy musical taste"
            ],
            "redFlags": [
              "✗ Songs are sometimes melancholic",
              "✗ Dislikes electronic dance music",
              "✗ Cries during acoustic ballads"
            ]
          }
        ],
        "opener": "Hi! Are you hooked on true crime podcasts or do you prefer fiction audiobooks?",
        "icebreakers": [
          "True crime mystery podcasts!",
          "Gripping fiction audiobooks!",
          "Acoustic indie folk music!"
        ],
        "prompt": "Explain Nadia's podcasting interests, audiobook consumption habits, and indie folk musical taste."
      }
    ],
    "daily_habits": [
      {
        "type": "profile",
        "level": "B1",
        "category": "Daily Routines & Habits 🌅",
        "title": "Sora",
        "age": "27",
        "location": "📍 1 mile away • Eco Village",
        "verified": true,
        "avatar": "🌱",
        "gradient": "linear-gradient(135deg, #16a34a 0%, #65a30d 100%)",
        "visualDescription": "Minimalist carrying reusable cotton totes and glass jars at a zero-waste neighborhood bulk shop.",
        "interests": [
          "#ZeroWasteLiving",
          "#Minimalism",
          "#SustainableHabits"
        ],
        "anthem": "🎵 'Big Yellow Taxi' by Joni Mitchell",
        "film": "🎬 'Minimalism: A Documentary' (Matt D'Avella)",
        "pages": [
          {
            "tag": "🌱 Zero-Waste Lifestyle",
            "bio": "I'm Sora, 27. I practice a zero-waste lifestyle by shopping with reusable cloth bags and glass jars at bulk stores. Small daily choices collectively make a massive environmental impact.",
            "greenFlags": [
              "✓ Highly eco-conscious lifestyle",
              "✓ Inspiring sustainable habits",
              "✓ Reduced household waste"
            ],
            "redFlags": [
              "✗ Inspects friends' recycling bins",
              "✗ Carries heavy glass jars everywhere",
              "✗ Rejects plastic packaging strictly"
            ]
          },
          {
            "tag": "🧹 Minimalist Home",
            "bio": "I follow minimalist principles in my home environment. Owning fewer possessions reduces mental clutter and allows me to focus on meaningful experiences rather than material things.",
            "greenFlags": [
              "✓ Clean clutter-free living space",
              "✓ Values experiences over stuff",
              "✓ Mindful consumer habits"
            ],
            "redFlags": [
              "✗ Apartment feels almost empty",
              "✗ Donates items too quickly",
              "✗ Dislikes receiving physical gifts"
            ]
          },
          {
            "tag": "🚲 Cycling Commute",
            "bio": "I commute to work by bicycle year-round regardless of weather. Pedaling through urban streets keeps me physically fit while eliminating my transportation carbon footprint.",
            "greenFlags": [
              "✓ Zero-emission transportation",
              "✓ Great physical endurance",
              "✓ Outdoor daily activity"
            ],
            "redFlags": [
              "✗ Arrives sweaty from cycling",
              "✗ Bicycle parked in living room",
              "✗ Complains about car drivers"
            ]
          }
        ],
        "opener": "Hello! Have you ever attempted a zero-waste challenge or embraced minimalist living?",
        "icebreakers": [
          "Minimalist living appeal!",
          "Zero-waste daily habits!",
          "Cycling commute year-round!"
        ],
        "prompt": "Discuss Sora's commitment to zero-waste habits, minimalism, and eco-friendly commuting."
      },
      {
        "type": "profile",
        "level": "B1",
        "category": "Daily Routines & Habits 🌅",
        "title": "Marcus",
        "age": "29",
        "location": "📍 1.8 miles away • Fitness Quarter",
        "verified": true,
        "avatar": "🏃‍♂️",
        "gradient": "linear-gradient(135deg, #ea580c 0%, #dc2626 100%)",
        "visualDescription": "Marathon runner lacing up running shoes at dawn on a misty riverfront track.",
        "interests": [
          "#MarathonTraining",
          "#ColdShowers",
          "#HabitTracking"
        ],
        "anthem": "🎵 'Eye of the Tiger' by Survivor",
        "film": "🎬 'Precant' (Steve Prefontaine bio)",
        "pages": [
          {
            "tag": "🏃‍♂️ Marathon Discipline",
            "bio": "I'm Marcus, 29. I train for long-distance marathons by running 15 kilometers every morning at 5:30 AM. Developing physical endurance builds mental resilience for life's challenges.",
            "greenFlags": [
              "✓ Extraordinary self-discipline",
              "✓ High physical fitness",
              "✓ Determined goal-oriented mindset"
            ],
            "redFlags": [
              "✗ Wakes up at 5 AM every single day",
              "✗ Talks constantly about marathon times",
              "✗ In bed asleep by 9:00 PM"
            ]
          },
          {
            "tag": "🧊 Cold Shower Habit",
            "bio": "I take a 2-minute ice-cold shower every morning after running. It boosts circulation, sharpens focus, and instantly dispels morning sluggishness.",
            "greenFlags": [
              "✓ High mental alertness",
              "✓ Health-boosting discipline",
              "✓ Ready for work early"
            ],
            "redFlags": [
              "✗ Screams under cold water",
              "✗ Tries to convince everyone to try cold showers",
              "✗ Strict wellness rituals"
            ]
          },
          {
            "tag": "📊 Habit Tracking App",
            "bio": "I log my daily water intake, sleep hours, and running mileage in a habit tracking app. Seeing unbroken consistency streaks motivates me to push further.",
            "greenFlags": [
              "✓ Data-driven goal progress",
              "✓ Consistent daily habits",
              "✓ Well-organized routine"
            ],
            "redFlags": [
              "✗ Obsessed with app streaks",
              "✗ Disrupted if a streak breaks",
              "✗ Measures everything in numbers"
            ]
          }
        ],
        "opener": "Hi! Do you thrive on strict disciplined routines or prefer a flexible, spontaneous day?",
        "icebreakers": [
          "Strict disciplined routines!",
          "Flexible spontaneous days!",
          "Morning marathon running!"
        ],
        "prompt": "Analyze Marcus's marathon training discipline, cold shower habit, and habit tracking."
      }
    ]
  },
  "B2": {
    "appearance": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Physical Appearance 👁️",
        "title": "Victoria",
        "age": "29",
        "location": "📍 1 mile away • Metropolitan Plaza",
        "verified": true,
        "avatar": "👩‍🦳",
        "gradient": "linear-gradient(135deg, #0284c7 0%, #475569 100%)",
        "visualDescription": "Statuesque 5'10\" woman with a razor-sharp platinum blonde bob, piercing ice-blue eyes, high cheekbones, wearing an asymmetrical blazer.",
        "interests": [
          "#HighFashionStyle",
          "#SharpHairstyle",
          "#ConfidentLook"
        ],
        "anthem": "🎵 'Vogue' by Madonna",
        "film": "🎬 'The Grand Budapest Hotel' (Wes Anderson)",
        "pages": [
          {
            "tag": "🖼️ High Fashion",
            "bio": "I'm Victoria, 29. I'm 5'10\" with ice-blue eyes and a short platinum blonde bob. I love clean, simple high-fashion outfits that look modern and stylish.",
            "greenFlags": [
              "✓ Confident personal style",
              "✓ Great posture and presence",
              "✓ Neat, stylish haircut"
            ],
            "redFlags": [
              "✗ Looks intimidating at first",
              "✗ High-maintenance hair bleaching",
              "✗ Strict outfit expectations"
            ]
          },
          {
            "tag": "⚡ Fashion & Work",
            "bio": "Working as a luxury brand coordinator requires impeccable presentation. I curate structured monochromatic wardrobes that project authority and elegance in executive boardrooms.",
            "greenFlags": [
              "✓ Impeccable professional presence",
              "✓ Sharp eye for design quality",
              "✓ Highly organized wardrobe"
            ],
            "redFlags": [
              "✗ Refuses casual sweatpants in public",
              "✗ Spends heavily on designer tailoring",
              "✗ Expects polished attire from dates"
            ]
          },
          {
            "tag": "💭 Style Philosophy",
            "bio": "True style isn't about chasing transient trends, but rather discovering timeless silhouettes that reflect inner discipline and refined artistic taste.",
            "greenFlags": [
              "✓ Deep aesthetic philosophy",
              "✓ Rejects fast-fashion waste",
              "✓ Articulate cultural knowledge"
            ],
            "redFlags": [
              "✗ Can come across as overly critical",
              "✗ Uncompromising aesthetic standards",
              "✗ Dismissive of casual wear"
            ]
          }
        ],
        "opener": "Hi! How much does someone's outfit influence your first impression of them?",
        "icebreakers": [
          "First impressions matter!",
          "Style isn't everything!",
          "I love simple, clean fashion!"
        ],
        "prompt": "Talk about Victoria's style and how clothing choices shape first impressions."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Physical Appearance 👁️",
        "title": "Gabriel",
        "age": "34",
        "location": "📍 2 miles away • Old Town District",
        "verified": true,
        "avatar": "👨‍🦱",
        "gradient": "linear-gradient(135deg, #334155 0%, #64748b 100%)",
        "visualDescription": "Charismatic 34-year-old man with distinguished silver-templed dark hair and captivating dimples, wearing a tailored charcoal wool coat.",
        "interests": [
          "#SilverHair",
          "#ClassicStyle",
          "#CharismaticLook"
        ],
        "anthem": "🎵 'Feeling Good' by Nina Simone",
        "film": "🎬 'La Grande Bellezza' (Paolo Sorrentino)",
        "pages": [
          {
            "tag": "🖼️ Sartorial Style",
            "bio": "I'm Gabriel, 34. I'm 6'0\" tall with silver-templed dark hair, hazel eyes, and a friendly smile. I prefer classic, tailored coats and timeless European style over fast fashion.",
            "greenFlags": [
              "✓ Elegant and mature style",
              "✓ Warm, welcoming smile",
              "✓ Classic fashion taste"
            ],
            "redFlags": [
              "✗ Too picky about vintage watches",
              "✗ Never wears sports clothes",
              "✗ Takes a long time styling silver hair"
            ]
          },
          {
            "tag": "⚡ Daily Routine",
            "bio": "I begin mornings with espresso while reading international architectural journals. My wardrobe consists of handmade wool coats, polished leather loafers, and vintage mechanical watches.",
            "greenFlags": [
              "✓ Cultured daily habits",
              "✓ Appreciates vintage craftsmanship",
              "✓ Sophisticated conversationalist"
            ],
            "redFlags": [
              "✗ Takes 20 minutes inspecting vintage watches",
              "✗ Refuses to wear athletic hoodies",
              "✗ Fastidious about shoe care"
            ]
          }
        ],
        "opener": "Hello! Do you prefer keeping up with modern trends or sticking to classic clothing styles?",
        "icebreakers": [
          "I follow current trends!",
          "Classic style is timeless!",
          "I mix modern and vintage!"
        ],
        "prompt": "Discuss how Gabriel's classic clothing and mature look give him a charismatic appearance."
      }
    ],
    "professions": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Professions 💼",
        "title": "Dr. Aris",
        "age": "33",
        "location": "📍 1.2 miles away • Innovation Campus",
        "verified": true,
        "avatar": "🧠",
        "gradient": "linear-gradient(135deg, #4338ca 0%, #6366f1 100%)",
        "visualDescription": "Researcher auditing algorithmic bias charts on transparent displays in a university research laboratory.",
        "interests": [
          "#TechEthics",
          "#FairTechnology",
          "#DigitalPrivacy"
        ],
        "anthem": "🎵 'Computer World' by Kraftwerk",
        "film": "🎬 'Ex Machina' (Alex Garland)",
        "pages": [
          {
            "tag": "🖼️ AI Ethics Research",
            "bio": "I'm Dr. Aris, a technology ethics researcher. I test artificial intelligence programs to prevent bias, protect user privacy, and make sure new tech is fair. I also help write clear policy guidelines for tech companies.",
            "greenFlags": [
              "✓ Working on important modern tech issues",
              "✓ Helps protect user privacy and fairness",
              "✓ Shapes future tech guidelines"
            ],
            "redFlags": [
              "✗ Complicated rules with no easy answers",
              "✗ Tech companies resist new regulations",
              "✗ Technology changes faster than policy"
            ]
          },
          {
            "tag": "⚡ Research & Debates",
            "bio": "My daily schedule involves reviewing algorithmic fairness metrics, speaking at academic conferences, and consulting with legal committees on digital rights and automated decision-making.",
            "greenFlags": [
              "✓ High intellectual rigor",
              "✓ Articulate public speaker",
              "✓ Committed to digital justice"
            ],
            "redFlags": [
              "✗ Over-analyzes everyday decisions",
              "✗ Uses dense academic jargon",
              "✗ Working late analyzing dataset bias"
            ]
          },
          {
            "tag": "💭 Vision for Tech",
            "bio": "Artificial intelligence should serve humanity with accountability and transparency. Technology without ethical boundaries risks compounding societal inequality.",
            "greenFlags": [
              "✓ Principled ethical conviction",
              "✓ Visionary approach to technology",
              "✓ Protects human dignity"
            ],
            "redFlags": [
              "✗ Cynical about Silicon Valley hype",
              "✗ Debates moral dilemmas at dinner",
              "✗ Intense philosophical focus"
            ]
          }
        ],
        "opener": "Hi! Should governments regulate artificial intelligence, or should tech companies decide for themselves?",
        "icebreakers": [
          "Governments should step in!",
          "Tech companies need freedom!",
          "We need balanced rules!"
        ],
        "prompt": "Discuss the pros and cons of setting clear ethical rules for artificial intelligence."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Professions 💼",
        "title": "Soren",
        "age": "32",
        "location": "📍 0.9 miles away • Heritage Bureau",
        "verified": true,
        "avatar": "🏛️",
        "gradient": "linear-gradient(135deg, #854d0e 0%, #a16207 100%)",
        "visualDescription": "Architect examining intricate stonework details of a historic cathedral during a restoration project.",
        "interests": [
          "#BuildingRestoration",
          "#CityPlanning",
          "#OldAndNew"
        ],
        "anthem": "🎵 'Symphony No. 5' by Ludwig van Beethoven",
        "film": "🎬 'Metropolis' (Fritz Lang)",
        "pages": [
          {
            "tag": "🖼️ Architectural Conservation",
            "bio": "I'm Soren, an architect who specializes in restoring old buildings. I renovate historical structures so they can be used as modern apartments or offices without losing their original character.",
            "greenFlags": [
              "✓ Protects historical architecture for the community",
              "✓ Gives old buildings new life",
              "✓ Combines classic style with eco-friendly features"
            ],
            "redFlags": [
              "✗ Slow approval processes with local councils",
              "✗ Unexpected repair costs",
              "✗ Strict budget limits"
            ]
          },
          {
            "tag": "⚡ Restoration Projects",
            "bio": "I spend half my week on dusty construction sites inspecting century-old masonry and timber, and the other half drafting sustainable retrofitting plans using modern insulation.",
            "greenFlags": [
              "✓ Hands-on site experience",
              "✓ Blends heritage with sustainability",
              "✓ Meticulous structural planning"
            ],
            "redFlags": [
              "✗ Comes home with brick dust on shoes",
              "✗ Frustrated by bureaucratic delays",
              "✗ Obsesses over historic tile patterns"
            ]
          }
        ],
        "opener": "Hello! How can cities preserve their historical buildings while building new housing and transport?",
        "icebreakers": [
          "Save the old buildings!",
          "Build modern architecture!",
          "Find a good balance!"
        ],
        "prompt": "Talk about the challenges Soren faces when trying to renovate historic buildings for modern use."
      }
    ],
    "properties": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Properties 🏠",
        "title": "Xavier",
        "age": "35",
        "location": "📍 0.5 miles away • Pinnacle Tower",
        "verified": true,
        "avatar": "🏙️",
        "gradient": "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)",
        "visualDescription": "Luxury penthouse terrace featuring a private glass-edge infinity pool, 360-degree skyline panoramas, and smart automation.",
        "interests": [
          "#SkylineView",
          "#InfinityPool",
          "#SmartHome"
        ],
        "anthem": "🎵 'Skyfall' by Adele",
        "film": "🎬 'Casino Royale' (Martin Campbell)",
        "pages": [
          {
            "tag": "🖼️ Penthouse Sky Villa",
            "bio": "I'm Xavier. My penthouse apartment sits on the top floor of Pinnacle Tower. It features a private glass-edge pool, 360-degree city views, and smart home controls for lights and security.",
            "greenFlags": [
              "✓ Amazing panoramic views of the city",
              "✓ Great privacy and top security",
              "✓ Smart home technology"
            ],
            "redFlags": [
              "✗ Very expensive maintenance fees",
              "✗ Slightly sways during heavy windstorms",
              "✗ Feels separated from street life"
            ]
          },
          {
            "tag": "⚡ High-Altitude Living",
            "bio": "Living 50 floors above the metropolis offers breathtaking sunsets and serene quietness above urban traffic. Automated climate controls and minimalist floor-to-ceiling glass define the space.",
            "greenFlags": [
              "✓ Unmatched skyline views",
              "✓ Quiet elevated retreat",
              "✓ Cutting-edge automation"
            ],
            "redFlags": [
              "✗ High wind noise during winter storms",
              "✗ Elevator delays during rush hour",
              "✗ High energy consumption"
            ]
          },
          {
            "tag": "💭 Modern Luxury",
            "bio": "I view architecture as elevated sanctuary. Having a peaceful refuge high above the bustle allows me to recharge, host intimate gatherings, and maintain strategic focus.",
            "greenFlags": [
              "✓ Refined hosting environment",
              "✓ Peaceful personal sanctuary",
              "✓ High aesthetic appreciation"
            ],
            "redFlags": [
              "✗ Detached from neighborhood street vibe",
              "✗ Intimidating high-tech interfaces",
              "✗ High financial maintenance demands"
            ]
          }
        ],
        "opener": "Welcome! Would you enjoy living high above the city in a luxury penthouse?",
        "icebreakers": [
          "The view looks amazing!",
          "I prefer living on the ground!",
          "Smart homes are awesome!"
        ],
        "prompt": "Describe what it would be like to live in Xavier's penthouse and discuss the pros and cons."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Properties 🏠",
        "title": "Eleanor",
        "age": "34",
        "location": "📍 2 miles away • Eco Valley",
        "verified": true,
        "avatar": "🌿",
        "gradient": "linear-gradient(135deg, #047857 0%, #10b981 100%)",
        "visualDescription": "Modern eco-sustainable villa with solar roof glass panels, rainwater collection system, and living plant walls.",
        "interests": [
          "#EcoVilla",
          "#SolarPowered",
          "#SustainableDesign"
        ],
        "anthem": "🎵 'Mercy Mercy Me' by Marvin Gaye",
        "film": "🎬 'Captain Fantastic' (Matt Ross)",
        "pages": [
          {
            "tag": "🖼️ Sustainable Villa",
            "bio": "I'm Eleanor. My architectural passion culminated in designing an off-grid sustainable villa powered by photovoltaic solar arrays, geothermal heating, and passive rainwater filtration.",
            "greenFlags": [
              "✓ Zero-carbon footprint living",
              "✓ Cutting-edge green technology",
              "✓ Healthy indoor air quality"
            ],
            "redFlags": [
              "✗ High initial construction investment",
              "✗ Energy rationing during winter cloudy weeks",
              "✗ Complex maintenance systems"
            ]
          },
          {
            "tag": "⚡ Green Innovation",
            "bio": "Triple-glazed glass walls maximize natural illumination while living moss facades regulate interior temperature and humidity naturally without air conditioning.",
            "greenFlags": [
              "✓ Abundant natural sunlight",
              "✓ Natural climate regulation",
              "✓ Inspiring eco-architecture"
            ],
            "redFlags": [
              "✗ Monitoring battery storage levels",
              "✗ Strict waste sorting protocols",
              "✗ Remote rural location"
            ]
          }
        ],
        "opener": "Welcome! Would you prefer living in a high-tech eco-villa or a conventional city apartment?",
        "icebreakers": [
          "Off-grid eco-villa!",
          "Conventional city apartment!",
          "Solar power technology!"
        ],
        "prompt": "Evaluate the ecological innovations and practical challenges of Eleanor's sustainable villa."
      }
    ],
    "character": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Character Traits 🧠",
        "title": "Helena",
        "age": "32",
        "location": "📍 1.5 miles away • Enterprise Square",
        "verified": true,
        "avatar": "🦁",
        "gradient": "linear-gradient(135deg, #b91c1c 0%, #dc2626 100%)",
        "visualDescription": "Determined female executive standing on a terrace overlooking the city skyline.",
        "interests": [
          "#TeamLeadership",
          "#Resilience",
          "#GoalOriented"
        ],
        "anthem": "🎵 'Unstoppable' by Sia",
        "film": "🎬 'Iron Lady' (Phyllida Lloyd)",
        "pages": [
          {
            "tag": "🖼️ Executive Leadership",
            "bio": "I'm Helena, 32. I'm a business manager focused on long-term strategy, staying calm under pressure, and supporting my team. I treat mistakes as useful lessons to help us improve next time.",
            "greenFlags": [
              "✓ Inspires high motivation and ambition",
              "✓ Stays calm and focused during crises",
              "✓ Clear and decisive leader"
            ],
            "redFlags": [
              "✗ High standards can feel intimidating",
              "✗ Works late hours regularly",
              "✗ Impatient with slow progress"
            ]
          },
          {
            "tag": "⚡ Executive Operations",
            "bio": "I lead cross-functional teams through complex corporate turnarounds. My management style combines empathetic listening with rigorous performance accountability.",
            "greenFlags": [
              "✓ Empathetic yet accountable",
              "✓ Strong crisis navigation",
              "✓ Mentors rising talent"
            ],
            "redFlags": [
              "✗ Expects prompt responses to emails",
              "✗ High-intensity calendar schedule",
              "✗ Hard time switching off work mode"
            ]
          }
        ],
        "opener": "Hello! What qualities do you think make someone a truly effective leader?",
        "icebreakers": [
          "Empathy and good communication!",
          "Staying calm under pressure!",
          "Setting a strong example!"
        ],
        "prompt": "Discuss Helena's leadership style and explain what traits make a good team leader."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Character Traits 🧠",
        "title": "Julian",
        "age": "35",
        "location": "📍 0.8 miles away • Diplomatic Quarter",
        "verified": true,
        "avatar": "⚖️",
        "gradient": "linear-gradient(135deg, #1e293b 0%, #475569 100%)",
        "visualDescription": "Strategic diplomat reflecting calmly beside a glass boardroom table before international negotiations.",
        "interests": [
          "#StrategicDiplomacy",
          "#CrisisManagement",
          "#Negotiation"
        ],
        "anthem": "🎵 'Wind of Change' by Scorpions",
        "film": "🎬 'Bridge of Spies' (Steven Spielberg)",
        "pages": [
          {
            "tag": "🖼️ Strategic Diplomacy",
            "bio": "I'm Julian, 35. As an international mediator, my character is characterized by stoic emotional composure, analytical tact, and an ability to reconcile conflicting stakeholder interests during high-stakes crises.",
            "greenFlags": [
              "✓ Unshakable emotional composure",
              "✓ Strategic problem solver",
              "✓ Respectful cross-cultural communicator"
            ],
            "redFlags": [
              "✗ Keeps emotional guard up",
              "✗ Calculates conversational outcomes",
              "✗ Intense schedule demands"
            ]
          },
          {
            "tag": "⚡ High-Stakes Mediation",
            "bio": "Navigating complex multilateral disputes requires active neutrality, cultural nuance, and de-escalating tension through precise linguistic phrasing.",
            "greenFlags": [
              "✓ Master of linguistic precision",
              "✓ Calm under intense pressure",
              "✓ Trustworthy impartial advisor"
            ],
            "redFlags": [
              "✗ Analyzes motives endlessly",
              "✗ Reluctant to make impulsive choices",
              "✗ Work travels interrupt personal life"
            ]
          }
        ],
        "opener": "Greetings! What leadership attributes are essential when mediating high-stakes conflict?",
        "icebreakers": [
          "Stoic emotional composure!",
          "Active neutrality and listening!",
          "Strategic linguistic precision!"
        ],
        "prompt": "Analyze Julian's diplomatic approach and emotional composure during high-stakes negotiations."
      }
    ],
    "hobbies": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Hobbies 🎨",
        "title": "Felix",
        "age": "30",
        "location": "📍 Darkroom & City Streets",
        "verified": true,
        "avatar": "📷",
        "gradient": "linear-gradient(135deg, #1f2937 0%, #374151 100%)",
        "visualDescription": "Vintage 35mm film camera resting on a darkroom wooden bench next to developed negatives.",
        "interests": [
          "#FilmPhotography",
          "#DarkroomProcess",
          "#AnalogPhotos"
        ],
        "anthem": "🎵 'Kodachrome' by Paul Simon",
        "film": "🎬 'Blow-Up' (Michelangelo Antonioni)",
        "pages": [
          {
            "tag": "🖼️ Analog Photography",
            "bio": "I'm Felix. I shoot analog photos with a 35mm film camera. You only get 36 shots per roll and cannot check the photo immediately, so you have to be patient and focus on lighting and composition before developing negatives in the darkroom.",
            "greenFlags": [
              "✓ Encourages focus and patience",
              "✓ Real physical photo prints to keep",
              "✓ Beautiful film texture and colors"
            ],
            "redFlags": [
              "✗ Expensive film rolls and chemicals",
              "✗ Accidental light leaks can ruin photos",
              "✗ No instant preview"
            ]
          },
          {
            "tag": "⚡ Darkroom Craft",
            "bio": "I convert my bathroom into a makeshift darkroom on Sunday afternoons. Mixing developer chemicals and watching silver-halide images emerge on photographic paper under red light is pure magic.",
            "greenFlags": [
              "✓ Master of classic analog darkroom chemistry",
              "✓ Tangible physical craftsmanship",
              "✓ Deep artistic dedication"
            ],
            "redFlags": [
              "✗ Chemical smell during darkroom sessions",
              "✗ Bathroom unavailable for hours",
              "✗ Dislikes digital smartphone snapshots"
            ]
          },
          {
            "tag": "💭 Visual Philosophy",
            "bio": "In an era of disposable digital media where people take thousands of forgotten smartphone photos, analog film forces you to slow down, respect light, and cherish single moments.",
            "greenFlags": [
              "✓ Mindful approach to photography",
              "✓ Artistic depth and intentionality",
              "✓ Enjoys tangible physical memories"
            ],
            "redFlags": [
              "✗ Preachy about digital camera shortcuts",
              "✗ Refuses to take quick selfie photos",
              "✗ Hoards rolls of expired film"
            ]
          }
        ],
        "opener": "Hi! Do you prefer taking fast photos on a smartphone or taking time with traditional film?",
        "icebreakers": [
          "Film has so much charm!",
          "Smartphones are way faster!",
          "I like a bit of both!"
        ],
        "prompt": "Compare smartphone photography with traditional film photography based on Felix's hobby."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Hobbies 🎨",
        "title": "Maya",
        "age": "28",
        "location": "📍 Philharmonic Hall",
        "verified": true,
        "avatar": "🎻",
        "gradient": "linear-gradient(135deg, #701a75 0%, #a21caf 100%)",
        "visualDescription": "Classical violinist practicing a concert passage with focused expression inside a wooden acoustics hall.",
        "interests": [
          "#ClassicalViolin",
          "#ChamberMusic",
          "#OrchestralPerformance"
        ],
        "anthem": "🎵 'The Four Seasons (Summer)' by Antonio Vivaldi",
        "film": "🎬 'The Red Violin' (François Girard)",
        "pages": [
          {
            "tag": "🖼️ Violinist Craft",
            "bio": "I'm Maya. Playing first violin in a chamber quartet is my life's passion. Interpreting complex polyphonic scores demands rigorous acoustic discipline, expressive phrasing, and seamless ensemble synergy.",
            "greenFlags": [
              "✓ Virtuosic musical dedication",
              "✓ Deep artistic expressiveness",
              "✓ Harmonic ensemble teamwork"
            ],
            "redFlags": [
              "✗ Practises 4 hours daily",
              "✗ Strict noise environment needed",
              "✗ Calloused fingertips and neck marks"
            ]
          },
          {
            "tag": "⚡ Concert Rehearsals",
            "bio": "Preparing for symphony hall performances involves dissecting subtle tempo rubatos and bowing dynamics until the collective sound resonates with emotional depth.",
            "greenFlags": [
              "✓ Meticulous attention to acoustic detail",
              "✓ Inspiring musical mastery",
              "✓ Passionate performance energy"
            ],
            "redFlags": [
              "✗ Rehearsal anxiety before concerts",
              "✗ Highly sensitive to pitch deviations",
              "✗ Expensive instrument care"
            ]
          }
        ],
        "opener": "Hello! Do you find classical chamber music emotionally moving or overly technical?",
        "icebreakers": [
          "Profoundly moving acoustic art!",
          "Technically impressive!",
          "Violin concertos are beautiful!"
        ],
        "prompt": "Examine the artistic discipline and ensemble synergy required for Maya's violin performances."
      }
    ],
    "nationalities": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Nationalities & Cultures 🌍",
        "title": "Camille",
        "age": "29",
        "location": "📍 Paris, France • 🇫🇷 French",
        "verified": true,
        "avatar": "🇫🇷",
        "gradient": "linear-gradient(135deg, #1d4ed8 0%, #ef4444 100%)",
        "visualDescription": "French woman in a trench coat seated at a Parisian pavement café with wine.",
        "interests": [
          "#FrenchCulture",
          "#ParisianLife",
          "#GoodFood"
        ],
        "anthem": "🎵 'La Vie en rose' by Édith Piaf",
        "film": "🎬 'Amélie' (Jean-Pierre Jeunet)",
        "pages": [
          {
            "tag": "🖼️ Parisian Lifestyle",
            "bio": "Bonjour! I'm Camille from Paris. In France, we value enjoying life through good food, art, and long discussions over dinner. I love exploring Parisian cafes, fresh pastries, and local markets.",
            "greenFlags": [
              "✓ Great knowledge of food and cheese",
              "✓ Loves lively, deep conversations",
              "✓ Elegant daily lifestyle"
            ],
            "redFlags": [
              "✗ Very particular about fresh bread",
              "✗ Sighs if the coffee isn't hot enough",
              "✗ Always compares food to French cuisine"
            ]
          },
          {
            "tag": "⚡ Daily Parisian Rhythms",
            "bio": "My morning starts with a fresh crispy baguette from the neighborhood boulangerie. On weekends, I browse contemporary art exhibits at the Palais de Tokyo before enjoying wine with friends.",
            "greenFlags": [
              "✓ Connoisseur of artisanal bakeries",
              "✓ Passionate art commentator",
              "✓ Cultured weekend itinerary"
            ],
            "redFlags": [
              "✗ Refuses day-old bread under any condition",
              "✗ Debates art theory for hours",
              "✗ Critical of non-espresso coffee"
            ]
          }
        ],
        "opener": "Bonjour! How much do food and cultural traditions influence your daily life?",
        "icebreakers": [
          "Food is a huge part of culture!",
          "French pastry is amazing!",
          "I love good dinner conversations!"
        ],
        "prompt": "Discuss French lifestyle and food traditions as described in Camille's profile."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Nationalities & Cultures 🌍",
        "title": "Leandro",
        "age": "30",
        "location": "📍 Rio de Janeiro, Brazil • 🇧🇷 Portuguese",
        "verified": true,
        "avatar": "🇧🇷",
        "gradient": "linear-gradient(135deg, #16a34a 0%, #eab308 100%)",
        "visualDescription": "Brazilian man playing beach volleyball on Ipanema Beach in Rio de Janeiro.",
        "interests": [
          "#BrazilianVibes",
          "#RioDeJaneiro",
          "#BossaNova"
        ],
        "anthem": "🎵 'The Girl from Ipanema' by Stan Getz & Astrud Gilberto",
        "film": "🎬 'City of God' (Fernando Meirelles)",
        "pages": [
          {
            "tag": "🖼️ Rio Spirit",
            "bio": "Olá! I'm Leandro from Rio de Janeiro. Brazil is a vibrant country rich in music, beach sports, and outdoor gatherings. From bossa nova melodies to lively weekend barbecues, community and music are at the heart of daily life.",
            "greenFlags": [
              "✓ Warm and welcoming personality",
              "✓ Plays acoustic guitar and bossa nova",
              "✓ Cooks delicious traditional feijoada"
            ],
            "redFlags": [
              "✗ Flexible idea of arrival times",
              "✗ Very loud social gatherings",
              "✗ Plays music late into the night"
            ]
          },
          {
            "tag": "⚡ Carioca Life",
            "bio": "I spend weekends playing footvolley on Ipanema beach, strumming samba chords on my nylon-string guitar, and preparing slow-cooked feijoada for big gatherings of family and friends.",
            "greenFlags": [
              "✓ Athletic beach lifestyle",
              "✓ Musical bossa nova talent",
              "✓ Master of slow-cooked Brazilian feast"
            ],
            "redFlags": [
              "✗ Sand everywhere in the apartment",
              "✗ Operates on relaxed 'Carioca time'",
              "✗ Jam sessions extend past 1 AM"
            ]
          },
          {
            "tag": "💭 Musical Connections",
            "bio": "Music in Brazil isn't just entertainment; it's our collective language for processing joy and saudade. Gathering people around rhythm and good food creates lifelong bonds.",
            "greenFlags": [
              "✓ Expressive emotional depth",
              "✓ Unifies people through music",
              "✓ Welcoming heart"
            ],
            "redFlags": [
              "✗ Cannot stand quiet solitary silence for long",
              "✗ Spontaneous samba circles anywhere",
              "✗ Invites entire neighborhood to dinner"
            ]
          }
        ],
        "opener": "Olá! What comes to mind when you think about Brazilian music and culture?",
        "icebreakers": [
          "Bossa nova is so relaxing!",
          "Rio de Janeiro looks incredible!",
          "I love lively music and food!"
        ],
        "prompt": "Talk about how music and social gatherings shape daily life in Brazil according to Leandro."
      }
    ],
    "food": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Food & Cuisine 🍜",
        "title": "Antoine",
        "age": "31",
        "location": "📍 0.8 miles away • Michelin District",
        "verified": true,
        "avatar": "🍷",
        "gradient": "linear-gradient(135deg, #475569 0%, #1e1b4b 100%)",
        "visualDescription": "Sophisticated sommelier swirling a glass of Burgundy wine in an elegant dimly-lit fine dining restaurant.",
        "interests": [
          "#Gastronomy",
          "#SommelierSelection",
          "#FineDining"
        ],
        "anthem": "🎵 'Autumn Leaves' by Cannonball Adderley",
        "film": "🎬 'Burnt' (John Wells)",
        "pages": [
          {
            "tag": "🍷 Gastronomic Pairings",
            "bio": "I'm Antoine, 31, a certified sommelier and culinary critic. I view gastronomy as an intricate art form where delicate flavor profiles, wine pairings, and ambient aesthetics harmonize to evoke emotion.",
            "greenFlags": [
              "✓ Exquisite wine pairings",
              "✓ Deep culinary knowledge",
              "✓ Sophisticated dining taste"
            ],
            "redFlags": [
              "✗ Pretentious about vintage years",
              "✗ Criticizes casual restaurant menus",
              "✗ Expensive tasting menu habits"
            ]
          },
          {
            "tag": "🔪 Precision Techniques",
            "bio": "In my spare time, I experiment with molecular gastronomy, sous-vide precision temperature cooking, and artisanal emulsifications. Cooking requires meticulous discipline and respect for raw ingredients.",
            "greenFlags": [
              "✓ Professional culinary mastery",
              "✓ Uncompromising quality standards",
              "✓ Fascinating food science knowledge"
            ],
            "redFlags": [
              "✗ Spends 3 hours plating a single dish",
              "✗ Expects perfection in every meal",
              "✗ Uses complex kitchen gadgets"
            ]
          },
          {
            "tag": "🧀 Artisanal Heritage",
            "bio": "I frequent small regional producers in France and Italy to source unpasteurized raw-milk cheeses, truffle oils, and rare heritage grains that preserve authentic culinary heritage.",
            "greenFlags": [
              "✓ Supports small artisanal producers",
              "✓ Rich cultural appreciation",
              "✓ Impeccable palate"
            ],
            "redFlags": [
              "✗ Refuses commercial grocery brands",
              "✗ Snobbish about cheese origins",
              "✗ Hard to please at dinner parties"
            ]
          }
        ],
        "opener": "Greetings! Do you evaluate meals based on nostalgic comfort or refined gastronomic technique?",
        "icebreakers": [
          "Nostalgic comfort food!",
          "Refined gastronomic artistry!",
          "A perfect wine pairing!"
        ],
        "prompt": "Evaluate Antoine's perspective on fine dining, wine pairing, and molecular gastronomy."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Food & Cuisine 🍜",
        "title": "Siddharth",
        "age": "30",
        "location": "📍 2 miles away • Culinary Hub",
        "verified": true,
        "avatar": "🍛",
        "gradient": "linear-gradient(135deg, #b45309 0%, #78350f 100%)",
        "visualDescription": "Innovative fusion chef tossing aromatic spices in a flaming wok, combining Indian and pan-Asian flavors.",
        "interests": [
          "#ModernFusion",
          "#SpiceChemistry",
          "#CulinaryInnovation"
        ],
        "anthem": "🎵 'Jai Ho' by A.R. Rahman",
        "film": "🎬 'The Lunchbox' (Ritesh Batra)",
        "pages": [
          {
            "tag": "🍛 Modern Culinary Fusion",
            "bio": "I'm Siddharth, 30. As a modern fusion chef, I synthesize traditional Indian spice layering with contemporary Japanese and Nordic culinary methodologies, reimagining classic recipes for modern palates.",
            "greenFlags": [
              "✓ Boundary-pushing flavor concepts",
              "✓ Deep mastery of spice science",
              "✓ Visually stunning dish presentation"
            ],
            "redFlags": [
              "✗ Experimental dishes don't always work",
              "✗ Overcomplicates simple recipes",
              "✗ Uses hard-to-find ingredients"
            ]
          },
          {
            "tag": "🔥 Tandoor & Wok Mastery",
            "bio": "My culinary philosophy centers on smoke, acidity, and umami balance. Utilizing high-heat tandoor ovens alongside wok-searing allows me to achieve extraordinary textural contrasts.",
            "greenFlags": [
              "✓ Masterful heat control",
              "✓ Complex layered flavor profiles",
              "✓ Passionate food innovator"
            ],
            "redFlags": [
              "✗ Sets off kitchen smoke alarms",
              "✗ Leaves scorched pans to clean",
              "✗ Cooks with intense fiery chili"
            ]
          },
          {
            "tag": "📜 Culinary Heritage",
            "bio": "While I embrace modern innovation, my foundation relies on ancient spice-roasting techniques passed down through generations. Honoring tradition while breaking rules defines true creativity.",
            "greenFlags": [
              "✓ Deep respect for ancestral recipes",
              "✓ Creative culinary storytelling",
              "✓ Inspiring food philosophy"
            ],
            "redFlags": [
              "✗ Debates food authenticity for hours",
              "✗ Highly critical of fast-food chains",
              "✗ Rigid opinions on spice grinding"
            ]
          }
        ],
        "opener": "Hello! Do you prefer respecting traditional regional recipes or exploring bold culinary fusion?",
        "icebreakers": [
          "Bold fusion innovations!",
          "Strict traditional recipes!",
          "Complex spice chemistry!"
        ],
        "prompt": "Analyze Siddharth's fusion philosophy and how he balances tradition with culinary innovation."
      }
    ],
    "travel": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Travel & Destinations ✈️",
        "title": "Sebastian",
        "age": "32",
        "location": "📍 1 mile away • Maritime Marina",
        "verified": true,
        "avatar": "⛵",
        "gradient": "linear-gradient(135deg, #0f172a 0%, #0369a1 100%)",
        "visualDescription": "Experienced sailor steering a white sailboat through deep blue waters along a rugged Mediterranean coastline.",
        "interests": [
          "#SailingExpeditions",
          "#MaritimeNavigation",
          "#CoastalExploration"
        ],
        "anthem": "🎵 'Sailing' by Christopher Cross",
        "film": "🎬 'Master and Commander' (Peter Weir)",
        "pages": [
          {
            "tag": "⛵ Open Water Voyages",
            "bio": "I'm Sebastian, 32, a licensed skipper and oceanography enthusiast. Navigating open marine waters using wind currents and celestial navigation offers an unparalleled sense of elemental freedom.",
            "greenFlags": [
              "✓ Masterful maritime skills",
              "✓ Deep environmental awareness",
              "✓ Calm under high-pressure conditions"
            ],
            "redFlags": [
              "✗ Motion sickness risks for guests",
              "✗ Rigorous nautical maintenance",
              "✗ Long periods isolated at sea"
            ]
          },
          {
            "tag": "🌊 Coastal Archipelagoes",
            "bio": "I specialize in charting remote island archipelagoes in the Aegean and Adriatic seas, dropping anchor in secluded coves unreachable by overland transit.",
            "greenFlags": [
              "✓ Access to untouched locations",
              "✓ Exceptional marine geography knowledge",
              "✓ Unforgettable coastal journeys"
            ],
            "redFlags": [
              "✗ Unpredictable weather disruptions",
              "✗ Limited freshwater supplies on board",
              "✗ High cost of vessel chartering"
            ]
          },
          {
            "tag": "🌌 Night Watches",
            "bio": "Standing night watch at the helm beneath bioluminescent waters and open starry horizons cultivates a profound reverence for ocean ecosystems and self-reliance.",
            "greenFlags": [
              "✓ Profound respect for nature",
              "✓ Self-reliant problem solver",
              "✓ Captivating nautical storytelling"
            ],
            "redFlags": [
              "✗ Sleep deprivation during passages",
              "✗ Demands strict onboard discipline",
              "✗ Dislikes tourist resort towns"
            ]
          }
        ],
        "opener": "Greetings! Do you find open-ocean sailing exhilarating or intimidating?",
        "icebreakers": [
          "Exhilarating oceanic freedom!",
          "A bit intimidating!",
          "Exploring secluded coves!"
        ],
        "prompt": "Examine Sebastian's nautical expertise, open-water navigation, and ocean conservancy."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Travel & Destinations ✈️",
        "title": "Camille",
        "age": "30",
        "location": "📍 0.5 miles away • Heritage Quarter",
        "verified": true,
        "avatar": "🏛️",
        "gradient": "linear-gradient(135deg, #701a75 0%, #a21caf 100%)",
        "visualDescription": "Cultural anthropologist writing in a journal beside ancient ruins at an archaeological site.",
        "interests": [
          "#CulturalHeritage",
          "#ArchitecturalHistory",
          "#SustainableTourism"
        ],
        "anthem": "🎵 'Eleni' by Capercaillie",
        "film": "🎬 'The English Patient' (Anthony Minghella)",
        "pages": [
          {
            "tag": "🏛️ Cultural Preservation",
            "bio": "I'm Camille, 30. As an architectural historian, my travel philosophy prioritizes heritage preservation, sustainable eco-tourism, and critical engagement with historical narratives.",
            "greenFlags": [
              "✓ Deep historical erudition",
              "✓ Ethical sustainable travel ethos",
              "✓ Articulate cultural guide"
            ],
            "redFlags": [
              "✗ Lectures on historical dates",
              "✗ Overly analytical of tourist spots",
              "✗ Dislikes resort entertainment"
            ]
          },
          {
            "tag": "📜 Living Histories",
            "bio": "I document endangered architectural craft traditions across Central Asia and the Mediterranean, advocating for community-led tourism models that protect indigenous heritage.",
            "greenFlags": [
              "✓ Passionate heritage advocate",
              "✓ Supports local indigenous economies",
              "✓ Profound cross-cultural insight"
            ],
            "redFlags": [
              "✗ Spends hours examining masonry",
              "✗ Academic research tone",
              "✗ Rejects commercial travel guides"
            ]
          },
          {
            "tag": "🌿 Sustainable Eco-Travel",
            "bio": "Minimizing carbon footprints by utilizing electric overland transit and supporting locally owned eco-lodges ensures tourism benefits host communities directly.",
            "greenFlags": [
              "✓ Environmentally responsible traveler",
              "✓ Conscious consumer habits",
              "✓ Inspiring ethical perspective"
            ],
            "redFlags": [
              "✗ Strict carbon audit of travel plans",
              "✗ Critiques cruise ships harshly",
              "✗ Uncompromising ethical standards"
            ]
          }
        ],
        "opener": "Hello! How do you balance tourist enjoyment with ethical cultural preservation when traveling?",
        "icebreakers": [
          "Ethical heritage preservation!",
          "Relaxation and fun first!",
          "Supporting local host communities!"
        ],
        "prompt": "Evaluate Camille's views on sustainable tourism, historical preservation, and responsible travel."
      }
    ],
    "entertainment": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Books, Films & Music 🎬",
        "title": "Julian",
        "age": "31",
        "location": "📍 0.9 miles away • Symphony Hall",
        "verified": true,
        "avatar": "🎼",
        "gradient": "linear-gradient(135deg, #312e81 0%, #4338ca 100%)",
        "visualDescription": "Orchestral conductor studying a complex musical score with a baton in hand.",
        "interests": [
          "#SymphonicComposition",
          "#ComparativeLiterature",
          "#AuteurCinema"
        ],
        "anthem": "🎵 'Symphony No. 5' by Ludwig van Beethoven",
        "film": "🎬 'TÁR' (Todd Field)",
        "pages": [
          {
            "tag": "🎼 Symphonic Mastery",
            "bio": "I'm Julian, 31, an assistant conductor and musicologist. Analyzing polyphonic counterpoint and orchestrating complex harmonic movements requires rigorous intellectual precision and emotional sensitivity.",
            "greenFlags": [
              "✓ Extraordinary musical erudition",
              "✓ Exceptional acoustic discipline",
              "✓ Profound artistic passion"
            ],
            "redFlags": [
              "✗ Highly critical of minor pitch errors",
              "✗ Intense perfectionist tendencies",
              "✗ Intimidating academic knowledge"
            ]
          },
          {
            "tag": "📖 Comparative Literature",
            "bio": "I study comparative literature, exploring narrative structural parallels between 20th-century modernist novels and symphonic movement forms.",
            "greenFlags": [
              "✓ Deep interdisciplinary intellect",
              "✓ Rich cultural perspectives",
              "✓ Fascinating philosophical conversation"
            ],
            "redFlags": [
              "✗ Rejects superficial pop culture",
              "✗ Complex abstract explanations",
              "✗ High expectations of conversation"
            ]
          },
          {
            "tag": "🎬 Auteur Cinema",
            "bio": "I revere auteur filmmakers who utilize minimalist soundscapes and long unedited takes to examine psychological isolation and moral ambiguity.",
            "greenFlags": [
              "✓ Nuanced cinematic appreciation",
              "✓ Rejects formulaic commercial film",
              "✓ Eloquent critique skills"
            ],
            "redFlags": [
              "✗ Finds popular comedies tedious",
              "✗ Strict silence rules during viewing",
              "✗ Opinionated film debates"
            ]
          }
        ],
        "opener": "Greetings! Does classical symphonic structure feel transcendent or overly formal to you?",
        "icebreakers": [
          "Transcendent emotional power!",
          "A bit too formal for me!",
          "Auteur cinema and soundscapes!"
        ],
        "prompt": "Critique Julian's musical philosophy, literature comparative studies, and auteur film tastes."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Books, Films & Music 🎬",
        "title": "Sienna",
        "age": "29",
        "location": "📍 1.4 miles away • Media Lab",
        "verified": true,
        "avatar": "🎮",
        "gradient": "linear-gradient(135deg, #be185d 0%, #db2777 100%)",
        "visualDescription": "Interactive narrative designer sitting before multi-screen game development software and concept art.",
        "interests": [
          "#InteractiveNarratives",
          "#SpeculativeFiction",
          "#ElectronicSoundscapes"
        ],
        "anthem": "🎵 'Midnight City' by M83",
        "film": "🎬 'Blade Runner 2049' (Denis Villeneuve)",
        "pages": [
          {
            "tag": "🎮 Interactive Storytelling",
            "bio": "I'm Sienna, 29, a narrative designer for story-driven video games. I craft branching plotlines where player agency shapes complex ethical dilemmas and emotional character arcs.",
            "greenFlags": [
              "✓ Innovative storytelling vision",
              "✓ Blends art and technology",
              "✓ Deep psychological character crafting"
            ],
            "redFlags": [
              "✗ Works intense game launch hours",
              "✗ Tests choice trees on friends",
              "✗ Over-analyzes game mechanics"
            ]
          },
          {
            "tag": "🚀 Speculative Fiction",
            "bio": "I devour cyberpunk, dystopian, and speculative fiction literature. World-building that examines technological ethics and human identity inspires my creative projects.",
            "greenFlags": [
              "✓ Imaginative speculative thinker",
              "✓ Sharp ethical awareness",
              "✓ Visionary world builder"
            ],
            "redFlags": [
              "✗ Pessimistic about AI future",
              "✗ Unrealistic sci-fi comparisons",
              "✗ Endless lore discussions"
            ]
          },
          {
            "tag": "🎹 Electronic Ambient Sound",
            "bio": "Synthesizer soundscapes and ambient electronic beats form the soundtrack to my creative design sessions, evoking futuristic urban atmospheres.",
            "greenFlags": [
              "✓ Modern atmospheric music taste",
              "✓ Great focus playlist curator",
              "✓ Creative ambient sound lover"
            ],
            "redFlags": [
              "✗ Listens to repetitive synth loops",
              "✗ Rejects acoustic folk music",
              "✗ Wears glowing LED headsets"
            ]
          }
        ],
        "opener": "Hello! Do you believe interactive video games can achieve the same narrative depth as classic novels?",
        "icebreakers": [
          "Interactive games are true art!",
          "Novels remain unmatched!",
          "Speculative world-building!"
        ],
        "prompt": "Evaluate Sienna's role as an interactive narrative designer and speculative fiction enthusiast."
      }
    ],
    "daily_habits": [
      {
        "type": "profile",
        "level": "B2",
        "category": "Daily Routines & Habits 🌅",
        "title": "Valerie",
        "age": "30",
        "location": "📍 0.7 miles away • Executive Suites",
        "verified": true,
        "avatar": "⏳",
        "gradient": "linear-gradient(135deg, #334155 0%, #0f172a 100%)",
        "visualDescription": "Focused executive practicing deep work techniques at a minimal wooden desk with an hourglass.",
        "interests": [
          "#DeepWorkFocus",
          "#CircadianOptimization",
          "#StoicPhilosophy"
        ],
        "anthem": "🎵 'Experience' by Ludovico Einaudi",
        "film": "🎬 'Limitless' (Neil Burger)",
        "pages": [
          {
            "tag": "⏳ Deep Work Blocks",
            "bio": "I'm Valerie, 30. As a strategy consultant, I structure my workday around uninterrupted 90-minute 'deep work' blocks, eliminating notifications to maximize cognitive output and flow state.",
            "greenFlags": [
              "✓ Exceptional cognitive focus",
              "✓ High professional productivity",
              "✓ Respects time boundaries"
            ],
            "redFlags": [
              "✗ Ignores messages during focus blocks",
              "✗ Strict no-interruptions rule",
              "✗ Intense work atmosphere"
            ]
          },
          {
            "tag": "☀️ Circadian Rhythm",
            "bio": "I optimize my circadian rhythm by getting direct sunlight exposure within 15 minutes of waking, delaying caffeine intake by two hours, and using dim red light spectrums before sleep.",
            "greenFlags": [
              "✓ Science-backed health optimization",
              "✓ High sustained energy levels",
              "✓ Excellent sleep hygiene"
            ],
            "redFlags": [
              "✗ Explains neurobiology behind coffee",
              "✗ Wears blue-light blocking glasses",
              "✗ Rigid evening light rules"
            ]
          },
          {
            "tag": "📜 Stoic Reflection",
            "bio": "Every evening I practice Stoic journaling, reflecting on what was within my control and mentally rehearsing potential challenges for the upcoming day to cultivate emotional tranquility.",
            "greenFlags": [
              "✓ Emotional resilience and composure",
              "✓ Deep philosophical maturity",
              "✓ Calm under crisis"
            ],
            "redFlags": [
              "✗ Stoic detachment can seem cold",
              "✗ Overly rational problem solving",
              "✗ High self-expectation standards"
            ]
          }
        ],
        "opener": "Greetings! Do you design your daily habits around scientific biohacking or intuitive flexibility?",
        "icebreakers": [
          "Scientific biohacking optimization!",
          "Intuitive flexible living!",
          "Deep work focus blocks!"
        ],
        "prompt": "Evaluate Valerie's deep work methodologies, circadian rhythm optimization, and Stoic philosophy."
      },
      {
        "type": "profile",
        "level": "B2",
        "category": "Daily Routines & Habits 🌅",
        "title": "Gabriel",
        "age": "32",
        "location": "📍 1.6 miles away • Creative Studio",
        "verified": true,
        "avatar": "🎨",
        "gradient": "linear-gradient(135deg, #c026d3 0%, #7e22ce 100%)",
        "visualDescription": "Artistic polymath sketching ideas in a leather journal at a sunlit wooden table surrounded by plants.",
        "interests": [
          "#SlowLiving",
          "#PolymorphicRoutines",
          "#ArtisanalCraft"
        ],
        "anthem": "🎵 'Rivers and Roads' by The Head and the Heart",
        "film": "🎬 'Paterson' (Jim Jarmusch)",
        "pages": [
          {
            "tag": "🌿 Slow Living Ethos",
            "bio": "I'm Gabriel, 32. I embrace a slow-living lifestyle that intentionally resists hyper-productivity culture, choosing instead to savor contemplative morning espresso rituals and artisanal craftsmanship.",
            "greenFlags": [
              "✓ Mindful peaceful lifestyle",
              "✓ Resists burnout pressure",
              "✓ Deep appreciation for craftsmanship"
            ],
            "redFlags": [
              "✗ Refuses to rush for deadlines",
              "✗ Takes 45 minutes to brew coffee",
              "✗ Dislikes fast-paced corporate life"
            ]
          },
          {
            "tag": "📚 Polymorphic Routines",
            "bio": "Rather than rigid schedules, my routines adapt fluidly to seasonal cycles and creative moods—focusing on writing in autumn and outdoor woodworking during summer months.",
            "greenFlags": [
              "✓ Highly adaptable creative flow",
              "✓ Harmonious seasonal living",
              "✓ Diverse artistic pursuits"
            ],
            "redFlags": [
              "✗ Hard to fix exact meeting times",
              "✗ Unpredictable daily schedule",
              "✗ Changes focus frequently"
            ]
          },
          {
            "tag": "🕯️ Evening Unplugging",
            "bio": "My evenings involve candlelit dinners, vinyl listening sessions, and manual fountain pen journaling—fostering authentic tactile connections in an increasingly digitized society.",
            "greenFlags": [
              "✓ Warm tactile atmosphere",
              "✓ Deep authentic connection",
              "✓ Unique artistic lifestyle"
            ],
            "redFlags": [
              "✗ Refuses smart home tech",
              "✗ Candle wax on wooden tables",
              "✗ Romanticizes old-fashioned ways"
            ]
          }
        ],
        "opener": "Hello! Do you lean toward optimizing daily efficiency or embracing the art of slow, seasonal living?",
        "icebreakers": [
          "Embracing slow seasonal living!",
          "Optimizing daily efficiency!",
          "Contemplative morning coffee!"
        ],
        "prompt": "Analyze Gabriel's slow living philosophy, polymorphic routines, and tactile evening rituals."
      }
    ]
  }
};

    const GENERAL_DILEMMAS = [
        {
            type: 'dilemma',
            level: 'A0-A1',
            category: 'Daily Routine 🌅',
            title: 'Morning Person vs Night Owl',
            age: 'Forever',
            location: '📍 Worldwide',
            verified: true,
            optionA: { emoji: '🌅', title: 'Early Bird', desc: 'Up at 6 AM, watching sunrise with green tea and high productivity.' },
            optionB: { emoji: '🌃', title: 'Night Owl', desc: 'Creative energy peaks between 11 PM and 3 AM when the world is quiet.' },
            prompt: 'Which routine fits your true self? Describe your ideal daily schedule.'
        },
        {
            type: 'dilemma',
            level: 'A2',
            category: 'Vacations 🏖️',
            title: 'Beach Resort vs Mountain Hike',
            age: 'All Seasons',
            location: '📍 Ocean or Peaks',
            verified: true,
            optionA: { emoji: '🏖️', title: 'Sunny Beach', desc: 'Warm sand, ocean waves, coconut smoothies, and complete relaxation.' },
            optionB: { emoji: '⛰️', title: 'Mountain Hike', desc: 'Fresh alpine air, steep trails, breathtaking views, and campfire nights.' },
            prompt: 'Where would you rather go on a 2-week holiday? Describe your travel style.'
        },
        {
            type: 'dilemma',
            level: 'B1',
            category: 'Superpowers 🦸‍♂️',
            title: 'Invisibility vs Time Travel',
            age: 'Hypothetical',
            location: '📍 Sci-Fi Universe',
            verified: true,
            optionA: { emoji: '🫥', title: 'Invisibility', desc: 'Sneak anywhere unseen, listen in on secret chats, escape awkward moments.' },
            optionB: { emoji: '⏳', title: 'Time Travel', desc: 'Visit ancient civilizations or leap 100 years into the future.' },
            prompt: 'Which power would you choose and why? How would you use it?'
        },
        {
            type: 'dilemma',
            level: 'B2',
            category: 'Life Values ⚖️',
            title: 'Corporate Career vs Creative Passion',
            age: 'Forever Relevant',
            location: '📍 Career Crossroads',
            verified: true,
            optionA: { emoji: '💼', title: 'Corporate Career', desc: 'Financial security, executive prestige, long hours, corporate structure.' },
            optionB: { emoji: '🎨', title: 'Creative Passion', desc: 'Total artistic freedom, personal purpose, variable and uncertain income.' },
            prompt: 'How do you strike a balance between financial security and personal fulfillment?'
        }
    ];

    function renderSetup() {
        document.getElementById('go-title').textContent = GAME_TITLE;
        document.getElementById('go-meta').textContent = GAME_META;
        const body = document.getElementById('go-body');
        if (!body) return;

        body.innerHTML = `
            <div class="setup-screen" style="text-align: center; max-width: 440px; margin: 0 auto; padding: 1.5rem 1rem;">
              <div style="font-size: 3.2rem; margin-bottom: 0.2rem;">🔥 Tinder Swipe</div>
              <h1 style="font-family:'DM Sans', sans-serif; font-weight: 800; margin-bottom: 0.5rem; background: var(--tinder-gradient); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">This or That?</h1>
              <p style="color:var(--ink-muted); margin-bottom: 1.25rem; line-height: 1.45; font-size:0.95rem;">
                Swipe left or right on Tinder profiles of people, professions, properties, and dilemmas. Practice speaking with level-tailored prompts!
              </p>

              <div class="setup-field" style="margin-bottom: 1rem; text-align: left;">
                <label style="font-weight:700; display:block; margin-bottom:0.4rem; font-size:0.9rem;">🌐 Target Language</label>
                <select class="styled-sel" id="tot-lang-sel" style="width: 100%; padding: 0.75rem; border-radius: 12px; border: 1px solid #e5e7eb; font-weight:600;">
                  <option value="en" selected>🇬🇧 English</option>
                  <option value="french">🇫🇷 Français (French)</option>
                  <option value="italian">🇮🇹 Italiano (Italian)</option>
                  <option value="russian">🇷🇺 Русский (Russian)</option>
                  <option value="greek">🇬🇷 Ελληνικά (Greek)</option>
                </select>
              </div>

              <div class="setup-field" style="margin-bottom: 1rem; text-align: left;">
                <label style="font-weight:700; display:block; margin-bottom:0.4rem; font-size:0.9rem;">🎯 Target CEFR Level</label>
                <select class="styled-sel" id="tot-level-sel" style="width: 100%; padding: 0.75rem; border-radius: 12px; border: 1px solid #e5e7eb; font-weight:600;">
                  <option value="A0_A1">A0–A1: Starter & Basic Words</option>
                  <option value="A2" selected>A2: Elementary & Daily Life</option>
                  <option value="B1">B1: Intermediate & Work/Travel</option>
                  <option value="B2">B2: Upper-Inter & Abstract Concepts</option>
                  <option value="ALL">All Levels Combined (A0–B2)</option>
                </select>
              </div>

              <div class="setup-field" style="margin-bottom: 1.75rem; text-align: left;">
                <label style="font-weight:700; display:block; margin-bottom:0.4rem; font-size:0.9rem;">🎴 Vocabulary Deck & Topic</label>
                <select class="styled-sel" id="tot-deck-sel" style="width: 100%; padding: 0.75rem; border-radius: 12px; border: 1px solid #e5e7eb; font-weight:600;">
                  <option value="appearance" selected>👁️ Physical Appearance (People)</option>
                  <option value="professions">💼 Professions & Careers</option>
                  <option value="properties">🏠 Properties & Real Estate (Flats/Houses)</option>
                  <option value="character">🧠 Character Traits & Personality</option>
                  <option value="hobbies">🎨 Hobbies, Passions & Lifestyle</option>
                  <option value="nationalities">🌍 Nationalities & Cultural Heritage</option>
                  <option value="food">🍜 Food & Cuisine</option>
                  <option value="travel">✈️ Travel & Destinations</option>
                  <option value="entertainment">🎬 Books, Films & Music</option>
                  <option value="daily_habits">🌅 Daily Routines & Habits</option>
                  <option value="mixed">🔥 Mixed Full Deck (All Categories)</option>
                  <option value="dilemmas">⚖️ "This or That" Dilemmas</option>
                </select>
              </div>

              <button class="btn-start-game" onclick="COSY_GAME.start()" style="width: 100%; padding: 0.95rem; font-size: 1.1rem; border-radius: 30px; background: var(--tinder-gradient); color: #fff; border: none; font-weight: 800; cursor: pointer; box-shadow: 0 6px 18px rgba(253,38,125,0.35);">▶ Start Swiping 🔥</button>
            </div>`;
    }

    window.COSY_GAME = {
        deck: [],
        currentIndex: 0,
        swipedChoices: [],
        activeStoryIndex: 0,
        fallbackNotice: null,

        async start() {
            this.fallbackNotice = null;
            const langSel = document.getElementById('tot-lang-sel')?.value || 'en';
            const levelSel = document.getElementById('tot-level-sel')?.value || 'A2';
            const deckSel = document.getElementById('tot-deck-sel')?.value || 'appearance';
            const body = document.getElementById('go-body');

            if (body) body.innerHTML = '<div style="text-align:center;padding:4rem;font-weight:700;color:var(--tinder-pink);">Shuffling Tinder deck... 🔥</div>';
            await new Promise(res => setTimeout(res, 200));

            let activeDecksSource = CEFR_DECKS;

            if (langSel !== 'en') {
                try {
                    const res = await fetch(`decks/${langSel}.json`);
                    if (res.ok) {
                        activeDecksSource = await res.json();
                    }
                } catch (e) {
                    console.warn(`Failed to fetch deck for language ${langSel}, falling back to English.`, e);
                }
            }

            let rawCards = [];

            if (deckSel === 'dilemmas') {
                rawCards = [...GENERAL_DILEMMAS];
                if (levelSel !== 'ALL') {
                    const targetLevelTag = levelSel.replace('_', '-');
                    rawCards = rawCards.filter(c => c.level === targetLevelTag || c.level.includes(targetLevelTag));
                }
            } else {
                const levelsToSearch = levelSel === 'ALL' ? ['A0_A1', 'A2', 'B1', 'B2'] : [levelSel];

                levelsToSearch.forEach(lvl => {
                    const levelData = activeDecksSource[lvl] || {};
                    if (deckSel === 'mixed') {
                        Object.keys(levelData).forEach(cat => {
                            rawCards.push(...levelData[cat]);
                        });
                    } else if (levelData[deckSel]) {
                        rawCards.push(...levelData[deckSel]);
                    }
                });
            }

            if (rawCards.length === 0) {
                // Determine names for user-friendly notice
                const langNames = {
                    french: 'French',
                    italian: 'Italian',
                    russian: 'Russian',
                    greek: 'Greek',
                    en: 'English'
                };
                const catNames = {
                    appearance: 'Physical Appearance',
                    professions: 'Professions',
                    properties: 'Properties',
                    character: 'Character Traits',
                    hobbies: 'Hobbies',
                    nationalities: 'Nationalities',
                    food: 'Food & Cuisine',
                    travel: 'Travel & Destinations',
                    entertainment: 'Books, Films & Music',
                    daily_habits: 'Daily Routines',
                    dilemmas: 'Dilemmas',
                    mixed: 'Mixed Deck'
                };

                const langLabel = langNames[langSel] || (langSel.charAt(0).toUpperCase() + langSel.slice(1));
                const catLabel = catNames[deckSel] || deckSel;
                const lvlLabel = levelSel === 'ALL' ? 'All Levels' : levelSel.replace('_', '–');

                // Try to find any cards in the active deck source
                let showingCat = null;
                const levelsToSearch = levelSel === 'ALL' ? ['A0_A1', 'A2', 'B1', 'B2'] : [levelSel, 'A0_A1', 'A2', 'B1', 'B2'];

                for (const lvl of levelsToSearch) {
                    const levelData = activeDecksSource[lvl] || {};
                    const availableCats = Object.keys(levelData).filter(c => levelData[c] && levelData[c].length > 0);
                    if (availableCats.length > 0) {
                        showingCat = availableCats[0];
                        levelData[showingCat].forEach(c => rawCards.push(c));
                        break;
                    }
                }

                if (rawCards.length > 0 && showingCat) {
                    const showingLabel = catNames[showingCat] || showingCat;
                    this.fallbackNotice = `No ${langLabel} cards for '${catLabel}' at ${lvlLabel} yet — showing ${showingLabel} instead.`;
                } else {
                    // Fall back to English A2 if activeDecksSource was totally empty
                    const levelData = CEFR_DECKS['A2'] || {};
                    Object.keys(levelData).forEach(cat => rawCards.push(...levelData[cat]));
                    this.fallbackNotice = `No ${langLabel} cards available for '${catLabel}' — showing English cards instead.`;
                }
            }

            this.deck = [...rawCards].sort(() => Math.random() - 0.5);
            this.currentIndex = 0;
            this.swipedChoices = [];
            this.activeStoryIndex = 0;

            if (window.COSYGame) {
                COSYGame.init(GAME_ID, langSel, levelSel);
                COSYGame.maxRounds = this.deck.length;
                COSYGame.score = 0;
            }

            this.renderCard();
        },

        renderCard() {
            const body = document.getElementById('go-body');
            if (!body) return;

            if (this.currentIndex >= this.deck.length) {
                this.renderEnd();
                return;
            }

            const item = this.deck[this.currentIndex];
            const isProfile = item.type === 'profile';
            const pages = isProfile && item.pages ? item.pages : [];
            const currentPage = pages[this.activeStoryIndex] || {};

            body.innerHTML = `
              <div class="tinder-app">
                <div class="tinder-top-bar">
                  <div class="tinder-brand">🔥 Tinder Words</div>
                  <div style="font-weight:800; font-size:0.85rem; color:#6b7280;">Card ${this.currentIndex + 1} / ${this.deck.length}</div>
                </div>

                ${this.fallbackNotice ? `
                  <div style="background:#fff3cd; color:#856404; border:1px solid #ffeeba; padding:8px 12px; border-radius:10px; font-size:0.82rem; margin: 0 0 10px 0; text-align:center; font-weight:600; line-height:1.35; box-shadow:0 2px 6px rgba(0,0,0,0.05);">
                    ⚠️ ${this.fallbackNotice}
                  </div>
                ` : ''}

                <div class="card-stack">
                  <div class="tinder-card" id="active-card">
                    <!-- Dynamic Stamp Overlays -->
                    <div class="badge-indicator badge-like" id="badge-right">${isProfile ? '❤️ LIKE' : '👉 THAT'}</div>
                    <div class="badge-indicator badge-pass" id="badge-left">${isProfile ? '❌ NOPE' : '👈 THIS'}</div>
                    <div class="badge-indicator badge-super" id="badge-super">⭐ SUPER LIKE</div>

                    <!-- Hero Avatar Box with Story Bars & Tap Navigation -->
                    <div class="card-hero-box" style="background: ${item.gradient || 'var(--tinder-gradient)'};">
                      ${isProfile && pages.length > 1 ? `
                        <div class="story-bar-container">
                          ${pages.map((_, idx) => `<div class="story-segment ${idx === this.activeStoryIndex ? 'active' : ''}"></div>`).join('')}
                        </div>
                        <div class="tap-zone left" onclick="COSY_GAME.prevStory(event)"></div>
                        <div class="tap-zone right" onclick="COSY_GAME.nextStory(event)"></div>
                      ` : ''}

                      <!-- CEFR Level Badge Overlay -->
                      <div style="position:absolute; top:12px; right:12px; background:rgba(0,0,0,0.6); color:#fff; font-weight:800; font-size:0.75rem; padding:3px 9px; border-radius:12px; backdrop-filter:blur(6px); border:1px solid rgba(255,255,255,0.3); z-index:26;">
                        ${item.level || 'CEFR'}
                      </div>

                      <div class="card-avatar-emoji">${isProfile ? item.avatar : '⚖️'}</div>

                      ${isProfile ? `
                        <div class="card-sub-badge">
                          <span>${currentPage.tag || '🖼️ Profile'}</span>
                          ${pages.length > 1 ? `<span style="opacity:0.75;">(${this.activeStoryIndex + 1}/${pages.length})</span>` : ''}
                        </div>
                      ` : ''}
                    </div>

                    <!-- Card Body Content -->
                    <div class="card-body">
                      <div>
                        <div class="profile-title-row">
                          <span class="profile-name">${item.title}</span>
                          <span class="profile-age">${item.age ? ', ' + item.age : ''}</span>
                          ${item.verified ? '<span class="verified-icon" title="Verified Tinder Profile">☑️</span>' : ''}
                        </div>
                        <div class="profile-meta-row">
                          <span>${item.location || '📍 Nearby'}</span>
                          <span>• ${item.category}</span>
                        </div>

                        ${item.visualDescription ? `
                          <div style="font-size:0.78rem; font-style:italic; color:#4b5563; background:#f0f9ff; padding:6px 10px; border-radius:8px; margin-bottom:0.6rem; border-left:3px solid #0284c7;">
                            🎨 <strong>Visual Details:</strong> ${item.visualDescription}
                          </div>
                        ` : ''}

                        ${isProfile ? `
                          <div class="interest-tags">
                            ${(item.interests || []).map(tag => `<span class="tag-pill">${tag}</span>`).join('')}
                          </div>

                          ${currentPage.bio ? `
                            <div class="profile-bio-box">
                              "${currentPage.bio}"
                            </div>
                          ` : ''}

                          ${currentPage.greenFlags || currentPage.redFlags ? `
                            <div class="flag-grid">
                              ${(currentPage.greenFlags || []).map(g => `<div class="flag-card green">${g}</div>`).join('')}
                              ${(currentPage.redFlags || []).map(r => `<div class="flag-card red">${r}</div>`).join('')}
                            </div>
                          ` : ''}

                          <div style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:0.65rem;">
                            ${item.anthem ? `
                              <div style="font-size:0.78rem; font-weight:700; color:#4b5563; background:#f3f4f6; padding:5px 10px; border-radius:10px; border:1px solid #e5e7eb;">
                                ${item.anthem}
                              </div>
                            ` : ''}
                            ${item.film ? `
                              <div style="font-size:0.78rem; font-weight:700; color:#4b5563; background:#f0f9ff; padding:5px 10px; border-radius:10px; border:1px solid #bae6fd;">
                                ${item.film}
                              </div>
                            ` : ''}
                          </div>
                        ` : `
                          <div class="dilemma-grid" style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:0.8rem;">
                            <div class="dilemma-opt" style="background:#f9fafb; padding:10px; border-radius:12px; border:1px solid #e5e7eb; text-align:center;">
                              <div style="font-size:2.2rem; margin-bottom:4px;">${item.optionA.emoji}</div>
                              <div style="font-weight:800; font-size:0.95rem; color:#111827;">${item.optionA.title}</div>
                              <div style="font-size:0.78rem; color:#6b7280; margin-top:4px; line-height:1.3;">${item.optionA.desc}</div>
                            </div>
                            <div class="dilemma-opt" style="background:#f9fafb; padding:10px; border-radius:12px; border:1px solid #e5e7eb; text-align:center;">
                              <div style="font-size:2.2rem; margin-bottom:4px;">${item.optionB.emoji}</div>
                              <div style="font-weight:800; font-size:0.95rem; color:#111827;">${item.optionB.title}</div>
                              <div style="font-size:0.78rem; color:#6b7280; margin-top:4px; line-height:1.3;">${item.optionB.desc}</div>
                            </div>
                          </div>
                        `}

                        <div class="prompt-box">
                          💬 <strong>Discussion Prompt (${item.level || 'CEFR'}):</strong> ${item.prompt}
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <!-- Tinder Action Control Bar -->
                <div class="tinder-actions">
                  <button class="t-btn btn-rewind" id="btn-rewind" title="Rewind / Undo Last Swipe" ${this.swipedChoices.length === 0 ? 'disabled style="opacity:0.4;cursor:default;"' : ''}>🔄</button>
                  <button class="t-btn btn-pass" id="btn-swipe-left" title="Pass / Swipe Left">❌</button>
                  <button class="t-btn btn-super" id="btn-super-like" title="Super Like!">⭐</button>
                  <button class="t-btn btn-like" id="btn-swipe-right" title="Like / Swipe Right">❤️</button>
                </div>
                <div style="font-size:0.78rem; color:#9ca3af; margin-top:0.5rem;">
                  Tap photo sides to flip details • Keyboard Arrow Keys supported!
                </div>
              </div>
            `;

            this.attachDragEvents();
        },

        nextStory(e) {
            if (e) e.stopPropagation();
            const item = this.deck[this.currentIndex];
            if (item && item.pages && this.activeStoryIndex < item.pages.length - 1) {
                this.activeStoryIndex++;
                this.renderCard();
            }
        },

        prevStory(e) {
            if (e) e.stopPropagation();
            if (this.activeStoryIndex > 0) {
                this.activeStoryIndex--;
                this.renderCard();
            }
        },

        attachDragEvents() {
            const card = document.getElementById('active-card');
            const btnLeft = document.getElementById('btn-swipe-left');
            const btnRight = document.getElementById('btn-swipe-right');
            const btnSuper = document.getElementById('btn-super-like');
            const btnRewind = document.getElementById('btn-rewind');

            const badgeLeft = document.getElementById('badge-left');
            const badgeRight = document.getElementById('badge-right');

            if (!card) return;

            let startX = 0, currentX = 0, isDragging = false;

            const removeKeyHandler = () => {
                if (this._onKeyDown) {
                    window.removeEventListener('keydown', this._onKeyDown);
                    this._onKeyDown = null;
                }
            };

            removeKeyHandler();

            const onStart = (e) => {
                isDragging = true;
                startX = e.type.includes('touch') ? e.touches[0].clientX : e.clientX;
                card.classList.add('dragging');
            };

            const onMove = (e) => {
                if (!isDragging) return;
                currentX = (e.type.includes('touch') ? e.touches[0].clientX : e.clientX) - startX;
                const rotate = currentX * 0.08;
                card.style.transform = `translate3d(${currentX}px, 0, 0) rotate(${rotate}deg)`;

                if (currentX > 30) {
                    if (badgeRight) badgeRight.style.opacity = Math.min(1, (currentX - 30) / 70);
                    if (badgeLeft) badgeLeft.style.opacity = '0';
                } else if (currentX < -30) {
                    if (badgeLeft) badgeLeft.style.opacity = Math.min(1, (-currentX - 30) / 70);
                    if (badgeRight) badgeRight.style.opacity = '0';
                } else {
                    if (badgeRight) badgeRight.style.opacity = '0';
                    if (badgeLeft) badgeLeft.style.opacity = '0';
                }
            };

            const onEnd = () => {
                if (!isDragging) return;
                isDragging = false;
                card.classList.remove('dragging');

                window.removeEventListener('mousemove', onMove);
                window.removeEventListener('mouseup', onEnd);
                window.removeEventListener('touchmove', onMove);
                window.removeEventListener('touchend', onEnd);

                if (currentX > 100) {
                    removeKeyHandler();
                    this.executeSwipe('right');
                } else if (currentX < -100) {
                    removeKeyHandler();
                    this.executeSwipe('left');
                } else {
                    card.style.transform = 'translate3d(0,0,0) rotate(0deg)';
                    if (badgeLeft) badgeLeft.style.opacity = '0';
                    if (badgeRight) badgeRight.style.opacity = '0';
                }
            };

            card.addEventListener('mousedown', (e) => {
                onStart(e);
                window.addEventListener('mousemove', onMove);
                window.addEventListener('mouseup', onEnd);
            });

            card.addEventListener('touchstart', (e) => {
                onStart(e);
                window.addEventListener('touchmove', onMove, { passive: true });
                window.addEventListener('touchend', onEnd);
            }, { passive: true });

            btnLeft?.addEventListener('click', () => {
                removeKeyHandler();
                this.executeSwipe('left');
            });

            btnRight?.addEventListener('click', () => {
                removeKeyHandler();
                this.executeSwipe('right');
            });

            btnSuper?.addEventListener('click', () => {
                removeKeyHandler();
                this.executeSwipe('super');
            });

            btnRewind?.addEventListener('click', () => {
                removeKeyHandler();
                this.rewind();
            });

            this._onKeyDown = (e) => {
                if (e.key === 'ArrowLeft') {
                    removeKeyHandler();
                    this.executeSwipe('left');
                } else if (e.key === 'ArrowRight') {
                    removeKeyHandler();
                    this.executeSwipe('right');
                }
            };
            window.addEventListener('keydown', this._onKeyDown);
        },

        executeSwipe(direction) {
            const card = document.getElementById('active-card');
            const item = this.deck[this.currentIndex];
            if (!card || !item) return;

            const isLike = direction === 'right' || direction === 'super';
            const flyX = isLike ? 500 : -500;
            const flyRotate = isLike ? 25 : -25;

            card.style.transition = 'transform 0.3s ease, opacity 0.3s ease';
            card.style.transform = `translate3d(${flyX}px, 0, 0) rotate(${flyRotate}deg)`;
            card.style.opacity = '0';

            if (window.gameUtils && window.gameUtils.playGameSound) {
                window.gameUtils.playGameSound('click');
            }

            const choiceObj = {
                item,
                type: direction,
                choice: isLike ? (item.type === 'profile' ? 'Matched ❤️' : 'Option B 👉') : (item.type === 'profile' ? 'Passed ❌' : 'Option A 👈')
            };

            this.swipedChoices.push(choiceObj);
            if (window.COSYGame) COSYGame.score += (direction === 'super' ? 20 : 10);

            setTimeout(() => {
                if (isLike && item.type === 'profile') {
                    this.openMatchModal(item);
                } else {
                    this.activeStoryIndex = 0;
                    this.currentIndex++;
                    this.renderCard();
                }
            }, 260);
        },

        rewind() {
            if (this.swipedChoices.length === 0 || this.currentIndex === 0) return;
            this.swipedChoices.pop();
            this.currentIndex--;
            this.activeStoryIndex = 0;
            if (window.COSYGame) COSYGame.score = Math.max(0, COSYGame.score - 10);
            this.renderCard();
        },

        openMatchModal(item) {
            const modal = document.getElementById('match-modal');
            const nameEl = document.getElementById('match-item-name');
            const emojiEl = document.getElementById('match-item-emoji');
            const openerEl = document.getElementById('match-opener-text');
            const icebreakersEl = document.getElementById('chat-icebreakers');
            const messagesContainer = document.getElementById('chat-messages-container');
            const inputEl = document.getElementById('chat-input');

            if (nameEl) nameEl.textContent = item.title;
            if (emojiEl) emojiEl.textContent = item.avatar || '🔥';
            if (openerEl) openerEl.textContent = item.opener || `Hey! What made you swipe right on ${item.title} today? 🔥`;
            if (inputEl) inputEl.value = '';

            if (messagesContainer) {
                messagesContainer.innerHTML = `
                    <div style="background: rgba(255,255,255,0.95); color: #1f2937; padding: 8px 12px; border-radius: 12px 12px 12px 2px; font-size: 0.85rem; max-width: 85%; align-self: flex-start; box-shadow: 0 2px 6px rgba(0,0,0,0.15);">
                      <span>${item.opener || `Hey! What made you swipe right on ${item.title} today? 🔥`}</span>
                    </div>
                `;
            }

            if (icebreakersEl) {
                const pills = item.icebreakers || ['I love this profile!', 'Defend your choice 💬', 'Tell me more!'];
                icebreakersEl.innerHTML = pills.map(p => `
                    <button onclick="COSY_GAME.quickChat('${p.replace(/'/g, "\'")}')" style="background: rgba(255,255,255,0.25); border: 1px solid rgba(255,255,255,0.4); color: #fff; padding: 4px 10px; border-radius: 16px; font-size: 0.75rem; font-weight: 600; cursor: pointer; transition: background 0.15s;">${p}</button>
                `).join('');
            }

            if (modal) modal.classList.add('open');

            if (window.gameUtils && window.gameUtils.createConfetti) {
                window.gameUtils.createConfetti();
            }
        },

        quickChat(text) {
            const inputEl = document.getElementById('chat-input');
            if (inputEl) {
                inputEl.value = text;
                this.sendChatMessage();
            }
        },

        sendChatMessage() {
            const inputEl = document.getElementById('chat-input');
            const messagesContainer = document.getElementById('chat-messages-container');
            if (!inputEl || !inputEl.value.trim() || !messagesContainer) return;

            const userText = inputEl.value.trim();
            inputEl.value = '';

            // Render User Bubble
            const userMsg = document.createElement('div');
            userMsg.style.cssText = 'background: #fd267d; color: #ffffff; padding: 8px 12px; border-radius: 12px 12px 2px 12px; font-size: 0.85rem; max-width: 85%; align-self: flex-end; box-shadow: 0 2px 6px rgba(0,0,0,0.15); margin-top: 4px;';
            userMsg.textContent = userText;
            messagesContainer.appendChild(userMsg);
            messagesContainer.scrollTop = messagesContainer.scrollHeight;

            if (window.COSYGame) COSYGame.score += 5;

            // Delayed Level-Tailored Reply
            setTimeout(() => {
                const replies = [
                    "Haha I love that answer! Perfect match energy 🔥",
                    "Great point! I knew we would hit it off! 🚀",
                    "Spot on! Definitely agree with you there ☕",
                    "Fascinating perspective! You really know your stuff 🎉"
                ];
                const replyText = replies[Math.floor(Math.random() * replies.length)];
                const replyMsg = document.createElement('div');
                replyMsg.style.cssText = 'background: rgba(255,255,255,0.95); color: #1f2937; padding: 8px 12px; border-radius: 12px 12px 12px 2px; font-size: 0.85rem; max-width: 85%; align-self: flex-start; box-shadow: 0 2px 6px rgba(0,0,0,0.15); margin-top: 4px;';
                replyMsg.textContent = replyText;
                messagesContainer.appendChild(replyMsg);
                messagesContainer.scrollTop = messagesContainer.scrollHeight;
            }, 600);
        },

        closeMatchModal() {
            const modal = document.getElementById('match-modal');
            if (modal) modal.classList.remove('open');
            this.activeStoryIndex = 0;
            this.currentIndex++;
            this.renderCard();
        },

        renderEnd() {
            if (window.COSYScores && window.COSYGame) {
                COSYScores.save(GAME_ID, COSYGame.language || 'en', COSYGame.level || 'intermediate', COSYGame.score || 0);
            }

            if (window.gameUtils) {
                if (window.gameUtils.playGameSound) window.gameUtils.playGameSound('success');
                if (window.gameUtils.createConfetti) window.gameUtils.createConfetti();
            }

            const body = document.getElementById('go-body');
            if (!body) return;

            const scoreVal = window.COSYGame ? COSYGame.score : this.swipedChoices.length * 10;

            body.innerHTML = `
                <div class="setup-screen" style="max-width: 440px; margin: 0 auto; text-align: center; padding: 1.5rem 1rem;">
                  <h2>Swipe Deck Complete! 🎉</h2>
                  <div style="font-size: 2.5rem; font-weight: 800; color: var(--tinder-pink); margin: 0.5rem 0;">${scoreVal} Points</div>
                  <p style="color:var(--ink-muted);">Here are your Tinder matches & passed items. Discuss your reasons with your learning partner!</p>

                  <div style="text-align: left; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 16px; padding: 1rem; margin: 1.25rem 0; max-height: 280px; overflow-y: auto;">
                    <h4 style="margin-top: 0; font-family:'DM Sans', sans-serif; font-weight:800;">Your Match History:</h4>
                    ${this.swipedChoices.map(c => `
                      <div style="padding: 8px 0; border-bottom: 1px dashed rgba(0,0,0,0.1); display: flex; justify-content: space-between; align-items: center; font-size: 0.88rem;">
                        <div>
                          <strong>${c.item.title}</strong> <span style="font-size:0.75rem; background:#f3f4f6; padding:2px 6px; border-radius:8px; margin-left:4px;">${c.item.level || ''}</span>
                          <div style="font-size: 0.78rem; color: #6b7280;">${c.item.category}</div>
                        </div>
                        <span style="font-weight: 700; padding: 4px 10px; border-radius: 20px; font-size: 0.78rem; background: ${c.choice.includes('Matched') || c.choice.includes('Option B') ? '#d1fae5' : '#fee2e2'}; color: ${c.choice.includes('Matched') || c.choice.includes('Option B') ? '#047857' : '#b91c1c'};">
                          ${c.choice}
                        </span>
                      </div>
                    `).join('')}
                  </div>

                  <div style="display:flex; gap:1rem; justify-content:center;">
                    <button class="btn-start-game" onclick="COSY_GAME.start()" style="padding:0.85rem 1.2rem; border-radius:30px; background:var(--tinder-gradient); color:#fff; font-weight:800; border:none; cursor:pointer;">Swipe Again 🔄</button>
                    <button class="btn-g-danger" onclick="COSY_GAME.reset()">Deck Settings ⚙️</button>
                  </div>
                </div>`;
        },

        reset: renderSetup
    };

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', renderSetup);
    } else {
        renderSetup();
    }
})();
