...
(function() {
    ...
    // Comprehensive cross-platform standard emoji dataset covering all devices/OS
    window.emojiData = window.emojiData || [
        '😀','😃','😄','😁','😆','😅','🤣','😂','🙂','🙃','😉','😊','😇','🥰','😍','🤩','😘','😗','☺️','😚',
        '😙','😋','😛','😜','🤪','😝','🤑','🤗','🤭','🤫','🤔','🤐','🤨','😐','😑','😶','😏','😒','🙄','😬',
        '🤥','😌','😔','😪','🤤','😴','😷','🤒','🤕','🤢','🤮','🤧','🥵','🥶','🥴','😵','🤯','🤠','🥳','😎',
        '🤓','🧐','😕','😟','🙁','😮','😯','😲','😳','🥺','😦','😧','😨','😰','😥','😢','😭','😱','😖','😣',
    ...
    window.COSY_GAME = {
        ...
        async start() {
            ...
            if (mode === 'guess') {
                let pool = vocab.filter(v => v.emoji);

                // Fallback to generating emoji guess cards from window.emojiData if vocab items lack emojis
                if (pool.length < 4) {
                    const fallbackEmojis = shuffle(window.emojiData).slice(0, 20);
                    const sampleWords = vocab.map(v => v.word);
                    pool = fallbackEmojis.map((e, idx) => ({
                        emoji: e,
                        word: sampleWords[idx] || `Item ${idx + 1}`
                    }));
                }
                ...
                const renderGuess = () => {
                    ...
                    const otherWords = shuffle(vocab.map(v => v.word).filter(w => w !== current.word)).slice(0, 3);
                    const options = shuffle([current.word, ...otherWords]);
                    body.innerHTML = `
                        <div class="score-bar">
